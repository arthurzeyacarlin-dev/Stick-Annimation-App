...
D-0102 proposes [`SPEC-0010 — Project Safety and Recovery`](specs/0010-project-safety-and-recovery.md) as the next beta-readiness foundation: Save and Exit; one separate local latest emergency recovery draft; and startup Recover Work / Discard Draft with final fault/regression proof. The recovery draft is not an official save and never silently replaces a saved project. The proposal is planning-complete but unapproved for implementation; every phase remains Unauthorized/Not started.
...
- review Proposed SPEC-0010, then separately authorize only Phase 1 before implementation
