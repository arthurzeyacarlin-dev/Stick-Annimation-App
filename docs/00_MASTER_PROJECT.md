...
D-0120 creates [`SPEC-0012 — Diamond Animator Guidance Assistant`](specs/0012-diamond-animator-guidance-assistant.md) from Arthur's accepted six-phase product direction. It turns the currently inert Home AI Assistant card into a separate guidance-only ChatGPT-Classic-style surface with bounded local sessions, fixed Terra guidance, optional truthful hosted search, dictation, and beta hardening. The Assistant has no animation/project mutation ownership, shares no AI Animator route/job/storage contract, and may explain navigation without controlling the interface. The planning package authorizes no implementation phase, live/paid model/search/transcription call, deployment, or Git publication; Phase 1 is ready only for later separate authorization after this package is reviewed and durably published.
...
- preserve SPEC-0012 as the decision-complete guidance-Assistant plan under D-0120; do not start Phase 1, call a provider, or publish until separately authorized
