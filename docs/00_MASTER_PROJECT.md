...
D-0085 creates [`SPEC-0008 — Conversational AI Animator and Editable Video Reconstruction`](specs/0008-conversational-ai-animator.md) as **Proposed; decision-complete and review-ready; no implementation phase authorized**. D-0086/GIT-067 publishes that exact planning-only package at `ba5ecd694df78414240ce83a9140c5196334ebdc`. Its bounded V1 sequence is Terra conversation/intent/jobs → provider-neutral reference video → deterministic frame slicing → isolated editable drawing reconstruction → registered-command conversational edits → beta/export closeout. Phase 1 is ready only for Arthur's separate proposal approval and Phase 1 authorization; Phases 2–6 remain gated. No provider, paid call, runtime implementation or deployment occurred in proposal publication.
