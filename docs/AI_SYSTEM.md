...
Last traced: 2026-09-01 through published SPEC-0004 Phase 1 and D-0037's Phase 2 activation design
...
D-0037 authorizes the next hidden local engine only after its docs activation is published. Phase 2 will normalize the unchanged fixed plans' important poses, create deterministic cubic-eased shortest-turn in-betweens, and bake every output slot into its own ordinary Stick keyframe before Preview. It deliberately adds no natural-language route, recipe catalog, Terra/provider/API call, working Task/Reasoning control, or normal visible UI. Phase 1 remains the default transaction behavior until a later phase explicitly selects the new engine.
...
Phase 2's private review rule is stricter: Arthur must not see that blue picker again, even in the unpublished copy. Four separate disposable loopback copies/links preload wave, jump, bow, and dodge one at a time and expose only the ordinary Stick canvas, timeline, AI transaction area, and manual tools. No query-controlled review surface, tester overlay/control, product route, public asset, or review import may be published.
...
- the authorized Phase 2 motion engine remains fully local/$0/provider-free; it sends no prompt or project data anywhere
