...
Last traced: 2026-09-23 through D-0126/GIT-092 publication, integration, runtime activation and cleanup of SPEC-0012 Phase 4. SPEC-0012 Phases 1–4 are fully closed; Phase 5 is entry-ready but unauthorized/not started. SPEC-0008 Phases 2–6 remain paused. SPEC-0012 transcription/public beta/deployment remain unauthorized.

## SPEC-0012 guidance Assistant — hosted search fully closed through D-0126/GIT-092
...
The final accepted live proof used fixed `gpt-5.6-terra`/Medium, one request and one tool call, processed eight sources, displayed three, persisted citations across reload, left the project sentinel unchanged and produced a `$0.050754` application receipt. Two earlier authorized searches reached the hosted tool but failed before application receipts, so their provider cost is unknown. The accepted corrupt-row correction preserves an unreadable canonical row byte-for-byte, permits a healthy new chat, rejects overwrite/capacity hazards and made zero new live/provider/search calls. D-0126/GIT-092 publish and fully close Phase 4. After a stale process-global pre-search provider was reproduced on the still-running canonical dev server, restarting only that server activated the integrated provider; the identical prompt returned two official YouTube Help citations. That verification made one additional live request whose exact application receipt was not extracted into this record. Transcription, public beta and deployment remain separate; the protected workspace AI Animator path is unchanged.
