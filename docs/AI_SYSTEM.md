...
Last traced: 2026-09-25 through D-0140's accepted but unpublished SPEC-0014 Phase 2 correction. SPEC-0008 Phases 2–6 remain paused; SPEC-0014 Phases 3–4, public beta and deployment remain unauthorized.
...
SPEC-0014 Phase 1 provides one local notification store and two view-scoped bells: Home's all-event inbox and Assistant's reply-completion/failure-only inbox. D-0140's accepted Phase 2 review copy now adds app-lifetime Assistant/Terra observers for jobs the user already started. They commit/reverify the authoritative session/ledger terminal before deriving one exact session/turn/job or project/generation/job notification, with no automatic resubmission. Only unread items are visible; an exact watched origin produces no visible row/ring. The Assistant row names the saved chat and Terra row the captured project; clicking focuses the exact source and clears the row only after confirmed arrival. Both bells share one read state. This remains unpublished until GIT-100, so it is a separate-review-copy behavior claim, not current canonical-main behavior. No model, reasoning, search, dictation, prompts, limits, retry, credits, animation mutation or configured/disconnected state changed. Current Terra chat does not emit the dormant future **AI Animator finished animating** event; SPEC-0008 stays paused.
...
The plan permits at most 50 persisted sessions with no silent eviction; blank chats are not persisted; the same Terra call returns the answer and automatic title; manual rename wins permanently; delete is confirmed. Truthful activity is server-event driven: Thinking, Searching the internet only during a real hosted search, and Finalizing answer only while composing after reasoning/tools. The accepted inherited presentation cycle is exactly one second bright, one second dark and 1.75 seconds paused; reduced motion is static. GIT-095 publishes Phase 5 microphone dictation with explicit Stop, editable text, explicit Send and no deliberate raw-audio persistence. D-0133 accepts Phase 6's explicit offline terminal/retry flow: no automatic retry or fake answer; user-selected retry reuses the original turn with a fresh job identity and at most one accepted answer.

D-0131 keeps guidance/search on fixed `gpt-5.6-terra` and adds an independent private transcription boundary using `gpt-transcribe` at the official audio endpoint. An explicit click requests the device; an audio worklet produces bounded mono WAV and the analyser samples the actual waveform. Cancel discards; Stop makes one no-retry transcription POST; a validated transcript enters the editable composer without triggering guidance. The route is loopback-only, uses the existing ignored server-only credential, and shares no project/animation or AI Animator data owner. Limits are 120 seconds/20 MiB, a 45-second transcription deadline, two active requests, 500 bounded cancellation identities, 6,000 transcript characters/12,000 UTF-8 bytes, and a dated maximum `$0.009` estimated audio charge at `$0.0045`/minute. Returned-model echo is optional in documented transcription JSON: a mismatch fails, absence is recorded as `null`, and independent provider-returned attestation is unproven. This phase-specific interpretation leaves fixed-Terra guidance identity checks unchanged.

Phase 6 technical proof seals 20 accepted source and 80 evidence bindings at manifest SHA-256 `a0e3eea7e973eb183ff274cfd9ff868d3e4b7712e60e431c9db1c67057702229`; offline/retry/reload, 90 stress, 10 restart, protected-product, 11 AI Animator, accessibility/security/performance groups passed with zero automated real/paid calls. D-0134/GIT-096 publishes it as `20612c3…`, preserves 82 files/4,888 KiB and closes SPEC-0012. The app has no cumulative temporary search/review budget; permanent per-request safety/source/time/token/storage/transcription/concurrency/retry bounds remain. Public-beta authentication, ownership, abuse, privacy, credits and deployment remain outside the local/private target.
