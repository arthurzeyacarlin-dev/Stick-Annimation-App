...
- D-0094 preserves the accepted standalone [`SPEC-0009 — Animation Export`](specs/0009-animation-export.md) planning result with exactly three separately gated phases: Choose and Watch; Create and Save Video; Social Destinations and Final Testing. GIT-074 publishes the planning-only package at `30f6f85e5c483c9df15946e7a451bf2a31374f2e`. It is **Proposed**, not Approved; no phase or programming is authorized. V1 is local-only, reads one immutable saved revision, writes a validated MP4 through Finder, defaults destination shapes to contain/no-crop framing and does not log in to, upload to or post to social services.
