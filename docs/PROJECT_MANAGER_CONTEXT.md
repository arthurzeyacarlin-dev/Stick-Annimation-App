...
- D-0085 creates SPEC-0008 as Proposed, decision-complete and review-ready without implementation authority. Its six-phase V1 is fixed Terra conversation/jobs → provider-neutral reference video → deterministic frame bundle → ordinary editable reconstruction → registered-command edits → beta/export closeout. It preserves the manual registry/atomicity laws, makes Sora 2/Videos deprecation a provider-neutrality constraint, and names all later cost/privacy/security gates. Phase 1 awaits separate proposal publication and Arthur authorization; no PM V4 handoff has been created.
