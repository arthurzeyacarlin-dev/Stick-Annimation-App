...
- The SPEC-0005 docs-only activation is published/integrated at `2b4f00e7a122c196b2c0600144cd638b461bbb2f`. D-0044 records Arthur's acceptance of the exact six-path Phase 1 proof-only result: the older wave is now protected as a readability floor by real `0 → … → 11 → 0` playback, independent mutation/reference evidence, and ordinary-app human review. Phase 1 changes no runtime motion; its publication is pending and Phase 2 is not authorized.
