...
## Current stopping point — SPEC-0012 Phase 4 fully closed; Phase 5 entry-ready but unauthorized

D-0120/GIT-089 establish [`SPEC-0012 — Diamond Animator Guidance Assistant`](specs/0012-diamond-animator-guidance-assistant.md); D-0121–D-0123/GIT-090 fully close Phase 1. D-0124/GIT-091 close combined Phase 2/absorbed Phase 3. D-0125 accepts Phase 4 hosted search plus corrupt-row-safe persistence, and D-0126/GIT-092 `23816164592743a59d4cc3b83ffd0ac39844d654` publish/integrate its exact 27 technical plus 15 reviewed record/tree paths. Canonical `main`, `origin/main`, and live GitHub `main` synchronized at clean `0/0` before this terminal record.

Phase 4 is active in the real app. Internal/stable guidance remains zero-search; genuine current/external questions may use fixed-Terra hosted `web_search`, with real Searching/Finalizing events, validated public HTTPS citations, bounded tools/sources/query/time/cost, `store:false`, and no animation/project mutation. The first real-main check reproduced one activation-only fault: the surviving dev-server process-global Assistant singleton retained the old local-only provider after the Git fast-forward. Restarting only that server cleared the singleton; the identical YouTube Shorts question then returned a hosted-search answer with two official YouTube Help citations. No source or proof byte changed.

Arthur later asked to cancel a request that had remained on Finalizing for about five minutes. The canonical dev server was stopped and restarted without a code change; the old job immediately returned `404`, proving it was gone, and `/assistant` returned HTTP 200 again. At this handoff the real app is listening on port 3000 under PID 38389. Refreshing the browser begins from the fresh server; the cancelled prompt will not continue running.

Immutable Phase 4 manifest SHA-256 is `08ff725e2d9bc658e198462e171623b1290d7b7de0b82bb3da93d085e4cfd095`; validation receipt SHA-256 is `3107a15f9193719fe3ef1b8461f1003187ae75157f2786956baca4096a528060`; source digest is `073bc363b1ba26e6be035df69136f7288687df028ed92ec9701ab649c17517fd`, with 27 source and 104 evidence bindings. Proof passed 47 deterministic oracle, 23 main browser, 27 corrupt-row, 20 fault, 90 inherited Assistant browser, 60 presentation, 21 protected-browser, ten restart, 33 repeated-browser and eleven Animator-smoke assertions plus thirteen protected oracles, TypeScript, focused lint/build and exact diff. The full build retains the inherited untouched dev AI-cost route type failure; physical devices/non-Chromium remain unproven.

All 823 Phase 4 proof files/33,608,214 bytes are preserved at `output/recovery/GIT-092-spec0012-phase4-2381616/phase-4`; source/destination checksums match and `SHA256SUMS.txt` hashes to `cd2ef1927802f9aac3d9b6e8b4bd8d6c87309458e5526378529e91f73ddc7d42`. The accepted review PID 31449 is stopped, port 58040 is closed, and the obsolete review worktree/local branch are removed under D-0054.

Exact next safe step: Phase 5 microphone dictation is entry-ready but remains **Unauthorized; Not started** and requires its separate provider/model/privacy/cost decision and authorization. Phase 6 beta hardening, public exposure and deployment remain unauthorized. Do not resume paused SPEC-0008, mutate animation/project owners, deploy, or begin Phase 5 automatically.
