...
SPEC-0005 status: **Phase 1 is Verified, published, and integrated. The first former Phase 2 is rejected/removed. GIT-041 published D-0046's eight-phase restructure at `46b97556ec4e9c9249a1ff354c546f7b1c32ea4d`. A distinct current Phase 2 body-safety result is preserved in stopped `/Users/arthurcarlin/.codex/worktrees/7dfc/stick-animation-app`, detached at `46b97556…`, empty index, exact eight phase paths, with 17,826-byte manifest SHA-256 `ea7ab9c5a142843b109ad49924f4709660347265099740312a666eca07972cd4`. Publication is blocked; it is not accepted, Verified, published, or rejected visible motion, and correction has not started.** D-0047's specification correction is ready for independent PM review. Phases 3–8 remain Unauthorized/Not started and no executor is active.
Current result: D-0047 defines checked immutable important-pose selection with no final frames/document, independent stationary-rest qualification, final binding/selection/all-frame/semantic revalidation, exactly one final candidate/Preview door, and a 10,000+10,000 proof floor. Phase 4's future exact twelve-path ceiling includes the safety module/current validators and permits only two-pass mechanics-qualified takeoff/airborne/contact; Phase 2 continues rejecting airborne. This task changes documentation only and leaves `/7dfc/`, `http://127.0.0.1:57795/`, all runtime/proof bytes, and Git state unchanged.
...
GIT-040 is complete in exact 12-document commit `b5ddd5e3f4fb3b19e5c7c2be8a1bd35b0f8d6070`; its former Phase 2 attempt was later rejected. GIT-041 is complete in exact canonical-main commit `46b97556ec4e9c9249a1ff354c546f7b1c32ea4d`, parent `b5ddd5e3…`, message `Restructure SPEC-0005 around body safety`. The distinct later Phase 2 technical result exists only as the stopped publication-blocked `/7dfc/` bytes described above.
...
The restructured Phase 2 remains technical-only and planner-independent with exactly eight tracked paths. Its correction proof must run `checked selection → test-local provisional in-betweens from returned poses → final completion/revalidation`, retain at least 10,000 valid/invalid plus 10,000 mirrored fixed-seed cases, and add tampered-selection, stale-binding, unsafe-rounded/repair-frame, helper-export, selection-to-Preview, and legacy/trust/skip/action/fixture bypass failures. Safe left/right/mirror/grounded rise-lower cases prove only 2D placement. The historical `ea7ab9c5…` manifest is never overwritten or upgraded into acceptance. One ordinary same-worktree smoke process may exist; if the old one exits, a later authorized correction may verify/stop it then start one replacement in `/7dfc/`, never concurrently or in a second copy.
...
GIT-041 is complete at `46b97556ec4e9c9249a1ff354c546f7b1c32ea4d`. The exact next action is independent PM review of this twelve-document D-0047 correction. If accepted, a separate GIT-042 docs publication must land from canonical `46b97556…`; no runtime work is part of that publication. Only a later explicit instruction may authorize the same-worktree `/7dfc/` correction executor. Before edits it must verify the executor is stopped, index empty, exact eight dirty paths and historical hashes/manifest, and the ordinary server/process state; prove the docs publication does not overlap those paths; then reconcile only detached HEAD to the docs SHA while hashing the eight dirty bytes before/after. Any drift/extra/staged/overlap fails closed. Correct only the same eight paths, write a new manifest, return the packet, and stop. Do not start Phase 3–8, create/delete an app copy, run two review processes, touch `/8de8/`, perform provider/API/paid/Drawing/deployment work, or publish runtime.
...
Proven for SPEC-0005 Phase 1: the exact independent historical-reference catalog, all required bad-motion rejections, complete two-viewport playback capture, timing/landmark/stability/limb checks, TypeScript/scoped-lint/diff/scope, permanent regression, zero-egress evidence, exact source/artifact bindings, Arthur's readability-floor acceptance, and review cleanup. Proven by this control-plane task: canonical GIT-041 basis, the PM stop's two interface blockers, the decision-complete two-stage API, exact Phase 2/Phase 4 ceilings, correction proof matrix, and same-worktree handoff. Proven read-only about `/7dfc/`: stopped/detached/empty-index/eight-path state and historical manifest hash/size supplied and rechecked. Explicitly not proven or changed: corrected runtime bytes, a passing correction manifest, accepted/Verified/published Phase 2, improved/new visible motion, whole-body pose creation, airborne mechanics, natural paths, gait, semantic actions, planner routing, provider behavior, or professional-release quality. The ordinary regression instance proves only observed existing app behavior, never safety or new motion.
...
SPEC-0001 Phases 1–6 and the visible Phase 6 wave behavior; SPEC-0002; SPEC-0003; every accepted SPEC-0004 and SPEC-0005 Phase 1 runtime/fixture/test/proof byte; Home/Tutorials/New/Open/My Project/AI Assistant/Export/AI Project Finalizer; Drawing workspace and Drawing AI; Stick manual menus/tools/layout, Play/Pause, onion, Creator presentation, line-head appearance, and manual post-Apply ownership; provider/API/search/Supabase/model configuration; Terra/Luna/Sol routing; packages/dependencies/configuration/environment/database/public assets; dashboard/auth/billing/deployment; rejected `/8de8/`; removed former Phase 2 copy; preserved `/7dfc/` runtime/proof bytes; `http://127.0.0.1:57795/`; other worktrees; and recovery material remain unchanged.
