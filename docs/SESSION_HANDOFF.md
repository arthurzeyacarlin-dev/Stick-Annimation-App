...
## Current stopping point — SPEC-0012 Phase 1 accepted; GIT-090 publication/integration authorized and in progress

D-0120 and [`SPEC-0012 — Diamond Animator Guidance Assistant`](specs/0012-diamond-animator-guidance-assistant.md) record Arthur's accepted six-phase outcome. GIT-089 `f2bda33842a3f5a4b560a7d3aab170a6ca4c2fd0` published and synchronized that plan. Arthur then separately authorized Phase 1, reviewed the corrected app, called it acceptable, and explicitly authorized control-plane propagation, commit, main integration and push. D-0121/D-0122 record the authorization/PASS and Control Plane Architect acceptance of the exact stopped eleven-path result. The accepted workspace AI Animator remains separate and unchanged: `DrawingAiPanel` → `/api/ai-animator` → `AiAnimatorJobService` → fixed `gpt-5.6-terra`, with its project-scoped ledger, typewriter/Thinking presentation and Phase 1 zero-mutation boundary.
...
The complete planning target is Tuesday, September 29, 2026 with an honest 85–90% local/private-beta goal only if reviews have no major rejection. Deterministic doubles remain default. Phase 1 adds the dedicated responsive shell only: real sessions, guidance answers, search, dictation and project/animation mutation remain absent.

Exact accepted technical dirty allowlist is eleven paths: `app/assistant/page.tsx`; `app/page.tsx`; seven files under `scripts/spec0012-assistant/`; and two files under `src/components/assistant/`. Immutable manifest SHA-256 is `3d25091e617b24544439419a99426b373cc560df64e01d4c735518d1f11104dd`; source digest is `3f3948195cec0f10c68889e1990782e0872cec2cb52c644ff532ea9058608e04`. The Control Plane Architect may add only the reviewed canonical record/tree allowlist. The index remains empty before publication staging.

Exact next safe step in this authorized closeout: stage only the exact accepted eleven technical paths plus reviewed canonical records/tree; commit GIT-090 on the phase branch; fast-forward an unchanged clean canonical `main`; push; verify clean local/origin/live equality; preserve proof; stop only the exact review server; and complete D-0054 cleanup. Phase 2 must not start here. After terminal records synchronize, Phase 2 becomes ready only for Arthur's separate authorization. Phases 2–6, all Assistant live/paid calls, transcription-provider selection, public beta and deployment remain unauthorized.
