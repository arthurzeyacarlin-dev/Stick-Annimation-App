...
Spec registry: completed SPEC-0001–0003 remain protected; completed SPEC-0004/0005 phases remain published history and unfinished phases remain superseded/inactive under D-0055. SPEC-0006 is **Approved and active**. Phase 1 is Verified/published/integrated at GIT-051 `804ff39dc73c88d4799570cce2ef18987745a0be`; Phase 2 is Verified/published/integrated at GIT-052 `d2096109900cc50a0a4dae2f603bd74b7b4a3427`. D-0060 authorizes Phase 3 stage/compositing only from that exact runtime base; it is Authorized/Not started. **Phases 4–7 remain Unauthorized/Not started.**
...
Current result: before this records-only D-0060 successor, canonical `HEAD`, local `main`, local `origin/main`, GitHub's live `main`, and `git ls-remote` were freshly verified equal at GIT-052 `d2096109900cc50a0a4dae2f603bd74b7b4a3427`, clean and `0/0`. The accepted Phase 2 manifest identity remains recorded as `output/spec-0006/phase-2/proof-manifest.json`, 16,649 bytes, SHA-256 `ebfeb699c0d9bcedd5b6b7c90d5cb4b71a3fe3ea89326a2820b86882e2c27cb7`; the ignored file is not present in this canonical worktree, so it was not rerun or altered here. The obsolete Phase 2 executor worktree is unregistered, PID `72206` is absent, and port `56362` is closed. This task changes only canonical control-plane/tree bytes and then publishes them normally as GIT-053; read the exact records-only commit SHA from Git.
Current roadmap phase: Phase 0 — Preserve and Stabilize

## SPEC-0006 Phase 2 published; Phase 3 authorized

[`SPEC-0006 — Unified Animation Workspace`](specs/0006-unified-animation-workspace.md) retains the exact seven-phase typed-data/lossless-migration/one-root design. GIT-051 published Phase 1. GIT-052 published the corrected exact 15-path Phase 2 result in `d2096109900cc50a0a4dae2f603bd74b7b4a3427`, parent `804ff39dc73c88d4799570cce2ef18987745a0be`, message `Implement SPEC-0006 Phase 2 unified shell`, exactly 29 paths. D-0060 records **Phase 3: Approved; Authorized; Not started**. **Phases 4–7 remain Unauthorized; Not started.**

New Project now enters one `AnimationWorkspace` directly with **Untitled Project** and no Drawing/Stick chooser. Open Project keeps the inherited dark Projects screen and card presentation, removes the type selector, omits type badges and new chrome, combines Drawing V1/V2 and Stick V1/V2 entries, disables invalid entries, and opens one inherited compatibility editor. Drawing and Stick compositor/timeline/tools/history/write ownership remain separate; no mixed editing or canonical Save is claimed.

Exact Phase 2 evidence: 15 technical paths inside the 16-path ceiling; manifest SHA/size above; 15 source bindings, 19 receipts, 38 artifacts, and 18 negative mutation cases; frozen Phase 1 contract/migration hashes; 40 browser operations and 13 screenshots at desktop/compact; all four supported source kinds; invalid, missing-asset, stale, IndexedDB, and localStorage failures; zero source writes; zero real API/external requests; zero console/page errors; production compile/generate; TypeScript; focused/full lint non-regression; inherited validators; exact scope/diff/index checks. This reconciliation changes no accepted technical or proof byte.

Next step: create one new dedicated Phase 3 Spec Executor task only after this records-only GIT-053 publication is visible. The executor starts in Plan mode; its technical/runtime diff base is exactly GIT-052 `d2096109900cc50a0a4dae2f603bd74b7b4a3427`, while its plan/proof must bind the exact later D-0060 authorization commit and current spec bytes. Before editing, freeze the exact technical path list within the 16-path ceiling and satisfy the existing Phase 3 mixed-fixture/reference-render/coordinate-transform/parity entry gate. Implement only stage/renderer/mixed compositing, produce and validate the technical manifest, return the Implementation Review Packet, and stop. Do not implement Phase 4 or publish Phase 3.
...
Current 2026-09-10 stopping point: GIT-051 is published/integrated at `804ff39dc73c88d4799570cce2ef18987745a0be`; GIT-052 is published/integrated at `d2096109900cc50a0a4dae2f603bd74b7b4a3427`. Phase 2's accepted manifest/source evidence remains unchanged. D-0054 cleanup is complete for the obsolete Phase 2 review copy: worktree unregistered, PID absent, port closed. D-0060/GIT-053 records Arthur's Phase 3-only authorization. Phase 3 is Not started; Phases 4–7 remain Unauthorized/Not started.
...
Current SPEC-0006 Phase 2 publication verification: exact parent/base `804ff39dc73c88d4799570cce2ef18987745a0be`; exact publication `d2096109900cc50a0a4dae2f603bd74b7b4a3427`; canonical/local-origin/live-remote equality and `0/0`; clean worktree and empty index; exact 15 technical paths plus 14 reviewed control-plane/tree paths; unchanged manifest SHA-256/size and source bindings; 19 receipts; 38 artifacts; 18 negative mutation cases; frozen Phase 1 contract/migration hashes; 40 browser operations/13 screenshots across desktop/compact and all four source kinds; zero writes/real API/external requests/errors; production compile/generate; TypeScript, lint non-regression, inherited validators, scope and diff checks. Proven visible scope is direct New, one inherited Projects collection, invalid-entry disablement, and one compatibility editor through `AnimationWorkspace`. Phase 3 authorization is proven by Arthur's instruction and D-0060; mixed compositing itself, one timeline/tools/history/persistence owner, and canonical Save remain unproven and unimplemented.
...
SPEC-0001 Phases 1–6, SPEC-0002, SPEC-0003, every accepted SPEC-0004/0005 runtime/proof byte, every GIT-052 runtime/fixture/test/proof byte, Home/Tutorials/My Project/AI Assistant/Export/AI Project Finalizer, Drawing and Stick editor internals, their separate compositor/timeline/tools/history/storage owners, existing Drawing Save/Save As and Stick Save semantics, Drawing AI, Stick AI/manual behavior, provider/API/search/Supabase/model configuration, Terra/Luna/Sol routing, packages/dependencies/configuration/environment/database/public assets, dashboard/auth/billing/deployment, legacy source-store bytes, rejected-result history, other worktrees, and recovery material remain unchanged by this records-only reconciliation. No mixed editor, canonical repository, Phase 3 implementation, provider/external/paid operation, or deployment is performed.
