...
SPEC-0005 status: **Phases 1 and numeric-safety Phase 2 v1 remain Verified/published/integrated history. GIT-044 published the former Phase 3 authorization in exact docs-only commit `269ac82335ee4576cb471bd9dffc8f7ce9bdec0f`, but Arthur rejected the resulting Phase 3 motion; none of that executor's runtime, fixture, test, proof, or review-UI bytes is accepted or reusable. D-0050 supersedes the future Preview gate with one formal Phase 2 v2 body-local safety correction and rewrites Phase 3's semantic natural-pose contract. Phase 2 v2 is Approved/Authorized/Not started pending docs-only GIT-045; corrected Phase 3 and Phases 4–8 remain Unauthorized/Not started.** No executor is active.
Current result: Phase 2 v1 remains current runtime truth and retains its 17,520-byte manifest at SHA-256 `e8a3fbbbb1a1cdf63e5d3648422def4736a4a1bf212ecc27335d2aa842db7874`. It is no longer sufficient for future SPEC-0005 Preview because screen-coordinate bend signs, broad angle limits, and numeric candidate cost did not prove body-local anatomical direction or natural pose quality. The exact next action is GIT-045 publication of D-0050 and the corrected spec/control-plane record, followed by one fresh Phase 2 v2 Spec Executor from that published canonical SHA.
...
The published Phase 2 v1 result remains immutable history, but D-0050 requires a formal Phase 2 v2 correction before any later pose output may reach Preview. The correction defines performer-facing projection, permitted limb planes, joint-guide half-planes, safe flexion bands, tightly controlled projected-sign transitions, backward-bend/sudden-flip rejection, forbidden-extra-movement checks, and independent validation of the final rounded sequence. It fails closed before Preview. The fresh executor has exactly the eight tracked paths and ignored output root listed in SPEC-0005; it may not use the rejected Phase 3 worktree or edit any Phase 3 implementation path.

Corrected Phase 3 receives semantic no-coordinate movement goals only. It must enumerate bounded whole-body candidates, qualify every candidate through Phase 2 v2 hard safety first, and deterministically rank safe complete sequences by prompt match, comfort, line of action, balance, silhouette, continuity, then minimum unrequested movement. Sparse landmarks and guides shape motion without becoming holds or easing boundaries. Its independent human-authored `SPEC0005-HUMANOID-NATURALNESS-v1` suite covers wave, walk, run, run-then-wave, punch, jump, mirrors, near-limit poses, and extra-action mutations; Phase 3 may only claim its bounded grounded pose probes, while locomotion and complete action quality remain owned by Phases 6 and 7.
...
GIT-044 is complete at exact docs-only commit `269ac82335ee4576cb471bd9dffc8f7ce9bdec0f`. The Phase 3 executor result authorized by that commit is rejected and must not be reused. The exact next action is separate GIT-045 publication of D-0050 and these reviewed correction records. Only after GIT-045 may one fresh Spec Executor implement Phase 2 v2 from that exact clean canonical SHA and exact eight-path allowlist. Corrected Phase 3 remains unauthorized until Phase 2 v2 is accepted, control-plane closed, separately published/integrated, and Arthur explicitly authorizes Phase 3. Phases 4–8 remain Unauthorized/Not started. Do not touch rejected worktrees or perform provider/API/paid/Drawing/persistence/deployment work.
...
Proven for SPEC-0005 Phase 1: the exact independent historical-reference catalog, all required bad-motion rejections, complete two-viewport playback capture, timing/landmark/stability/limb checks, TypeScript/scoped-lint/diff/scope, permanent regression, zero-egress evidence, exact source/artifact bindings, Arthur's readability-floor acceptance, and review cleanup. Proven for Phase 2 v1 only: the published finite numeric safety contract and its accepted manifest. Proven by this Spec Architect task: exact local canonical basis `269ac82335ee4576cb471bd9dffc8f7ce9bdec0f`; the rejected Phase 3 evidence supplied by Arthur; the body-local Phase 2 v2 contract; corrected Phase 3 semantic schema, candidate and naturalness ordering; exact executor gates/allowlists/proof; and unchanged eight-phase ownership. Explicitly not proven or changed: Phase 2 v2 runtime or proof, an accepted body-local correction, any corrected Phase 3 implementation or visible motion, airborne mechanics, natural paths, gait, complete semantic actions, planner routing, provider behavior, or professional-release quality.
...
SPEC-0001 Phases 1–6 and the visible Phase 6 wave behavior; SPEC-0002; SPEC-0003; every accepted SPEC-0004 and SPEC-0005 Phase 1/Phase 2 v1 runtime, fixture, test, and proof byte; Home/Tutorials/New/Open/My Project/AI Assistant/Export/AI Project Finalizer; Drawing workspace and Drawing AI; Stick manual menus/tools/layout, Play/Pause, onion, Creator presentation, line-head appearance, manual animation behavior, and manual post-Apply ownership; provider/API/search/Supabase/model configuration; Terra/Luna/Sol routing; packages/dependencies/configuration/environment/database/public assets; dashboard/auth/billing/deployment; rejected executor worktrees/results; other worktrees; and recovery material remain unchanged.
