...
## Current stopping point — SPEC-0012 Phase 4 accepted and propagated; publication pending

D-0120/GIT-089 establish [`SPEC-0012 — Diamond Animator Guidance Assistant`](specs/0012-diamond-animator-guidance-assistant.md); D-0121–D-0123/GIT-090 fully close Phase 1. D-0124 accepts combined Phase 2/absorbed Phase 3, and GIT-091 `fa2ef6c526d04de9c77b356b53b4768890c46e1f` publishes/integrates the exact 34 technical plus 15 reviewed record/tree paths, preserves its proof and completes cleanup. D-0125 records Arthur/PM acceptance and sequential exclusive Control Plane Architect propagation of Phase 4 hosted search plus the corrupt-row recovery correction. The accepted Phase 4 implementation is not staged, committed, published, integrated, proof-preserved in a publication backup or cleaned up yet.

Current implemented scope is:

1. Phase 1 and combined Phase 2/absorbed Phase 3 are fully closed.
2. Accepted Phase 4 adds local-first search eligibility, exact hosted `web_search` for genuine current/external facts, real Searching/Finalizing events, validated accessible citations, bounded search receipts and corrupt-row-safe local persistence without project mutation.
3. Phase 5 microphone dictation is entry-ready only after Phase 4 publication/cleanup but remains **Unauthorized; Not started**. Phase 6 beta hardening, public exposure and deployment remain unauthorized.

Accepted Phase 4 owns only the dedicated Assistant route/job/provider/search policy/session path. It receives no project identity/bytes/workspace context, contains no mutation commands, and imports no animation repository/storage/recovery/Export/project-management/manual-registry or AI Animator job/storage owner. Internal/stable guidance remains zero-search. Eligible search is fixed `gpt-5.6-terra`, maximum two tools/eight processed/six displayed sources/512 query characters/45 seconds, no retry, `$0.01` per tool call inside the `$0.15` request ceiling, `store:false`, and no arbitrary fetch/scrape/download/media inspection/image search/social auth/fallback model.

Accepted base/HEAD is `fa2ef6c526d04de9c77b356b53b4768890c46e1f` on branch `codex/spec0012-phase-4-fresh-search`; index is empty. Final manifest `output/spec-0012/phase-4/proof-manifest.json` SHA-256 is `08ff725e2d9bc658e198462e171623b1290d7b7de0b82bb3da93d085e4cfd095`; validation receipt SHA-256 `3107a15f9193719fe3ef1b8461f1003187ae75157f2786956baca4096a528060`; source digest `073bc363b1ba26e6be035df69136f7288687df028ed92ec9701ab649c17517fd`; 27 sources and 104 evidence bindings. Prior seal manifest/validation/source identities are `f29b22db…18a`, `ec8f0f37…96b`, and `7dd51abe…1d1`. Strict pre-propagation validation passed. CPA independently revalidated all immutable technical source/evidence bindings after records were added. Accepted technical bytes remain unchanged.

Proof passed 47 deterministic oracle, 23 main browser, 27 corrupt-row, 20 fault, 90 inherited Assistant browser, 60 presentation, 21 protected-browser, ten restart, 33 repeated-browser and eleven Animator-smoke assertions plus thirteen protected oracles, TypeScript, focused lint/build and exact diff. Final authorized live search used Terra/Medium, one request/no retry, one tool call, eight processed/three displayed sources, `$0.050754`, citations/reload/project-sentinel PASS. Provider cost for two earlier authorized failed searches is unknown. Correction calls were zero. Full build retains the inherited byte-identical dev AI-cost typing failure; physical devices/non-Chromium remain unproven.

Exact publication-ready scope is the 27 accepted technical paths from the manifest plus these fifteen reviewed record/tree paths: `docs/00_MASTER_PROJECT.md`; `docs/AI_SYSTEM.md`; `docs/CURRENT_STATE.md`; `docs/DECISIONS.md`; `docs/PROJECT_MANAGER_CONTEXT.md`; `docs/README.md`; `docs/ROADMAP.md`; `docs/SESSION_HANDOFF.md`; `docs/TODO.md`; `docs/architecture.md`; `docs/changelog.md`; `docs/specs/0012-diamond-animator-guidance-assistant.md`; `docs/specs/README.md`; `docs/testing_workflow.md`; and `project/project_structure.txt`. Combined publication-ready allowlist: exactly 42 paths.

The review app remains at `http://127.0.0.1:58040/assistant`, exact PID 31449, in this exact worktree for Arthur. Exact next safe step: a later explicit GIT-092 publication instruction may stage only the 42 reviewed paths, commit/integrate/push after unchanged-base checks, preserve all proof, stop PID 31449, verify port 58040 closed and complete D-0054 cleanup. Until then do not stage, commit, push, stop the review app, clean this worktree, begin Phase 5, deploy or make another live/provider/search call.
