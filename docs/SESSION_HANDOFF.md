...
D-0094 preserves [`SPEC-0009 — Animation Export`](specs/0009-animation-export.md) as a decision-complete but **Proposed** three-phase plan: (1) Choose and Watch from Home Export and workspace File → Export; (2) Create and Save Video from one immutable saved revision to a local Finder-selected MP4; and (3) Social Destinations and Final Testing through a versioned local preset catalog with contain/no-crop framing. It performs no direct social login/upload/post and no AI/provider/network work. No phase is Approved or authorized.
