...
All seven SPEC-0006 phases are closed through GIT-061. Drawing-only SPEC-0007 is fully Verified, published, integrated, recorded and cleaned up through D-0083/D-0084/GIT-066 `c193b8ba89fa55ead02d84ea10bb81f71b960f8a`. D-0085 creates SPEC-0008, D-0086/GIT-067 publishes its plan, D-0087/GIT-068 approves it with Phase 1 only, and D-0088 rejects/cleans the first result and authorizes one correction. D-0089 records Arthur's acceptance and technical verification of the corrected Phase 1 result.

## Current stopping point — publish accepted SPEC-0008 Phase 1 as GIT-070, then preserve proof and clean the review copy
...
The first executor's app failed visible review and remains Rejected/unpublished/non-reusable with preserved evidence and completed cleanup. The fresh correction is different accepted work from exact synchronized base/HEAD `3da58e096dd748c7c3bd23fbb9271d53e33597ca` in `/Users/arthurcarlin/.codex/worktrees/22ac/stick-animation-app`, with an empty index and exactly 19 technical dirty paths.
...
Exact next step: under Arthur's explicit publication instruction, stage only the 19 accepted technical paths plus reviewed control-plane/tree paths, commit once on a `codex/` GIT-070 publication branch, fast-forward unchanged clean canonical `main`, push normally, and verify canonical/local-origin/live GitHub equality at clean `0/0`. Then preserve the complete ignored proof, stop the exact port-57120 review server, verify closure, remove only `/22ac/` and the merged publication branch, and record that terminal closeout. No pull, merge commit, rebase, amend, force-push, history rewrite or scope expansion. Phase 2 remains Unauthorized/Not started until this closure is recorded and Arthur separately authorizes it.
