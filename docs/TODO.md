...
- [ ] **GIT-006 — Publish and integrate the D-0011 Phase 1.5 approval record.** After Arthur and Project Manager review of the Control Plane Architect packet, a separate explicit instruction must authorize staging exactly the reviewed seven control-plane paths, committing, pushing canonical `main`, and verifying clean `0/0` synchronization. Phase 1.5 implementation and Phase 2 are outside this publication task.
...
- [ ] **QLT-004 — Implement SPEC-0001 Phase 1.5 permanent automatic browser tester.** Status: **Approved; Authorized; Not started.** The bounded tester must protect Home → New → Stick, Stick → Creator → Back, and deterministic/mocked Drawing Generate Frames; run with browser and tested-server external egress denied; fulfill exact font requests only inside the guarded tester from hash-bound fixtures; restore temporary setup byte-for-byte without any permanent application font, style, source, or behavior change; leave no tester route/page/button/API/asset/production import; capture repeatable evidence and cleanup; and expose only the minimum additive Phase 2 checkpoint connection. It may begin only after this approval record is separately published/integrated and one new Plan-mode Spec Executor task/worktree starts under D-0010. Proof requires the Phase 1.5 stop gate, Spec Executor Implementation Review Packet, Arthur/Project Manager acceptance, later Control Plane Architect propagation, separate Git-publication authorization, and canonical-main integration before Phase 2 may be authorized. OpenAI, search, Supabase, paid services, and deployment remain forbidden.
