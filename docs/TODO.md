...
- [x] **SPEC-002 — Review and decide lossless local Drawing Save/Reopen.** D-0013 records Arthur's exact approval of [`SPEC-0002`](specs/0002-lossless-local-drawing-save-and-reopen.md), acceptance of OD2-01 through OD2-08 exactly as written, and authorization of Phase 1 only as Not started. SPEC-0002 Phase 2 and SPEC-0001 Phase 2 remain Unauthorized/Not started.
- [ ] **GIT-009 — Publish and integrate the D-0013 SPEC-0002 approval record.** After Project Manager review, a separate explicit instruction must authorize staging only the reviewed proposal/control-plane paths, committing, integrating into canonical `main`, pushing, and verifying clean `0/0` synchronization. No implementation is part of this publication.
- [ ] **DATA-002 — Implement SPEC-0002 Phase 1 safe save format and storage engine.** Start only after GIT-009 completes, in one new Plan-mode Spec Executor task/worktree from the then-current canonical-main SHA and after a refreshed conflict audit. Scope is exactly the new files in SPEC-0002 §11.2; Phase 2 UI wiring remains forbidden.
