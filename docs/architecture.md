...
Last traced: 2026-09-25 through D-0140's accepted SPEC-0014 Phase 2 correction from `acd4031cade0e4e960223190a8c1645d007d61af`; this implementation is still in its separate review worktree, not canonical `main`.
...
## SPEC-0014 notification architecture — Phase 2 accepted in review copy, publication pending
...
In D-0140's accepted but unpublished Phase 2 review copy, `NotificationCenterProvider` starts one app-lifetime `assistantCompletionObserver` and `terraCompletionObserver`. `useAssistantSessions` and `DrawingAiPanel` subscribe/present rather than owning a competing terminal poller; user-started jobs alone keep their existing server request door. The observer commits and rereads the exact Assistant IndexedDB turn or project-scoped Terra ledger terminal before publishing one deterministic notification. The notification store remains a separate observer, never an Assistant/project source owner. `NotificationTrigger` renders only unread rows, with live Assistant session title or captured Terra project title as presentation context. The Assistant destination handler navigates same-route chat changes through `hashchange` as well as cross-route Home clicks; `DiamondAssistantScreen` and `DrawingAiPanel` confirm exact target arrival before marking the shared row read. Unavailable/blocked destinations remain unread. Watched exact-origin outcomes are stored already read for dedupe but have no visible bell row/ring. Neither stored event schema nor provider/search/dictation/request limits changes.

`ExportAnimationPlayer` still owns and aborts its export controller on unmount; Phase 3 must move same-runtime export lifecycle above Export UI only after separate authorization. Phase 4 still owns offline/reload/concurrency/dedupe/accessibility closeout. Browser/app termination must never be misrepresented as continued export execution. This Phase 2 architecture is technically verified in its review worktree but is not a canonical-main or deployment claim until GIT-100 publishes it.
