...
Last traced: 2026-09-07 at exact local canonical `269ac82335ee4576cb471bd9dffc8f7ce9bdec0f`: published SPEC-0005 Phase 1 and Phase 2 v1 remain runtime truth; the GIT-044-authorized Phase 3 executor result is rejected; D-0050 authorizes a fresh Phase 2 v2 body-local safety correction pending GIT-045 and leaves corrected Phase 3 unauthorized
...
| Stick animation plan/executor | `src/lib/ai/stickFigureAiContract.ts`, `stickFigureCommandExecutor.ts`, `stickFigureMotionEngine.ts`, `stickFigureAiWorkspaceAdapter.ts` | Published SPEC-0004 Phases 1/2/2.5 remain normal runtime truth. SPEC-0005 Phase 1 is a published proof gate. GIT-043 publishes Phase 2 v1's finite numeric body-safety seam; no normal route invokes it yet. D-0050 requires Phase 2 v2 to add performer-facing/body-local projection, limb planes, joint guides, safe flexion bands, controlled projected-sign transitions, final rounded revalidation, and fail-closed Preview integration. The rejected Phase 3 result is not architecture. Corrected semantic pose creation, mechanics, paths, gait, actions, and planner routing remain unimplemented. |
...
D-0050 preserves exactly eight phases while reopening the future Preview dependency as a formal Phase 2 v2 correction. The correction derives a body-local frame from torso and declared performer facing, qualifies the permitted limb plane and joint-guide half-plane, enforces named flexion bands, permits projected-sign change only through an explicit bounded transition, rejects backward bends/sudden flips and forbidden extra movement, and rechecks final rounded output before Preview. Corrected Phase 3 consumes a semantic no-coordinate movement goal, enumerates bounded whole-body candidates, applies Phase 2 v2 hard safety first, then ranks complete safe sequences lexicographically by prompt match, comfort, line of action, balance, silhouette, continuity, and minimum unrequested movement. Sparse landmarks/guides do not create holds. Phases 4–8 retain mechanics, paths/timing, gait, recipes, and the sole provider-free planner-door ownership respectively.
...
- broader Stick scene/language/model behavior beyond published SPEC-0004 Phases 1–2.5; rejected motion attempts are not runtime truth, SPEC-0005 Phase 2 v1 is only the published numeric-safety foundation, Phase 2 v2 is authorized pending GIT-045, and corrected Phase 3 whole-body pose behavior is unauthorized/unimplemented
