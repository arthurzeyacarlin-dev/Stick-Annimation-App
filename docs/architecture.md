...
Last traced: 2026-09-26 through D-0142's accepted SPEC-0014 Phase 4 notification-only result from `57f3a8560dc61f79db87c13dbff7285684b74793`; GIT-102 publication pending.
...
## SPEC-0014 notification architecture — Phase 4 accepted, GIT-102 pending

D-0142 adds `offlineIncidentObserver` under the already root-mounted `NotificationCenterProvider`. Browser `navigator.onLine` plus actual online/offline events report state; an active `browser-connectivity-incident/v1` metadata row and a sealed offline notification are committed atomically in the existing IndexedDB notification store under its lock/lease/CAS writer. Repeated events and tabs converge on one incident. Both existing Home and Assistant triggers display the shared offline row and remain red while the browser reports offline, even after acknowledgement. A genuine return-online event clears the active incident, retires its stale offline unread row and seals one shared `system.internet.restored` row. The restored item stays unread through reload and subsequent outages until explicit acknowledgement from either bell; its past-tense copy remains truthful when another outage occurs. Initial online observation clears stale offline metadata without inventing a success notice. Online provider failure does not declare the browser offline. This notification observer neither gates AI requests nor probes provider health; no automatic retry or new bell exists. The 14 accepted technical paths are technically Verified in a separate review worktree and not yet integrated into canonical main.

The accepted Phase 1 correction mounts `NotificationCenterProvider` from `app/layout.tsx`, with strict notification contracts, independently bound terminal receipt validation, a local IndexedDB store, exact origin/target registration seams and a typed navigation adapter. The provider renders no global bell. Home's `AppChrome` mounts one all-event `NotificationTrigger`; `/assistant` mounts one Assistant-completion/failure-only trigger. Credits, workspace, Export, Tutorials, Open Project and My Projects mount none. Both views share durable per-record read state, while Assistant's rows/count/blue dot/green ring/announcements/Mark-all are filtered to its event kinds. At the historical Phase 1 checkpoint no production source/destination registered and the ordinary app received no actual completion alert; the accepted Phase 2 review copy below supersedes that producer gap.

In D-0140's accepted, GIT-100-published Phase 2, `NotificationCenterProvider` starts one app-lifetime `assistantCompletionObserver` and `terraCompletionObserver`. `useAssistantSessions` and `DrawingAiPanel` subscribe/present rather than owning a competing terminal poller; user-started jobs alone keep their existing server request door. The observer commits and rereads the exact Assistant IndexedDB turn or project-scoped Terra ledger terminal before publishing one deterministic notification. The notification store remains a separate observer, never an Assistant/project source owner. `NotificationTrigger` renders only unread rows, with live Assistant session title or captured Terra project title as presentation context. The Assistant destination handler navigates same-route chat changes through `hashchange` as well as cross-route Home clicks; `DiamondAssistantScreen` and `DrawingAiPanel` confirm exact target arrival before marking the shared row read. Unavailable/blocked destinations remain unread. Watched exact-origin outcomes are stored already read for dedupe but have no visible bell row/ring. Neither stored event schema nor provider/search/dictation/request limits changes.

`ExportAnimationPlayer` still owns and aborts its export controller on unmount. D-0141 rejects the separate Phase 3 Export-notification/background-lifecycle proposal; this current screen-owned behavior remains unchanged and has no notification producer. D-0142's Phase 4 result is accepted but pending GIT-102 publication. Browser/app termination must never be misrepresented as continued export execution. Phase 2 is published in canonical main under GIT-100, but neither that fact nor the accepted Phase 4 review copy is a deployment or public-readiness claim.
