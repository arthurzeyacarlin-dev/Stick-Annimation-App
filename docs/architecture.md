...
| Stick animation plan/executor | `src/lib/ai/stickFigureAiContract.ts`, `stickFigureCommandExecutor.ts`, `stickFigureMotionEngine.ts`, `stickFigureAiWorkspaceAdapter.ts` | Published Phase 6 wave contract plus published SPEC-0004 Phase 1 strict fixed-fixture plan. Published Phase 2 adds a separately selected independent-keyframe materializer. Published timing-only Phase 2.5 adds the strict plan-bound timing sidecar and six bounded profiles while preserving Phase 1/2 defaults. The unpublished SPEC-0004 Phase 2.6 and first former SPEC-0005 Phase 2 results were rejected and are not runtime truth. SPEC-0005 Phase 1 adds only a published independent full-playback proof gate. D-0046 authorizes a fresh planner-independent body-safety seam across `stickFigureBodySafety.ts`, the motion engine, and executor, but no implementation exists yet; the exact movement-goal → candidate enumeration → important-pose safety → mechanics/insertion → path/timing bake → post-bake safety → semantic/continuity → Preview runtime still does not exist. |
...
D-0046 corrects the staged local architecture to exactly eight phases and one non-bypassable pipeline: movement goal → enumerate whole-body candidates → validate important-pose safety → apply mechanics and insert required poses → bake paths/timing → validate again after rounding/contact → run final semantic/continuity checks → Preview complete ordinary independent keyframes. Pretend AI and a future Terra adapter must use the same bounded intent and local engine. The planner cannot return raw coordinates/code, select another animation format, or bypass either safety pass. Restructured Phase 2 authorizes only the planner-independent `humanoid-11-v1` body-safety contract and integration seam; it is technical-only and creates no visible motion or product/UI path. After its technical gates pass, exactly one ordinary unchanged-source app instance may run directly from the exact unpublished executor worktree on one loopback-only non-`3000` root URL so Arthur can smoke-check existing behavior. That normal app does not invoke or prove the new safety seam. Phase 3 is the first new visible-output/Arthur-review phase. No decision connects a route, language interpretation, provider/API call, app UI, live tween/controller, or stored AI authority.
