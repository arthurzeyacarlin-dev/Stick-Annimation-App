...
`ExportAnimationPlayer` still owns and aborts its export controller on unmount. D-0141 rejects the separate Phase 3 Export-notification/background-lifecycle proposal; this current screen-owned behavior remains unchanged and has no notification producer. D-0142's Phase 4 result is published under GIT-102. Browser/app termination must never be misrepresented as continued export execution. Phases 2 and 4 are published in canonical main, but neither is a deployment or public-readiness claim.
