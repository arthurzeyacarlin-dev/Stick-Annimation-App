...
D-0018 fixes real Stick onion skin as part of Phase 3 without changing Drawing onion behavior. D-0019 authorized Phase 3. D-0020 keeps that authorization and D-0018 unchanged, adds the required versioned tester registration ceiling, supersedes the rejected Creator-lock requirements, and makes the published §10.5A shell/manual timeline the exact Phase 3 baseline. That correction is published/integrated at `54234b7c7b95201e274975a804859fa9c36806a1`. Phase 3 is **Authorized; In progress; proof-blocked** only: the preserved implementation tree has no accepted technical manifest, and this amendment authorizes one proof-only correction without changing the product outcome. Phases 4–7 remain **Unauthorized; Not started**.
...
SPEC-0001 remains Approved and active. Phase 1 and the historical Phase 1.5 are Verified, published, and integrated into canonical `main`; Phase 1.5's implementation commit is `8df64552e29e4170df8000097fe857b7a31dff69`, and its six-document control-plane publication record is `687cbeaf6acbf9625e0d940e78bc600251eb0604`. D-0015 authorized Phase 2 and its later activation record established exact implementation base `68338d54542bbfd3fb1f0fab06548f0424871f80`. D-0016 accepted the Phase 1.5 compatibility correction in §10.4A, which was subsequently published/integrated at exact prerequisite foundation `e07268cc80751baba99ac6708a1e8a93d4fc4756`. The original Phase 2 bytes remain historically published and integrated at `adbda9dd4f42a103c3c5af41ccc19b110b6825c0`; Arthur rejected their visible product result, and the D-0017 §10.5A correction is now Verified, published, and integrated at `edfb3dea023119b91336e6e5da645d4982a9f068`. Drawing Save remains outside the historical Phase 2 automated coverage and is recorded separately as Arthur's human verification. D-0018 remains the exact accepted Phase 3 onion contract, D-0019 authorizes Phase 3, and D-0020's exact tester/Creator correction is published/integrated at `54234b7c7b95201e274975a804859fa9c36806a1`. Phase 3 is Authorized/In progress/proof-blocked with an unaccepted preserved result and no technical manifest. Phases 4–7, the Phase 7 Policy Gate, and paid/live requests remain unauthorized and not started.
...
| 3 — Atomic history and minimal local persistence | **Authorized; In progress; proof-blocked** | Every manual construction action and joint drag has exact Undo/Redo; explicit local save/reopen preserves user-authored content. One AI-sized three-pose batch remains representable as one undo entry for Phase 4 without exposing an AI path. The existing Onion control gains the exact D-0018 paused active-layer neighbor renderer. | §10.5A Verified/published/integrated at `edfb3dea023119b91336e6e5da645d4982a9f068`; D-0018 unchanged; D-0019 owner authorization; D-0020 boundary/base `54234b7c7b95201e274975a804859fa9c36806a1`; stopped 29-path result unaccepted with no manifest | After this amendment is published, one proof-only Plan-mode executor may add the one validator path and three additive bindings, rerun the full exact proof, return its Implementation Review Packet, and stop. No Phase 4. |
...
**Status:** **Authorized; In progress; proof-blocked.** D-0019 remains the Phase 3 authorization, and D-0020's corrected implementation/proof boundary is published/integrated at exact base `54234b7c7b95201e274975a804859fa9c36806a1`. A stopped Phase 3 Spec Executor implemented the authorized product/test boundary in a preserved dirty worktree and passed the standalone product/browser proof, but correctly refused acceptance because the exact recorder command reached the unchanged baseline `scripts/validateDrawingProjectAiMemory.ts` and failed before creating a technical manifest. No Phase 3 implementation is accepted, Verified, published, or integrated. Arthur's accepted outcome remains unchanged: Phase 3 adds only Stick Undo/Redo, local Save/Open, and D-0018 onion skin to the published §10.5A editor. This amendment authorizes one proof-only correction inside Phase 3; it is not a new phase or product change. Phases 4–7 remain **Unauthorized; Not started**.
...
| Phase 3 activation and Stick onion contract | Authorized; In progress; proof-blocked | D-0018 remains unchanged and fixes exact Drawing tints, nearest-distinct active-layer neighbors, hold skipping, blank boundaries, duplicate collapse, playback/OFF suppression, non-interactivity, no canonical mutation, origin-independent rendering, and Drawing byte preservation. D-0019 authorizes §10.6 only. A stopped executor implemented the bounded Phase 3 result, but it remains unaccepted and has no valid manifest. |
...
| Phase 3 Drawing-memory proof blocker | Reproduced at base and result; correction authorized, not implemented | The exact validator command fails identically with `.map is not a function`; validator blob `ab3d483…` is synchronous/localStorage-only while storage blob `badbc5f…` is async/IndexedDB-backed. Both are unchanged by Phase 3. Standalone 71-action product/browser evidence passed, but the recorder emitted no technical manifest. §10.6 authorizes one newly dirty validator path and three additive bindings; full fresh proof remains required. |
| Phase 3 proof-blocker Control Plane Architect closeout | Pass; ready for separate documentation publication only | After Spec Architect shutdown and exclusive takeover, exact base/HEAD `54234b7c7b95201e274975a804859fa9c36806a1`, empty index, six-document diff, clean canonical `main`, untouched 29-path executor tree, matching protected blobs, absent manifest, and unchanged recovery refs were independently reverified. Memory generation/check-only, diff/scope/status consistency, and final empty-index checks pass; no implementation or Git publication action occurred. |
...
| Approval/status consistency | Pass for current correction state | SPEC-0001 remains Approved and active. Phase 1, historical Phase 1.5, §10.4A, and the original Phase 2 bytes remain published/integrated; the original Phase 2 visible result is owner-rejected. The §10.5A correction is Verified/published/integrated at `edfb3dea023119b91336e6e5da645d4982a9f068`. D-0018 remains exact, D-0019 authorizes Phase 3, D-0020's boundary is published at `54234b7c7b95201e274975a804859fa9c36806a1`, Phase 3 is Authorized/In progress/proof-blocked, and Phases 4–7 remain Unauthorized/Not started. SPEC-0002 remains complete/protected; SPEC-0003 remains separate Proposed/inactive. |
...
Final spec status is **Approved** and active. **Phase 1 and historical Phase 1.5 remain Verified, published, and integrated.** The §10.4A compatibility correction remains Verified/published/integrated at exact prerequisite foundation `e07268cc80751baba99ac6708a1e8a93d4fc4756`. The original Phase 2 bytes remain historically published/integrated at `adbda9dd4f42a103c3c5af41ccc19b110b6825c0`, with their original proof hashes preserved, but Arthur rejected their visible product result. The D-0017 §10.5A correction is **Verified, published, and integrated** at `edfb3dea023119b91336e6e5da645d4982a9f068`. D-0018 remains the exact accepted Stick onion-skin contract. D-0019 authorizes Phase 3, and D-0020's exact tester-registration/Creator-preservation boundary is published/integrated at `54234b7c7b95201e274975a804859fa9c36806a1`. Phase 3 is **Authorized; In progress; proof-blocked** with no accepted technical manifest. Phases 4–7 remain **Unauthorized; Not started**. SPEC-0002 remains complete/protected, and SPEC-0003 remains Proposed/inactive in its separate preserved worktree.

Exact next step: under a later explicit publication instruction, publish/integrate only the exact six Control Plane Architect-reviewed documentation paths from this correction worktree and report the resulting clean synchronized canonical-main SHA. After that publication and explicit sequential worktree handoff, authorize one proof-only Phase 3 Spec Executor to take exclusive ownership of the preserved dirty worktree at base/HEAD `54234b7c7b95201e274975a804859fa9c36806a1`. It may change only `scripts/validateDrawingProjectAiMemory.ts` plus the three exact mechanical binding paths in §10.6, yielding exactly 30 dirty paths; it must rerun the exact recorder, independent manifest validator, complete 71-action browser proof, TypeScript, lint comparison, diff checks, and protected regressions, then return the Implementation Review Packet and stop. It may not update canonical control-plane files, stage, commit, merge, push, publish, deploy, alter Drawing runtime/persistence, or begin Phase 4.

The reviewed correction worktree has exactly these six dirty canonical record paths:
...
Arthur's published shell correction remains recorded as D-0017; D-0018 remains the exact onion contract; D-0019 records Phase 3 authorization; D-0020 records Arthur's accepted tester/Creator correction. `docs/DECISIONS.md` is unchanged because this amendment resolves bounded proof mechanics under the latest explicit owner authorization and does not accept a new owner decision. `project/project_structure.txt` remains unchanged because deterministic memory regeneration produces no tree-content change. `AGENTS.md`, `docs/PROJECT_MANAGER_CONTEXT.md`, all product/runtime/fixture/test/proof bytes, package/configuration bytes, SPEC-0002, the separate SPEC-0003 worktree, and recovery material are preserved exactly by the Spec Architect amendment and Control Plane Architect closeout. Every later task must establish exact current Git state rather than infer a future HEAD from this specification.
