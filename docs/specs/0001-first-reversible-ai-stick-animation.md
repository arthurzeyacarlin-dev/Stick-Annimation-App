...
Decision links: accepted SPEC-0001 decision D-0009, permanent role-separation decision D-0010, Phase 1.5 approval decision D-0011, and still-broader pending decisions P-0004, P-0005, P-0007, and P-0008 in `../DECISIONS.md`
...
Last verified branch/commit: Phase 1 corrections and renewed offline proof were completed from exact base `832d1f93630d7093514af3e81399077ebed696b4` and published in canonical-main commit `21a88feb65cf1cc51138c9ad4879b962ee468569`; D-0010 was published and integrated in canonical-main commit `2029fd07e14b6f48feb6d04e02dbd52ec683d55d` and leaves that Phase 1 result intact

Arthur approved this specification on 2026-08-11. OD-01 through OD-06 and OD-09 are accepted exactly as written; OD-07, OD-08, OD-10, and OD-11 are approved engineering rules/prerequisites; OD-12 through OD-14 remain deferred to the separate Phase 7 Policy Gate. That approval authorized Phase 1, which is Verified, published, and integrated into canonical `main` as of 2026-08-13. On 2026-08-14 Arthur approved the **Phase 1.5 — Permanent Automatic Browser Tester** amendment in D-0011 and authorized Phase 1.5 only as **Approved; Authorized; Not started**. Phases 2–7, the Phase 7 Policy Gate, and every live/paid/external request remain unauthorized and not started. No visible application behavior changed.
...
No development server or browser/app flow was run in this docs-only revision. Current UI claims below are therefore either **code observed on 2026-08-11** or explicitly inherited as **live verified on 2026-08-09** from [`CURRENT_STATE.md`](../CURRENT_STATE.md). Fresh visible behavior remains unproven and must be rerun through the permanent offline tester after Phase 1.5 is approved, implemented, Verified, and integrated.
...
| Browser proof infrastructure | No repository-owned browser runner/test framework or browser-test package script exists; `app/layout.tsx` imports `next/font/google`, and scrubbed app credentials do not deny server egress. | Approved Phase 1.5 will leave a permanent developer-only, non-routable browser tester in the repository, supply child-process server-egress denial and tester-only hash-bound font responses for guarded offline dev/build proof without changing application source or styling, protect the current Home/New/Stick/Creator and mocked Drawing Generate Frames flows, and provide the minimum additive Phase 2 checkpoint connection. | Temporary `app/__dev` routes are routable product surfaces, old screenshots are not repeatable proof, and the earlier spec placed this hard prerequisite in a second future specification. | Phase 1.5 must produce a schema-valid repeatable proof bundle, production-exclusion scan, byte-identical temporary-setup cleanup proof, unchanged-product-source proof, and zero external request ledger before Phase 2 can be authorized. |
...
SPEC-0001 remains Approved and active. Phase 1 is Verified, published, and integrated into canonical `main`. D-0011 approves and authorizes Phase 1.5 only as **Approved; Authorized; Not started**. It does not authorize Phase 2, Phases 3–7, the Phase 7 Policy Gate, a paid/live request, or implementation inside this control-plane task.
...
Before Phase 2, proposed **Phase 1.5 — Permanent Automatic Browser Tester** must be separately approved, technically Verified, propagated by a Control Plane Architect, explicitly published, and integrated into canonical `main`. It closes the relevant QLT-004 proof gap inside this spec rather than creating a second feature specification. Its permanent source remains in the development repository, but it is never a product route, page, panel, button, public asset, API, or published-application import. No SPEC-0001 phase may add a temporary `app/__dev` route. If Phase 1.5 is absent or unverified, Phase 2 stops before runtime changes and Phases 3–7 remain forbidden.
...
Later phases may add new **versioned** action schemas, fixture kinds, checkpoint fields, and proof plans under their own exact file boundaries. An additive registry entry must name its owning phase, strict schema and digest, remain unavailable to earlier plans, and preserve every accepted Phase 1.5 fixture/result byte. Existing action names, versions, meanings, accepted evidence, and tester-core behavior are immutable; a breaking change requires an explicit tester protocol version and review. The more detailed operation names below are later-phase requirements/allocations, not Phase 1.5 implementation scope and not a claim that their adapters already exist.
...
| 1.5 — Permanent Automatic Browser Tester | **Approved; Authorized; Not started** | One permanent developer command opens the real app offline, proves Home → New → Stick, fresh Stick → Creator → Back, and mocked Drawing Generate Frames, emits repeatable evidence, proves cleanup, and proves no tester UI/route/import/asset or visible source/style change ships in the production result. | Phase 1 Verified/published/integrated; D-0010 published/integrated at `2029fd07e14b6f48feb6d04e02dbd52ec683d55d`; D-0011 approval/authorization record separately reviewed, published, and integrated; new Plan-mode Phase 1.5 Spec Executor task/worktree | Stop until the tester runs entirely offline through tester-only font fulfillment, denies browser/server egress, leaves byte-identical pre-run application source/styling and a clean owned build/process/font-cache state, proves its production absence, and demonstrates the minimum non-routable Phase 2 frame/joint checkpoint connection. |
| 2 — Independent per-frame Stick state | **Unauthorized; Not started** | Home → New → Stick visibly supplies the built-in figure, and the user creates the complete three-pose/12-frame wave without AI using the bounded hold/blank/start-pose controls; all 11 joints, playback, holds, and gesture boundaries use independent canonical poses. | Phase 1.5 Verified, Control Plane Architect propagation accepted, explicitly published, and integrated into canonical `main`; separate Phase 2 owner authorization | Stop until the real no-AI manual construction flow matches the Phase 1 manual content digest and all-11-role isolation, rendering, playback, held-frame, and gesture proof passes. |
...
**Status:** **Verified, published, and integrated into canonical `main` on 2026-08-13 after corrected PM review.** D-0011 later authorized Phase 1.5 only as Approved/Authorized/Not started. Phase 1 did not implement Phase 1.5, and Phases 2–7 remain unauthorized and not started.
...
**Status:** **Approved; Authorized; Not started.** Arthur approved the amendment in D-0011. Phase 1.5 implementation may begin only after this approval-state propagation is separately reviewed, published, and integrated, in a new Plan-mode Spec Executor task/worktree under D-0010. This approval authorizes Phase 1.5 only; it does not authorize Phase 2, automatic dependency/browser/font downloading, external services, Git publication, or any product feature.

**Exact observable outcome:** From one repository command, a developer can start the real Diamond Animator app under a fail-closed offline guard, automatically exercise Home → New → Stick, fresh Stick → Creator → Back, and the protected Drawing Generate Frames browser/render/apply path with a deterministic intercepted response, and receive a strict pass/fail evidence bundle containing action/click results, visible assertions, screenshots, console failures, blocked network attempts, production-exclusion results, and cleanup results. The permanent tester is development source only: no tester page, route, API, button, panel, label, asset, or import appears in the published application, and Diamond Animator's normal source, styling, font selection, and visible behavior remain unchanged. A hash-verified non-HTTP anchor handshake proves Phase 2 can add versioned frame/joint checkpoints later without a temporary route.
...
→ separate isolated context exercises Drawing Generate Frames through one browser-fulfilled deterministic `/api/ai` response
...
**Dependencies:** Phase 1 remains Verified/published/integrated at `21a88feb65cf1cc51138c9ad4879b962ee468569`; D-0010 is published/integrated at `2029fd07e14b6f48feb6d04e02dbd52ec683d55d`; this amendment is approved by Arthur and the Project Manager; the future Spec Executor starts in Plan mode in a dedicated worktree at the then-current canonical-main SHA. The implementation may add one exact pinned `playwright-core` development dependency and lockfile entry only after its Plan-mode evidence confirms the selected version/browser compatibility. If required package bytes, compatible local browser, or legally usable exact locally sourced font-response bytes are unavailable—and Arthur has not separately authorized the applicable one-time acquisition—the executor stops before mutation; the tester never downloads a browser, package, or font as an automatic fallback.
...
No other file or system is authorized. In particular, `app/layout.tsx`, every stylesheet, every font/style asset, every file under `app/api/**`, every `app/**/page.tsx` or `route.ts`, `app/page.tsx`, every Drawing or Stick product component other than the exact inert anchor comment, every provider/storage/memory file, every environment file, every migration, and every database are outside scope.

**Explicit non-goals:** a general E2E platform; CI/cloud runners; tests for Phase 3–7 internals; a product test menu; a development page or URL; arbitrary browser evaluation; broad accessibility/performance/visual-regression coverage; changing Drawing behavior; changing Stick state; adding provider mocks to application code; live API/search/Supabase/analytics/deployment calls; or solving QLT-001/002/003/005.
...
3. Implement a loopback-only runner. It starts the direct local Next binary with scrubbed OpenAI/Supabase/analytics variables, `NEXT_TELEMETRY_DISABLED=1`, and inherited `networkDeny.cjs`; the guard denies non-loopback `fetch`, HTTP(S), socket/TLS, and DNS attempts in the tested server/child process and rejects unguarded child spawning. Browser request interception independently denies every non-loopback URL. Guard self-tests must attempt each supported escape path and prove denial before app assertions begin.
4. Use fresh temporary browser profiles and ports. Never attach to a user's normal browser profile or read/write real saved-project storage. Phase 1.5 plans use only visible role/name interactions and strict assertions; no arbitrary `evaluate`, CSS-selector scenario convention, or product-only debug route.
5. Exercise the exact initial flows: dismiss/handle the first-run welcome deterministically; assert Home; click New; open Stick; open Creator and click Back in a fresh context; in a separate context open Drawing, explicitly select Generate Frames, submit a fixed benign prompt, fulfill the single `/api/ai` POST in the browser with `drawing-generate-frames-response.json`, assert the request remains `workspaceType: "drawing"`, and visibly prove the existing client Canvas2D render/apply result. The real route, provider, search, and Supabase are never contacted by that Drawing mock.
6. Implement only the Phase 1.5 tester-core registry entries `tester.connection.ping/v1` and read-only `stick.phase2.checkpoint/v1`. The runner verifies the exact Workspace preimage, temporarily replaces only the inert anchor to connect the registered adapter, proves the read-only scaffold frame-selection/count and joint/limb-count checkpoint shape, then restores the exact original bytes in `finally` and signal cleanup. There is no mutation operation in Phase 1.5.
7. Run a guarded production build only after anchor restoration, using the same tester-only exact font-response interception without changing product source or styling. Fail if route/build manifests, emitted JavaScript/CSS/assets, server trace/NFT manifests, or production DOM contain the tester marker/path/control or tester-fixture path; fail if any `app/**` or `src/**` import reaches `scripts/runSpec0001BrowserProof.ts`, `scripts/spec0001-browser/**`, or the tester font fixtures; assert known forbidden tester URLs return 404. Bind the phase-base and final hashes of `app/layout.tsx`, every stylesheet, and every visible product source path and require equality except for the already authorized inert anchor comment. Source remains in the private development repository, but no secrecy claim is made if that repository later becomes public.
8. Before running, require an empty index and record the exact allowed unstaged phase diff. Refuse a pre-existing `.next`, tester font-cache/setup path, or tester-owned profile/output collision rather than deleting unknown data. On success or failure, close the browser/server/process tree, release ports, remove the runner-owned `.next`, temporary font setup/cache, and temporary profiles, restore the anchor byte-for-byte, and leave only declared ignored evidence. Cleanup failure fails the phase.
9. Record one strict evidence manifest with base/HEAD, exact argv/env allowlist and exit codes, source/fixture/schema/browser hashes, exact font request/response/fixture hashes, action ledger, visible assertion results, screenshot hashes, console ledger, browser/server blocked-network ledger, mocked Drawing request/response digests, unchanged visible-product-source hashes, production-exclusion inventory, process/port/profile/font-cache/build cleanup, anchor hashes, final empty index, and exact allowed diff. The independent validator rejects missing, extra, forged, privacy-unsafe, product-source-changing, or cleanup-failed evidence.

**Deterministic fixtures:** the strict Phase 1.5 plan and action registry; the fixed Drawing request expectation and response digest; exact allowlisted next/font request metadata and hash-bound tester-only response/font bytes; two viewports `1440×900` and `1024×768`; empty isolated storage; welcome shown/hidden states; Home/New/Stick/Creator/Back visible expectations; Drawing task-selection/request/apply expectation; loopback allowed and non-loopback denied attempts for browser plus each server network primitive; unexpected/malformed font request, fixture-hash mismatch, and font-cache cleanup failures; console warning/error capture; anchor wrong-preimage/wrong-replacement/restoration failure; missing browser/dependency; pre-existing owned-output collision; production marker/import/route/UI/asset/test-font leak fixtures; server/build failure; browser crash; and cleanup-on-success/failure/signal cases.
...
- Drawing Generate Frames uses exactly one browser-intercepted deterministic POST response, reaches the existing browser Canvas2D render/apply path, and makes zero route/provider/search/Supabase request.
...
- `app/layout.tsx`, stylesheets, and all other visible product-source bytes remain equal to their phase-base hashes except the separately authorized inert anchor comment; offline font fulfillment exists only in the guarded tester process, uses the app's requested font bytes, and creates no permanent style/source change or shipped tester asset.
...
- After both success and injected failure, the anchor/source bytes, temporary font setup/cache, process tree, ports, browser profiles, and build output are restored/removed exactly; only ignored evidence remains.
...
- Phase 1's 631-assertion contract proof remains valid; its published files/status are not rewritten. The known lint baseline is reported honestly and no Phase 1.5 file adds a lint finding.

**Protected regression flows:** Home → New; Home → New → Stick; fresh Stick → Creator → Back with Creator Save still disabled; Home → New → Drawing; Drawing task selection and existing Generate Frames browser render/apply; existing disabled Drawing tasks; application routing manifest; published UI; Phase 1 contracts/proof; repository cleanliness and recovery-branch isolation.
...
```bash
node --experimental-strip-types scripts/spec0001-browser/finalizePhase15Closeout.ts --base=<phase-base-sha> --proof=output/spec-0001/phase-1.5/proof-manifest.json --output=output/spec-0001/phase-1.5/proof-closeout-manifest.json
...
**Stop condition:** Phase 1.5 stops if offline dev/build proof cannot run through tester-only exact font-response interception without changing `app/layout.tsx`, a stylesheet, font selection, or any other real product behavior/source; no fallback font or product edit is permitted. It also stops if the tester needs a product route/page/API, a permanent application import, arbitrary evaluation, a remote asset, provider/search/Supabase/analytics access, unknown-data deletion, or any runtime behavior change beyond the inert anchor comment. Any failed visible flow, network-deny/font-interception self-test, Drawing regression, unchanged-product-source check, production-exclusion check, evidence validation, or cleanup assertion also stops the phase. Do not enter Phase 2.

**Implementation Review Packet requirements:** The Spec Executor reports the exact base/branch/worktree, implementation file allowlist, dependency/browser evidence and any separately authorized acquisition, all clicks/visible results/screenshots, Drawing request/response digests, console and browser/server network ledgers, exact next/font request/response/fixture hashes, phase-base/final `app/layout.tsx`/stylesheet/visible-source hashes, production manifests/forbidden-URL results, anchor preimage/replacement/restoration hashes, cleanup/process/port/profile/font-cache/build results, Phase 1 regression result, TypeScript/lint status, proof-manifest path/SHA, empty index, skipped/unproven checks, and exact systems untouched; then it stops. This packet never authorizes propagation or Git publication.

**Handoff requirements:** Arthur and the Project Manager accept or reject the stopped executor result. On acceptance only, a Control Plane Architect takes exclusive ownership of the same worktree, preserves all accepted implementation bytes, performs canonical propagation and closeout, returns its PM Review Packet, and stops. A later explicit instruction is required to stage/commit/integrate/push. Phase 2 remains unauthorized until Phase 1.5 is Verified, propagated, published, integrated into canonical `main`, and Arthur separately authorizes Phase 2.
...
**Systems intentionally unchanged:** `app/layout.tsx`, every stylesheet, font selection, and all visible website behavior; every product route and API; `app/page.tsx`; Drawing/Stick editor behavior; Drawing AI contracts/route/runtime; Stick state/timeline/canvas/right-panel/Creator behavior except the inert comment; persistence/memory/Supabase; provider/search; migrations/database/environment values; deployment; Git history and recovery branch.

### 10.5 Phase 2 — Independent per-frame Stick state

**Status:** **Unauthorized; Not started.** May begin only under separate owner authorization in a new task/worktree after Phase 1.5 is Verified, its Control Plane Architect propagation is accepted, and it is separately published and integrated into canonical `main`.
...
**Dependencies:** Phase 1 Verified/integrated contracts/goldens; Phase 1.5 permanent automatic browser tester/server-egress/tester-only-offline-font guard Verified and integrated; current timeline/playback/gesture behavior freshly reproduced through that tester; separate Phase 2 authorization.
...
| AC-22 Evidence | Phase 1.5 has a schema-valid reproducible proof bundle bound to its base SHA, tester-core/fixture/font hashes, exact initial action outcomes, click/visible/screenshot/console/network/unchanged-product/production-exclusion evidence, anchor preimage/replacement/restoration, temporary font-setup cleanup, and final diff allowlist. Every later browser phase adds only a versioned owning-phase extension, reruns all earlier accepted plans, and binds its own state evidence. No undefined scenario convention, temporary routable proof page, permanent anchor replacement, product font/style change, or retroactive change to accepted tester behavior remains. |
...
- Phase 1.5 gate: before Phase 2, use only the Verified/integrated permanent automatic browser tester, tested-process server-egress guard, and tester-only exact offline-font response strategy. It may not change application source, styling, font selection, or visible behavior and must restore all temporary setup byte-for-byte. Credentials are also scrubbed and `NEXT_TELEMETRY_DISABLED=1`, but those are defense in depth—not egress proof.
...
This table retains historical proposal/Phase 1 evidence and adds the docs-only Phase 1.5 amendment evidence.
...
| Phase 1.5 implementation/browser/build/type/lint/provider checks | Not rerun / not implemented | This task changes specification/control-plane Markdown only. No tester exists yet; no visible behavior, build, TypeScript, lint, browser, app, provider, or external-service pass is claimed. |
...
- Arthur approved the permanent automatic browser tester amendment in D-0011 and authorized Phase 1.5 only as **Approved; Authorized; Not started**.
...
- Phase 1 remains Verified, published, and integrated. Phases 2–7 remain Unauthorized/Not started. OpenAI, search, Supabase, paid services, deployment, and Git publication remain unauthorized.
- No Phase 1.5 implementation, dependency acquisition, browser run, runtime change, external request, stage, commit, or push occurred while recording approval.
...
| Approval/status consistency | Pass | SPEC-0001 remains Approved and active. OD-01–OD-06 and OD-09 are accepted; OD-07/08/10/11 retain their accepted safety/process boundary; OD-12–OD-14 remain deferred. Phase 1 is Verified/published/integrated; D-0011 makes Phase 1.5 Approved/Authorized/Not started; Phases 2–7 remain Unauthorized/Not started. |
| Documentation/control-plane finishing gates | Pass | The seven canonical Phase 1.5 approval/control-plane files were updated, `bash scripts/update_memory.sh` regenerated the canonical tree without changing it, its later `--check-only` passed, final diff checks passed, and the index/untracked set remained empty. `docs/DECISIONS.md` records D-0011; `AGENTS.md` and `docs/PROJECT_MANAGER_CONTEXT.md` remain unchanged because Arthur changed no PM-process rule. |
...
Final spec status is **Approved** and active. **Phase 1 is Verified, published, and integrated into canonical `main`. Phase 1.5 is Approved, Authorized, and Not started.** Phases 2–7 remain Unauthorized/Not started.

Exact next step: Arthur and the Project Manager review this Control Plane Architect approval-propagation packet, followed by a separate publication instruction if accepted. Only after D-0011 and the reviewed control-plane state are published/integrated may a new Phase 1.5 Spec Executor start in Plan mode in a dedicated worktree under D-0010; no implementation begins in this task. Phase 2 cannot begin until Phase 1.5 is technically Verified, accepted, propagated by a Control Plane Architect, separately published, integrated into canonical `main`, and Arthur separately authorizes Phase 2. The Phase 7 Policy Gate, every external lookup, and any paid/live request remain forbidden.

Control-plane files updated by this Phase 1.5 amendment are exactly:
...
`docs/DECISIONS.md` records Arthur's durable D-0011 approval. `AGENTS.md` and `docs/PROJECT_MANAGER_CONTEXT.md` remain unchanged. `project/project_structure.txt` changes only through the required memory helper. Final Git status is recorded in the PM Review Packet; no file is staged, committed, or pushed.
