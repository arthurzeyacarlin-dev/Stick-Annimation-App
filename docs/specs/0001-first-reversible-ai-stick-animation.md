...
Decision links: accepted SPEC-0001 decision D-0009, permanent role-separation decision D-0010, Phase 1.5 approval decision D-0011, protected-Drawing correction decision D-0012, Phase 2-only authorization decision D-0015, Phase 1.5 compatibility-correction decision D-0016, and still-broader pending decisions P-0004, P-0005, P-0007, and P-0008 in `../DECISIONS.md`
...
SPEC-0001 remains Approved and active. Phase 1 and the historical Phase 1.5 are Verified, published, and integrated into canonical `main`; Phase 1.5's implementation commit is `8df64552e29e4170df8000097fe857b7a31dff69`, and its six-document control-plane publication record is `687cbeaf6acbf9625e0d940e78bc600251eb0604`. D-0011 approved the original Phase 1.5 tester boundary, D-0012 authorized its conditional Drawing correction, and D-0015 plus its activation record were published in `a85690de9396cf97e3063005cbb6da85f109ae1d`, authorizing Phase 2 only as **Authorized; Not started**. D-0016 accepted the Phase 1.5 compatibility correction in §10.4A and was published/integrated at `6c2973caecb742334fb432bdda8fbc674bb7db42`. The correction is now **technically Verified/accepted in its dedicated worktree; unpublished/unintegrated**. It does not reopen the accepted Phase 1.5 user-visible result and did not implement or test product Phase 2. Phase 2 remains blocked until correction publication/integration, both required clean committed tester gates, and a later activation naming a new exact base/executor. Phases 3–7, the Phase 7 Policy Gate, and paid/live requests remain unauthorized and not started.
...
| Phase 1.5 compatibility correction — versioned later-phase proof | **Technically Verified/accepted; unpublished/unintegrated** | Preserves the exact no-plan Phase 1.5 suite while adding a fail-closed, hash-bound plan/registry/declarative-browser-adapter path, exact dirty/clean Git binding, truthful lint comparison, and synthetic compatibility proof. No product/UI behavior changes. | Historical Phase 1.5 Verified/published/integrated; D-0016 published/integrated; exact 23-path dirty proof and independent validation accepted; technical-manifest SHA-256 `53202d21ba7248e46a3e3423a623fbc785b5c3beedd6dd4d2a984b37614cffe8` | A separate publication task must preserve the accepted bytes, run the clean committed no-plan and byte-identical synthetic-plan gates, publish/integrate, and stop; technical proof alone does not unblock or start Phase 2. |
| 2 — Independent per-frame Stick state | **Authorized; Not started; blocked by unpublished compatibility correction** | Home → New → Stick visibly supplies the built-in figure, and the user creates the complete three-pose/12-frame wave without AI using the bounded hold/blank/start-pose controls; all 11 joints, playback, holds, and gesture boundaries use independent canonical poses. | Phase 1.5 Verified/published/integrated; D-0015 activation published; compatibility implementation technically accepted but not published/integrated; both clean committed tester gates and a later activation remain required | Stop before any Phase 2 byte until correction publication/integration and new activation are complete and a new Plan-mode Phase 2 executor starts from that exact SHA; then stop until the real no-AI manual construction flow matches the Phase 1 manual content digest and all-11-role isolation, rendering, playback, held-frame, and gesture proof passes. |
...
**Status:** **Technically Verified/accepted in the dedicated worktree; unpublished/unintegrated.** D-0016's approval is published/integrated at `6c2973caecb742334fb432bdda8fbc674bb7db42`. This is a named proof-infrastructure correction inside the already Approved SPEC-0001. It is not a new numbered product phase, does not reopen the historical Phase 1.5 user-visible result, did not implement or test a Phase 2 runtime byte, and does not change D-0015. The accepted implementation/proof now awaits a separate publication-only task.
...
**Exact accepted correction outcome:** The technically accepted implementation provides the following contract. Clauses that explicitly require clean committed or integrated state remain publication gates:
...
Arthur and the Project Manager accepted that stopped result, and D-0010 then transferred exclusive ownership to one Control Plane Architect. The architect revalidated the unchanged manifest/hash/allowlist, updated only the authorized canonical records, ran memory/check-only/diff/status checks, finalized `output/spec-0001/phase-1.5-compatibility/proof-closeout-manifest.json`, returned the Control Plane Architect packet, and stopped with an empty index. A later explicit publication task must commit only the accepted 23 implementation paths plus reviewed control-plane paths. On the clean committed phase branch, before canonical-main integration or push, it must pass both the exact no-plan `npm run test:spec0001-browser` and the exact synthetic `--plan` invocation. That invocation must derive `clean-committed`, observe `[]`, select the plan's immutable `cleanExpectedPaths: []`, prove the base-to-HEAD ceiling projection equals its immutable 23-path `dirtyExpectedPaths`, and bind catalog/plan/registry/adapter bytes identical to the accepted dirty proof. No second plan or pre-publication byte rewrite is allowed. Any failure stops publication. Only after clean publication/integration may a new control-plane activation name the new exact Phase 2 base and authorize a new dedicated Plan-mode Phase 2 executor.

**Non-goals and stop gate:** No runtime/UI/product behavior, Phase 2 implementation, historical evidence rewrite, package/dependency/configuration/environment/database change, new product/dev route, public asset, broad browser framework, external lookup, provider/search/Supabase request, paid service, deployment, or Git operation is part of this correction. The accepted result contains no product byte, broader route/network/filesystem/storage/evaluation authority, weaker default suite/recovery/temporary-restoration/production-exclusion rule, unlisted path, v1 evidence rewrite, lint regression, or residual process/profile/port/build/anchor byte. It remains unpublished/unintegrated. Technical success alone does not unblock Phase 2; publication/integration, both clean committed gates, and a later activation are still mandatory.

### 10.5 Phase 2 — Independent per-frame Stick state

**Status:** **Authorized; Not started; blocked.** D-0015 supplies the owner authorization and its activation record is published/integrated at `a85690de9396cf97e3063005cbb6da85f109ae1d`. D-0016's approval is published/integrated at `6c2973caecb742334fb432bdda8fbc674bb7db42`, and the §10.4A correction is technically Verified/accepted, but it remains unpublished/unintegrated. The historical Phase 1.5 product/tester result remains satisfied; product Phase 2 has not been implemented or tested. Phase 2 may begin only after the accepted correction is published/integrated with both clean committed tester gates passing and a later control-plane activation names the exact new Phase 2 SHA and executor.
...
- Phase 1.5 gate: the protected Drawing settled-pixel correction and original permanent tester are already Verified/published/integrated. Before Phase 2, the technically accepted and control-plane-propagated D-0016 §10.4A compatibility correction must be separately published/integrated, must pass both required clean-committed tester gates, and must be followed by a new activation naming the exact Phase 2 base/executor. The tested-process server-egress guard and tester-only exact offline-font response strategy may not permanently change `app/layout.tsx`, application styling, font selection, or unrelated visible behavior and must restore all temporary setup byte-for-byte. Credentials are also scrubbed and `NEXT_TELEMETRY_DISABLED=1`, but those are defense in depth—not egress proof.
...
- Publication state: this Control Plane Architect record and closeout preserve the accepted implementation bytes but do not stage, commit, merge, push, publish, or integrate them. Phase 2 remains Authorized/Not started/blocked until a later explicit publication task passes both clean committed tester gates, integrates the correction, and a later activation names a new base/executor. Phases 3–7 remain Unauthorized/Not started.
...
| Phase 1.5 compatibility external/product/non-Chrome/deployment | Not tested / not applicable to accepted correction | No OpenAI/provider/search/Supabase/paid/external/deployment action ran. No product Phase 2 or broader general E2E/non-Chrome behavior is claimed. The clean committed no-plan and synthetic-plan publication gates remain not run until the separately authorized publication task. |
...
| Current lint truth | Pass as measured non-regression; repository lint still fails | Fresh accepted v2 measurement at exact base/HEAD `8b663d2b80144e9aeba9ea0ecf0f78ccefa78926` records base/result 5 errors and 73 warnings, zero changed-line/new-file findings, and unchanged Git state. Historical Phase 1/Phase 1.5 6/73 receipts remain valid for their recorded commits. |
| Approval/status consistency | Pass | SPEC-0001 remains Approved and active. OD-01–OD-06 and OD-09 are accepted; OD-07/08/10/11 retain their accepted safety/process boundary; OD-12–OD-14 remain deferred. Phase 1 and historical Phase 1.5 are Verified/published/integrated. The §10.4A correction is technically Verified/accepted but unpublished/unintegrated. Phase 2 remains Authorized/Not started/blocked; product Phase 2 was not implemented or tested. Phases 3–7 remain Unauthorized/Not started. SPEC-0002 remains complete/protected; SPEC-0003 remains separate Proposed/inactive. |
...
Final spec status is **Approved** and active. **Phase 1 and historical Phase 1.5 are Verified, published, and integrated into canonical `main`; Phase 1.5 publication commit is `8df64552e29e4170df8000097fe857b7a31dff69`, accepted technical proof-manifest SHA-256 is `da2dd8cff32367a548a2e7d2e4e789fcf1a4dd129e9dc6200e25650f586f9fc9`, and closeout SHA-256 is `d7d74d9a48e31f997ed772625cc75be53a7d408b96fe093023b50c793c421423`.** D-0015 and the Phase 2 activation are published at `a85690de9396cf97e3063005cbb6da85f109ae1d`. D-0016 and the compatibility-correction approval are published/integrated at `6c2973caecb742334fb432bdda8fbc674bb7db42`. The §10.4A correction is **technically Verified/accepted in its dedicated worktree; unpublished/unintegrated**. Its accepted manifest SHA-256 is `53202d21ba7248e46a3e3423a623fbc785b5c3beedd6dd4d2a984b37614cffe8`; it proves compatibility infrastructure and synthetic Phase-2-shaped actions only. Product Phase 2 is **Authorized; Not started; blocked** and was not implemented or tested. Phases 3–7 remain **Unauthorized; Not started**.

Exact next step: Arthur and the Project Manager review this Control Plane Architect packet, then issue a separate explicit publication-only instruction. That later task may stage only the accepted 23 implementation/proof paths plus the reviewed eight control-plane paths, commit them on the phase branch, and must run both the exact no-plan permanent tester and the byte-identical synthetic `--plan` in clean-committed mode before canonical-main integration or push. It may fast-forward/push only if canonical `main` is still clean and unchanged. Only after publication/integration may a later control-plane activation name a new exact Phase 2 base/executor. This control-plane task starts neither publication nor Phase 2. Phases 3–7, the Phase 7 Policy Gate, every external lookup, and any paid/live request remain forbidden.

The historical six-document Phase 1.5 record remains published in `687cbeaf6acbf9625e0d940e78bc600251eb0604`. This compatibility closeout changes exactly these eight control-plane paths:
...
- `docs/testing_workflow.md`; and
- `project/project_structure.txt` through deterministic `bash scripts/update_memory.sh` regeneration.

No new owner decision occurred, so `docs/DECISIONS.md` remains unchanged. `AGENTS.md`, `docs/PROJECT_MANAGER_CONTEXT.md`, all accepted implementation/fixture/test/proof bytes, product/package/configuration bytes, SPEC-0002, the separate SPEC-0003 worktree, and recovery material are preserved exactly. Every task must establish exact current Git state with fresh `git rev-parse`, `git rev-list`, `git status`, index, and untracked-file checks rather than infer a future HEAD from this specification.
