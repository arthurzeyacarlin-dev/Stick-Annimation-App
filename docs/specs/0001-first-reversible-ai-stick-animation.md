...
Decision links: accepted SPEC-0001 decision D-0009, permanent role-separation decision D-0010, and still-broader pending decisions P-0004, P-0005, P-0007, and P-0008 in `../DECISIONS.md`
...
D-0010 governs all future implementation and prerequisite tasks. SPEC-0001 Phase 1 remains the completed historical exception under the former combined executor/control-plane workflow: it is already Verified, published, and integrated and must not be repeated, rewritten, or republished.
...
| OD-10 | Use seven sequential SPEC-0001 implementation phases. Approval authorized Phase 1 only. Each future phase retains one Spec Executor task/worktree, stop gate, technical proof, human review, separate Control Plane Architect propagation, separate Git-publication instruction, and canonical-main integration under D-0010. The practical sequence also includes a separately specified/implemented/published browser harness after Phase 1 and before Phase 2, plus a separate Phase 7 Policy Gate before provider implementation; this is therefore **not only six or seven Codex tasks**. | Bundling roles, phases, or prerequisites removes independent review and breaks the repository's Verified publication lifecycle. |
...
Decision D-0009 records the product/phase approval boundary: OD-01–OD-06 and OD-09 are accepted exactly; OD-07/08/10/11 are approved engineering rules/prerequisites; OD-12–OD-14 are explicitly deferred and unaccepted until the Phase 7 Policy Gate. D-0010 supersedes only OD-10's former combined executor/control-plane mechanics. No decision authorizes Phases 2–7, the separate browser-harness implementation, the Phase 7 Policy Gate, an external lookup, or a paid/live request.
...
Exact next step: review and separately publish the D-0010 control-plane process record without touching Phase 1. After that publication, the separately specified browser-harness prerequisite may begin only under its own future authorization and role-separated task sequence; Phase 2 still cannot begin until that prerequisite is Verified/published/integrated and Arthur separately authorizes Phase 2. The Phase 7 Policy Gate, every external lookup, and any paid/live request remain forbidden.
