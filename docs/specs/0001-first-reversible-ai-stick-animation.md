...
| 2 — Independent per-frame Stick state | **Authorized; Not started** | Home → New → Stick visibly supplies the built-in figure, and the user creates the complete three-pose/12-frame wave without AI using the bounded hold/blank/start-pose controls; all 11 joints, playback, holds, and gesture boundaries use independent canonical poses. | Phase 1.5 Verified/published/integrated; D-0015 owner authorization; D-0015 activation record must be separately published/integrated before executor start | Stop until the activation record is published and a new Plan-mode executor starts from that publication SHA; then stop until the real no-AI manual construction flow matches the Phase 1 manual content digest and all-11-role isolation, rendering, playback, held-frame, and gesture proof passes. |
...
**Status:** **Authorized; Not started.** D-0015 supplies the owner authorization. Its Phase 1.5 prerequisite is satisfied, but implementation may begin only after this activation record is separately reviewed, published, and integrated, in one new dedicated Plan-mode Phase 2 Spec Executor task/worktree from that activation publication SHA after a refreshed conflict audit.
...
**Dependencies:** Phase 1 Verified/integrated contracts/goldens; Phase 1.5 permanent automatic browser tester/server-egress/tester-only-offline-font guard Verified and integrated; current timeline/playback/gesture behavior freshly reproduced through that tester; D-0015 Phase 2 authorization published/integrated before executor start.
...
- Phase state at this publication point: Phase 1.5 was Verified, published, and integrated; Phase 2 and Phases 3–7 remained Unauthorized/Not started. D-0015 later authorized Phase 2 only as Not started, still requiring its activation record to be separately published before executor start.
...
| Approval/status consistency | Pass | SPEC-0001 remains Approved and active. OD-01–OD-06 and OD-09 are accepted; OD-07/08/10/11 retain their accepted safety/process boundary; OD-12–OD-14 remain deferred. Phase 1 and Phase 1.5 are Verified/published/integrated. D-0015 authorizes Phase 2 only as Not started; Phases 3–7 remain Unauthorized/Not started. |
