...
Last verified branch/commit: the accepted Phase 6 package is published and integrated in exact 33-path canonical-main commit `caa6c2d946780f384d0a8c58f4ea75a771483bcd`, parent `f19955c336be43b19b2c4cc13d16abcc7ded7247`, message `Implement SPEC-0001 Phase 6 deterministic Stick chat`. It contains exactly 25 manifest-bound technical paths inside the 26-path ceiling plus eight canonical records. Local `main`, local `origin/main`, and live GitHub `main` matched cleanly at `0/0` after publication.

Arthur approved this specification on 2026-08-11. OD-01 through OD-06 and OD-09 are accepted, with D-0027 superseding only OD-01's former exact-sentence prompt boundary, D-0028 correcting Phase 6's visible outcome, and D-0029 accepting the corrected implementation and same-root Creator continuity. OD-07, OD-08, OD-10, and OD-11 are approved engineering rules/prerequisites. D-0009 historically deferred OD-12 through OD-14 to the former Phase 7 Policy Gate; D-0033 retires that route without accepting any of those values. Phase 1 through Phase 6 are **Verified, published, and integrated**. The former Phase 7 is **Superseded by SPEC-0004; retired; unauthorized; and must not be implemented as written**.
...
D-0018 fixes real Stick onion skin as part of Phase 3 without changing Drawing onion behavior. D-0019/D-0020 govern the published Phase 3 result at `3fe3a5487389647b67216e9466121e00f1a73856`; D-0022/D-0023 govern Phase 4 at `71841e96499f7627139c53d87114bba65e19d29d`; and D-0024 through D-0026 govern Phase 5 at `9bbcc1df2fe4c79c0947601d0ea6274a85732d85`. D-0027 is published at `f46ed3b13e6bca3a09c9b2926c972bea8c331f2c`; D-0028 is published at `f19955c336be43b19b2c4cc13d16abcc7ded7247`; and D-0029's corrected exact 25-path Phase 6 result plus eight records is published/integrated at `caa6c2d946780f384d0a8c58f4ea75a771483bcd`. D-0033 preserves that history and retires Phase 7 in favor of [`SPEC-0004`](0004-future-real-ai-animator-requirements.md). D-0035 historically authorized only its Phase 1; D-0036 later accepted that result, and GIT-033 published/integrated it at `086420e6b0cbe683adbb8f0024e65a2fc1d68d6d`. Phases 2–8 remain unauthorized.

> **D-0033/D-0035 supersession boundary:** Every Phase 7 sentence, command, file list, policy proposal, cost figure, acceptance criterion, proof instruction, and owner gate anywhere in this document is retained only as superseded historical design material. None carries implementation authority. Do not implement the old Phase 7, extract individual requirements from it, start its Policy Gate, or treat its proposed values as accepted. SPEC-0004 is the sole replacement. D-0035's historical authorization covered only Phase 1; D-0036 later accepted it, and GIT-033 published/integrated it at `086420e6b0cbe683adbb8f0024e65a2fc1d68d6d`. Phases 2–8 remain unauthorized.
...
**Current Phase 6 visible interpretation under D-0017/D-0028:** the published §10.5A workspace, not the owner-rejected automatic Phase 2 initializer, is the visible starting point. A fresh Phase 6 flow therefore begins with one compact blank cell and an empty canvas. The strict Phase 1 built-in starter remains the internal command-contract foundation used by Phase 4, but it must not be mistaken for already-rendered visible content. Preview and Cancel leave the canvas empty; only explicit Apply projects the one approved built-in `humanoid-11-v1` result into the editable timeline. After Apply, the real figure must visibly render through the existing `structureGraph` canvas path on each resolved keyframe/hold. This clarification changes neither the accepted command bytes nor the transaction, history, editability, or manual-equivalence rules.
...
| Availability | Published Phase 5 exposes a provider-free marked availability GET, but canonical Stick still uses a read-only Drawing panel and has no Phase 6 client state. | The client may check same-origin readiness silently before submission, but D-0028 requires the original idle panel until the user sends; only then may bounded readiness/eligibility copy appear. | The first rejected review copy exposed readiness too early and displaced the owner-required user-first idle state. | Mocked GET contract plus initial-idle absence proof and post-send `checking | available | unavailable | error` UI; no provider call. |
...
| Workspace AI composer | Read-only/currently unavailable until Phase 6; D-0028 removes the initial wave suggestion and pre-send readiness copy | Before Phase 6 the existing read-only Drawing panel remains canonical. Corrected Phase 6 preserves the original presentation and exact Stick idle sentence until the user sends; section 8.7 then governs precise outcome/readiness copy. |
...
| Creator Back and Creator-local canvas/rig controls | Creator remains a separate screen; Back is enabled where currently functional | Back returns to the same mounted Stick project/history/storage/selection/view root and rebinds stage observation to the remounted current canvas. It does not create a fresh project or discard accepted editor state. |
...
Typing the exact canonical sentence is no longer required. D-0028 supersedes D-0027's initial-load suggestion-chip requirement: Phase 6 shows no suggestion chip or shortcut explanation before the user's first send attempt. The golden sentence remains a fixture and canonical server projection, not required visible starter content.
...
1. Choose **Home → New Project → Stick Figure Animation**. The published §10.5A one-cell empty canvas is visible; no figure has been created.
2. Confirm the untouched bottom-right panel retains the original presentation: the existing AI Animator header/helper style and source-authored helper string `Generate frames • Clean drawings • Animate faster`, placeholder `Chat here`, white 28×28 circular checkmark send control, and the transparent/muted idle sentence `Ask the assistant for help with your stick figure.` No eligibility warning, suggestion chip, shortcut explanation, or Preview/Apply/Cancel result is visible yet.
3. Type any accepted section 4.3 prompt and activate the checkmark send control. Sending requests a preview; the button must not visibly say **Preview**.
4. Inspect the authoritative preview copy: **“Understood: one stick figure, a three-pose wave, 12 frames at 12 FPS. No changes have been made.”** The canvas remains empty. Choose **Cancel** in one run and prove no figure or timeline content is created; request a new preview and choose **Apply** in the complete run.
5. After Apply, visibly inspect the real built-in figure on the canvas at displayed Frames 1, 5, and 9 and representative held Frames 2, 6, and 10. Turn Onion ON while paused on Frame 5 and visibly inspect the previous figure in the existing purple treatment and the next figure in the existing green treatment.
6. Choose **Play**, pause, select any controlling keyframe or hold, and drag any joint to prove the AI-created positions are ordinary editable poses.
7. Choose **File → Save**, hard-reload to Home, choose **Open Project → Stick Figure → the saved project**, and play again.
...
3. Before any user send, confirm the bottom-right panel exactly preserves the original header/helper presentation, source-authored helper string `Generate frames • Clean drawings • Animate faster`, placeholder `Chat here`, and white 28×28 circular checkmark submit control; shows only the transparent/muted exact idle sentence `Ask the assistant for help with your stick figure.`; and shows no availability/eligibility warning, suggestion chip, shortcut explanation, preview, Apply, or Cancel. A provider-free no-store availability `GET /api/ai` may complete silently, but it must not replace or decorate the idle state. The initial stage remains the published empty canvas.
4. Type every section 4.3 A-case through the visible composer and activate the checkmark send control, proving each maps to the same server-authoritative canonical intent. Exercise representative R-cases visibly and the complete R-set through shared/guarded-route proof with no preview or mutation. An unavailable/ineligible/digest-pending state becomes visible only after that send attempt and exposes only the bounded safe reason, never a credential name/value, exact mode, provider identity, environment name/value, or secret.
5. Assert exactly one `POST /api/ai` for each eligible accepted submission, with `X-Diamond-AI-Workspace: stick-figure`, body `workspaceType: "stick-figure"`, request/context/manifest version 2, the current project ID/revision, a request ID, a transaction ID, and search/tools disabled. An unavailable/ineligible send makes no POST. The earlier optional availability GET is not a provider request. Assert no search, Supabase, provider, or other remote request in mock mode.
...
7. Before Apply, compare canonical project, history, and storage bytes with their pre-request values; all must be identical. The clipped real stage has zero current-figure limb/joint geometry and the same exact pixel digest as the fresh empty stage. In a separate visible Preview → Cancel run, Cancel preserves those bytes, the one blank cell, zero figure geometry, and the exact empty-stage pixel digest.
8. Choose Apply once.
9. Assert the real Stick project/timeline now contains the canonical built-in rig/figure identity projected into the editable `structureGraph`, has three independent owner keyframes at 0/4/8 and nine valid holds, and revision incremented once. Assert exactly one undo entry. Do not require a legacy `figures[]` stage glyph: the integrated renderer's actual built-in figure is the resolved 11-joint/10-limb graph.
10. Select displayed Frames 1, 5, and 9 and representative held Frames 2, 6, and 10. For each, bind the resolved render-input digest to the live non-onion SVG limb/joint attributes and clipped-stage pixels. Prove the Frame 1/5/9 geometry and pixel masks are three distinct positions; Frames 2–4 exactly match Frame 1, Frames 6–8 exactly match Frame 5, and Frames 10–12 exactly match Frame 9. Then choose Play and visibly confirm those three positions, wrap after Frame 12 at 12 FPS, and pause. Timeline/document counts alone and an uninspected screenshot are insufficient.
11. Pause on displayed Frame 5 and turn Onion ON. Prove one previous and one next complete 10-limb/11-joint overlay are actually rendered: every previous node carries `data-onion-side="previous"` and exact tint `rgba(92, 63, 158, 0.58)`; every next node carries `data-onion-side="next"` and exact tint `rgba(44, 122, 91, 0.56)`. Bind each overlay's live DOM geometry to the expected Frame 1/Frame 9 neighbor and prove visible purple/green pixel clusters at those transformed coordinates in the clipped stage. The normal Frame 5 figure remains present/editable. Turn Onion OFF and prove both overlay DOM nodes and colored pixels disappear without canonical mutation.
12. Choose Undo once and confirm the complete AI pose/timing batch disappears, restoring the exact one-cell empty canvas with zero figure geometry/pixels. Choose Redo once and confirm exact candidate digest, IDs, poses, FPS, timeline, selection, visible graph geometry/pixels, and onion behavior are restored.
...
16. Confirm rig/figure/pose/frame identities, both corrected joint roles and coordinates, the derived line-head position through its `head` joint, 12 FPS, keyframe/held-frame ownership/timing, selected/current frame behavior, **Saved on this browser**, visible graph geometry/pixels, and onion colors/neighbor figures survive reopen; playback begins paused and then renders the same three body positions.
...
18. For every failure, assert an honest scoped message, zero partial content, byte-identical canonical project/history/storage pre-state, unchanged stage geometry/pixels, and no unhandled browser error. Exact redelivery while preview is open reuses one preview; after successful Apply it returns `duplicate` with the original candidate/revision and adds no mutation/history entry.
19. Protect the corrected §10.5A Home → New → Stick empty-canvas/manual-timeline flow, Stick → Creator → Back before and after edits and local operations, Home → Open Drawing project, and the existing Drawing Generate Frames request/render/apply path. No Creator lock or replacement copy may appear; the Creator Save button remains in its existing unimplemented state. At both required viewports, compare exact-base and result screenshots/computed geometry and require zero unintended pixel/layout change outside the bottom-right Stick AI panel and the intentionally created canvas/timeline content after Apply. Fat frames, altered panel dimensions, and any unrelated font/color/size/layout/navigation change fail.
...
| --- | --- | --- |
| Initial idle before the first send attempt | Preserve the original bottom-right AI panel header/helper presentation, placeholder `Chat here`, and white circular checkmark send control. The message area is transparent/unboxed muted text exactly “Ask the assistant for help with your stick figure.” Show no availability/eligibility warning, suggestion chip, shortcut explanation, preview, Apply, or Cancel. | One provider-free no-store availability GET may run silently. It causes no POST/provider call, project/history/storage mutation, or visible state before the user acts. |
| First send while availability is still checking | “Checking Stick AI availability. No changes were made.” | This copy appears only after the send attempt. Finish or repeat only the provider-free GET; make no POST/provider call or mutation. |
| First send while capability is off | “Stick AI wave creation is not enabled in this environment. No changes were made.” | No POST/provider call or mutation. |
| First send with missing credentials | “Stick AI is unavailable because the server is not configured. No changes were made.” | No POST/provider call, secret exposure, or mutation. |
| First send when production is forbidden | “Stick AI wave creation is not available in production. No changes were made.” | `production_forbidden`; no POST/provider call or mutation. |
| First send while temporarily unavailable | “Stick AI is temporarily unavailable. Check again. No changes were made.” Check again repeats only the availability GET. If a previously available request loses the one-use-authority race, use the same copy. | No provider call, preview, or mutation. A losing POST returns stable `temporarily_unavailable`, HTTP 503. |
| First send after availability-check failure | “Stick AI availability could not be checked. Try again. No changes were made.” | Fail closed; no POST/provider call or mutation until a valid available response. |
| First send while document digest is pending | “Preparing this Stick project. No changes were made.” | Keep rendering only the last ready document; publish no candidate and make no POST/provider call. Authoring, Save, preview exposure, and Apply remain unavailable under the existing root-safety rule. |
| First send after document digest failure | “This Stick project could not be prepared safely. Retry. No changes were made.” | Preserve the last ready document/history/generation; Retry hashes that published document only and never revives a discarded candidate. |
...
| First send from an ineligible edited project | “This AI shortcut works only in a new empty 12 FPS Stick project. You can still build the wave with the timeline controls. No changes were made.” | Evaluate locally after user action; no availability POST/provider call or mutation. |
...
| Unsupported or ambiguous prompt | “I couldn’t safely match that request to the one supported result: one stick figure, a three-pose wave, 12 frames at 12 FPS. No changes were made.” | Reject before mock/provider and preview; do not suggest that an exact sentence or a suggestion chip is required. |
| Non-eligible project | “This AI shortcut works only in a new empty 12 FPS Stick project. You can still build the wave with the timeline controls. No changes were made.” | Reject before mock/provider; manual creation remains available. |
...
| New `src/components/workspace/stickfigure/StickFigureAiPanel.tsx` | Phase 6 writable scoped chat that reuses the original AI panel presentation, hides all readiness/shortcut/result content until the first send, then shows preview, Apply/Cancel, and honest states | User-facing primary interface | No Drawing panel rename/redesign; no initial suggestion/warning/shortcut; original helper, placeholder, idle treatment, and checkmark submit presentation are protected |
| `src/components/workspace/stickfigure/StickFigureWorkspace.tsx` | Own one canonical editable root; expose shared manual/AI transaction/history/save adapters; and ensure Phase 4 Apply installs the resolved canonical wave into the existing active `structureGraph` render/onion source | The current visible starter is empty; only Apply creates the real editable built-in figure, and renderer state must follow that publication | Home/Creator mount flows; no canvas renderer redesign |
...
| `src/components/workspace/ai/WorkspaceAiPanelShell.tsx` | Reuse the original panel presentation unchanged; this remains inside the 26-path authorization ceiling only if a non-visual compatibility prop is mechanically required | Preserve the protected AI Animator shell rather than redesigning it | Existing source-authored helper string, fonts, colors, sizing, spacing, dimensions, and Drawing defaults remain byte/visually identical; an implementation that needs no shell edit leaves this path unchanged |
...
SPEC-0001 remains Approved historical implementation authority through Phase 6. Phase 1, historical Phase 1.5, §10.4A, §10.5A, and Phases 3 through 6 are Verified, published, and integrated. D-0027's Phase 6 activation is published at `f46ed3b13e6bca3a09c9b2926c972bea8c331f2c`; D-0028 records Arthur's rejection of the first unpublished review copy and the corrected visible outcome; D-0029 accepts the corrected exact 25-path result and same-root Creator continuity, published with eight records at `caa6c2d946780f384d0a8c58f4ea75a771483bcd`. D-0033 retires old Phase 7 and its Policy Gate; neither can start. SPEC-0004 is the sole future AI Animator home. D-0035 historically authorized only Phase 1; D-0036 accepted it, and GIT-033 published/integrated it at `086420e6b0cbe683adbb8f0024e65a2fc1d68d6d`. Paid/live requests and Phases 2–8 remain unauthorized.
...
| 5 — Server raw-dispatch and mocked boundary | **Verified, published, and integrated** | Marked availability and mock POST work through a guarded real `/api/ai`; malformed/oversized Stick intent fails before Drawing, while marker-free Drawing reaches the original unread path; no editor/UI mutation is possible. | Exact publication commit `9bbcc1df2fe4c79c0947601d0ea6274a85732d85`, parent `a2b4f3e0fc492df9cd63bda32554e382a344cdb6`; corrected exact 22 technical paths plus eight records; proof SHA `f9024a5d86cc1febbac7df3d4219348cd8289d17723406bac5d90929d4cf5c0f`; closeout SHA `fd71afec8e637bb3736a63dbbe245c6616e14a4330b91f7e354297d96c2c24dd`; 362 source assertions; 31 real-route cases; Arthur visible acceptance under D-0026 | Complete. D-0027's separate Phase 6 activation is published at `f46ed3b13e6bca3a09c9b2926c972bea8c331f2c`; D-0028 changes no Phase 5 history. |
| 6 — Writable Stick chat and UI integration | **Approved; Authorized; correction required** | The closed typo-friendly V2 grammar maps every accepted spelling/paraphrase/default to one server-authoritative no-mutation preview. Before the first send the original panel presentation shows only the exact idle sentence; after explicit Apply the real editable built-in figure visibly renders as three positions/holds, with existing purple/green onion neighbors. Unsupported/ambiguous/expanded requests and Cancel create no figure. | Phase 5 Verified/published/integrated; D-0027 activation published at `f46ed3b13e6bca3a09c9b2926c972bea8c331f2c`; D-0028 correction must be reviewed and published before a separately authorized correction executor resumes Phase 6 | Stop until the exact A01–A15/R01–R36 matrix, V1 preservation, client/server parity, V6 tester registration, initial-panel/strict-visual baseline, Cancel-empty proof, applied canvas geometry/pixels, held-frame rendering, exact onion DOM/color/pixel proof, manual-versus-mocked-AI comparison, failure/Creator states, Drawing Generate Frames, and all protected flows pass. The first unpublished review copy is rejected evidence, not implementation acceptance. |
| 7 — Retired feature-flagged provider proposal | **Superseded by SPEC-0004; retired; must not be implemented** | Historical proposal only. It defines no current product outcome or executable phase. | None; the former Policy Gate is retired | Do not start. SPEC-0004 is the replacement. D-0035 historically authorized only Phase 1; D-0036 accepted it, and GIT-033 published/integrated it at `086420e6b0cbe683adbb8f0024e65a2fc1d68d6d`. Phases 2–8 remain unauthorized. |
...
**Status:** **Accepted, technically Verified, published, and integrated.** D-0027 is published at `f46ed3b13e6bca3a09c9b2926c972bea8c331f2c`; D-0028 is published at `f19955c336be43b19b2c4cc13d16abcc7ded7247`; and D-0029's accepted exact 25-path implementation plus eight records is published/integrated in canonical `main` at `caa6c2d946780f384d0a8c58f4ea75a771483bcd`. The first review remains rejected history. Phase 7 remains unauthorized and not started.

**D-0033 status update:** The preceding Phase 7 wording is preserved Phase 6 history only. D-0033 now retires old Phase 7 and its Policy Gate; the accepted Phase 6 result remains unchanged.

**Exact observable outcome:** Using the already Verified Phase 5 mock boundary, the initial bottom-right Stick AI panel preserves the original AI Animator presentation. Its helper remains the protected source-authored `Generate frames • Clean drawings • Animate faster`; its input placeholder is exactly `Chat here`; its submit control is the original white 28×28 circular checkmark and never visibly says **Preview**; and its only initial message is transparent/unboxed muted text exactly **“Ask the assistant for help with your stick figure.”** Before the user's first send attempt there is no readiness/eligibility warning, suggestion chip, shortcut explanation, preview, Apply, or Cancel. The user acts first. Sending then requests a preview or reveals the appropriate safe rejection/eligibility explanation.

The panel accepts section 4.3's V2 closed grammar, including Arthur's named paraphrases, safely implied three-pose/12-FPS values, the audited `wwave`/`animaton`/`onne` corrections, and the uniquely bounded `122 FPS` repeated-digit correction. Before Apply it says exactly **“Understood: one stick figure, a three-pose wave, 12 frames at 12 FPS. No changes have been made.”** and shows the validated candidate. Preview and Cancel keep the current one-cell canvas empty and create no figure. Only explicit Apply invokes Phase 4 and must visibly install the real built-in 11-joint/10-limb figure through the existing editable `structureGraph` render path. Frames 1/5/9 and their holds visibly show the three intended positions. Paused Onion on Frame 5 visibly shows the previous figure with exact purple `rgba(92, 63, 158, 0.58)` and the next figure with exact green `rgba(44, 122, 91, 0.56)`. The result remains manually editable, undoable, saveable, and reopenable. Rejected/ambiguous/expanded prompts create no preview or mutation. The complete mocked browser acceptance binds document/render inputs to live SVG geometry and clipped-stage pixels rather than accepting timeline counts or screenshots on faith, while preserving Creator, Drawing, canvas/timeline/workspace presentation, and zero non-loopback traffic.

**Dependencies:** Phases 1–5 and Phase 1.5 Verified/published/integrated; D-0027 activation published at `f46ed3b13e6bca3a09c9b2926c972bea8c331f2c`; current sidebar/shell/Workspace, Phase 4 editable-wave projection, Phase 3 onion resolver, unchanged canvas renderer, published Phase 5 parser/mock path, and tester-extension catalog freshly traced. Phase 6 may add V2 interpretation beside V1 in the shared files below because the live Phase 5 dispatcher already delegates semantic parsing/materialization there. It may correct the Workspace integration inside the existing ceiling so Apply's resolved editable content reaches the canvas/onion source. It may not rewrite V1, the canvas renderer, onion resolver/tints, raw reader/classifier, route ownership, availability protocol, or Phase 5 history/proof.

**Exact authorized systems/files:**

The authorization ceiling remains exactly these 26 technical paths; no 27th tracked technical path is permitted. The correcting Spec Executor must report and bind its exact final dirty subset. A ceiling path that needs no correction—especially the protected shared shell—stays byte-identical rather than being touched merely to reach 26:
...
8. `src/components/workspace/ai/WorkspaceAiPanelShell.tsx` only if a non-visual backward-compatible compatibility prop is mechanically required; its existing helper copy/style, dimensions, defaults, and Drawing rendering must remain exact;
...
**Immutable/read-only boundary:** `src/components/workspace/stickfigure/StickFigureCanvas.tsx`, `src/lib/stickFigure/stickProjectHistory.ts` including both exact onion tints/resolution, `app/api/ai/route.ts`, `src/lib/ai/stickFigureAiServerDispatch.ts`, `src/lib/ai/strictStickJson.ts`, `src/lib/ai/stickFigureAiAvailability.ts`, every `scripts/fixtures/stick-ai/v1/*` fixture, all existing tester v2–v5 schema files and the first five authorization records, all Phase 1–5 proof artifacts/records, Phase 1–4 executor/history/storage/canonical-document behavior, `DrawingAiPanel`, Drawing AI/runtime/storage/memory, Creator, dependencies/package scripts, config, environment, and deployment. The original panel helper/source style, all non-panel workspace styling, and every unrelated font/color/size/layout/panel/canvas/timeline/navigation byte are protected. If the visible figure/onion result cannot be corrected through `StickFigureWorkspace.tsx` plus the already authorized panel/adapter/proof paths, the executor stops and returns to the Spec Architect; it does not add Canvas as a 27th path, reopen Phase 5, or expand scope.

**Explicit non-goals:** General fuzzy matching, runtime edit-distance search, arbitrary synonyms, other prompts/actions/counts/FPS, route/raw-parser/classifier/availability-protocol changes, live provider/model/policy, OpenAI/search/tools, transcript persistence, memory/Supabase, Drawing panel/task redesign, generic adapter refactor, structural/timing editor enablement beyond the integrated bounded human path, Reasoning/Task controls, or any unrelated visual cleanup. Reasoning and Task remain deferred to a future spec and may remain absent.
...
3. Add the narrow Stick panel by reusing the original panel shell/presentation. The initial header/helper style and exact source string, composer dimensions, `Chat here` placeholder, 28×28 white circular checkmark button, and transparent/unboxed muted idle treatment are protected. Show the exact Stick idle sentence and no warning, suggestion chip, shortcut explanation, preview, Apply, or Cancel before the user sends. The checkmark is the send affordance that requests a preview; do not label it **Preview**. Do not add Reasoning/Task controls.
4. On mount/project change, the marked provider-free availability GET may run silently and cache only coarse readiness. It must not alter the pre-send idle presentation. On the first send attempt, resolve readiness and project/digest eligibility before POST; only then show the applicable section 8.7 explanation when blocked. Never expose secret/mode/environment details. A normal ready fresh project lets the user type and send without a pre-emptive warning.
...
6. Dry-run the returned command through Phase 4, then show the exact understood-result copy and candidate-derived bounded preview. Planning/preview never mutates document/history/storage or visible stage geometry. Preview, Cancel, Request/Stop, errors, and session-only transcript do not persist. Cancel removes the preview and leaves the one-cell empty canvas with zero figure geometry/pixels.
7. Route Apply exclusively through Phase 4's executor/composite-root publication and shared mutation functions; never build a second AI-only document or set timeline/graph/history fields directly. Correct only the existing Workspace integration as necessary so the applied canonical wave projects into `activeContent.structureGraph`, selection/playback resolve the published timeline, and existing onion inputs resolve from the same editable root. Disable Apply after any ready-digest/generation/project invalidation.
8. Register `phase-6/v1` as the sixth tester extension with schema/result version 6, a Phase 6 product-port adapter, and only the operation families `visible-role`, `visible-testid`, `pointer`, `workspace-driver`, `runner-environment`, `guarded-http`, `checkpoint`, `screenshot`, `protected-regression`, and `fixture`. The visible action module may read the existing `[data-testid="stick-stage"]` SVG, current limb/joint attributes, `data-onion-side`, `data-onion-tint`, computed geometry/style, and clipped screenshots; it may not add a product test route/control or modify Canvas. Rerun the five earlier accepted plans/modes without changing their fixtures or meaning.
9. Run the no-AI Human path in one isolated context and mocked V2 AI path from the exact eligible visible empty starter in another. Compare their exact content projections/digests and Frame 1/5/9 render inputs before any post-Apply correction. For the AI context, bind preview/Cancel to zero stage geometry/pixels; bind Apply to the live 10-limb/11-joint graph; bind each keyframe/hold to its live SVG geometry plus clipped-stage pixel mask; and bind Frame 5 Onion to both exact-tint neighbor graphs plus visible color pixels. Then edit at least two different joint kinds through normal visible controls, including one held-frame-selected edit; prove no permanent glow, exact Undo/Redo, Save/Open, and honest status/local-only messages. Protect Drawing Generate Frames by fulfilling its mocked response in-browser while the server guard denies all non-loopback traffic.
10. At `1440×900` and `1024×768`, bind exact-base `f46ed3b…` and corrected-result screenshots plus computed rectangles/styles. Before user action, require the bottom-right panel to match the protected original presentation except the exact Drawing-to-Stick idle noun substitution; require zero pixel/layout delta outside the panel. During/after Apply, allow diffs only inside the existing panel output region and the already existing canvas/timeline content regions. Reject any font, color, size, spacing, panel dimension, button-size, toolbar/right-panel/workspace/navigation change or fat-frame regression.

**Deterministic fixtures:** The exact section 4.3 accepted/rejected set; typo-map uniqueness/distance/budget failures; V1 exact-normalizer and mock-route preservation; request/manifest/envelope V1/V2 version-crossing failures; byte-identical client/server canonical intent; accepted local preflight followed by authoritative server rejection/disagreement; paired bounded editor contexts; available/off/production/temporarily-unavailable/invalid availability; eligible/ineligible/pending/failed document states; pre-send idle/presentation with no forbidden content; valid V2 request/envelope/preview; Preview Cancel with empty canvas; Apply with live three-position graph/holds; exact previous/next onion geometry/tints/pixels; delayed timeout/Stop/stale/project switch; invalid/unsupported response; active-preview duplicate/conflict; Apply failure; exact-base/result visual masks at both viewports; and visible Creator-preservation cases for silent availability, first send, successful request, timeout, Stop, invalid output, stale response, Cancel, Undo, Save, Open, and project switch. No suggestion-fill fixture remains.

**Acceptance criteria:**

- Before the first send, the panel displays only the protected original header/helper presentation, placeholder `Chat here`, white circular checkmark send control, and exact muted unboxed Stick idle sentence. No availability/eligibility warning, suggestion chip, shortcut explanation, preview, Apply, or Cancel is visible. Silent availability checking may not change that state.
- The user acts first. The checkmark send control requests the preview but never visibly says **Preview**. After a send attempt, only the bounded preview, rejection, readiness, or eligibility result may appear. Every section 4.3 accepted prompt succeeds without an exact-sentence or suggestion-chip requirement; Drawing task selectors/text remain absent, and Reasoning/Task controls remain deferred/optional-absent.
...
- Preview visibly says **“Understood: one stick figure, a three-pose wave, 12 frames at 12 FPS. No changes have been made.”** before Apply. Pre-Apply document/history/storage bytes, saved baseline, zero current-figure geometry, and clipped empty-stage pixel digest are exact. Cancel changes none of them and does not create a figure.
...
- Apply publishes one Phase 4 root and the real built-in figure visibly renders through the existing editable `structureGraph`: exactly 10 current limbs and 11 current joints with live attributes bound to the canonical resolved pose. Frames 1/5/9 have distinct geometry/pixel masks; their respective held spans 2–4/6–8/10–12 exactly match. Timeline/document counts or screenshots without the bound DOM/render/pixel evidence do not pass.
- With playback paused on Frame 5 and Onion ON, one complete previous 10-limb/11-joint figure uses exact tint `rgba(92, 63, 158, 0.58)` and Frame 1 geometry, while one complete next figure uses exact tint `rgba(44, 122, 91, 0.56)` and Frame 9 geometry. Exact attributes and transformed DOM geometry bind to visible purple/green clipped-stage pixel clusters; OFF removes both without canonical mutation. Normal visible UI then edits at least two different joint kinds (including a held-frame-selected edit), with no permanent role highlight. Exact Undo/Redo, Save/Open, identities/FPS/keyframe/hold timing, and local-only disclosure satisfy section 6.
- First user AI submission, timeout, Stop, invalid/stale response, Cancel, Undo, Save, and Open leave the real Creator control present and usable after terminal resolution; silent availability GET does the same.
...
- Client and server network guards prove zero OpenAI/search/Supabase/non-loopback traffic. Drawing Generate Frames and disabled Drawing tasks retain their current behavior. Exact-base/result proof at both viewports shows no unrelated font, color, size, spacing, layout, panel dimension, button size, timeline, canvas shell, right-panel, workspace, or navigation change and no fat-frame regression; only the specified bottom-right Stick AI states and intentionally created canvas/timeline content may differ.

**Protected regression flows:** All section 6 step 19 flows; corrected Home → New → Stick empty canvas/manual timeline; Stick → Creator → Back before and after edits/local/request outcomes; Stick timeline/playback/manual/history/persistence; Drawing panel/task selection, Generate Frames mocked request/render/apply, project operations, storage, and memory boundaries. No lock explanation may appear. The exact-base/result visual mask permits only the bottom-right Stick AI panel states and the intentional post-Apply canvas/timeline content; no font, color, size, spacing, panel dimension, 28×28 send-button dimension, toolbar, right-panel, workspace, navigation, canvas-shell, or fat-frame difference is allowed.
...
The V6 authorization/catalog validator requires exactly six ordered records and binds the Phase 6 plan, registry, result schema, action module, maximum 26-path ceiling, exact reported dirty subset, and fixture hashes. The first five records and all v2–v5 schemas/plans/registries/results remain byte-identical. Phase 6 technical and closeout modes must reject an unreported/extra/reordered dirty path, any path outside the ceiling, wrong authorization, direct-driver substitution, changed earlier fixture, unregistered operation, unbound matcher/UI/render matrix, or un-restored proof anchor. An authorized ceiling path that remains byte-identical is not a failure.

The Verified harness first proves the untouched initial panel at both viewports, then executes every A01–A15 accepted prompt through the visible composer/checkmark control to an authoritative server-returned preview and Cancel, followed by one complete A01 Preview → Apply path. R01–R36 run through the shared Node matcher and guarded real HTTP boundary; representative semantic, numeric, ambiguous, Unicode, and oversized rejections also run visibly and prove no preview/mutation. The harness separately executes the Human path and mocked AI path through normal visible UI in fresh isolated contexts. It records their content/render digests before any post-Apply edit. The AI run additionally records zero pre-Apply/Cancel stage geometry and pixels; exact post-Apply current/onion SVG geometry and attributes; transformed screen coordinates; clipped-stage pixel masks/color samples; Frame 1/5/9 distinctness; hold equality; Onion OFF cleanup; and reopened visual equivalence. Exact-base/result screenshots and computed rectangles/styles prove the strict visual mask. Malformed raw bytes remain guarded-HTTP plus Node proof, exhaustive internal failure points remain driver plus Node proof, and only user-reachable controls/messages are claimed as visible UI. Direct driver operations cannot substitute for timeline clicks, panel typing/checkmark/Cancel/Request/Stop/Apply clicks, Onion clicks, or visible figure/color proof. The one visible injected failure uses `armNextVisibleApplyFailure` then the real Apply button. Environment fixtures supply delayed/aborted/invalid HTTP, digest, and storage schedules. Drawing requests are browser-fulfilled and server egress is independently denied. Manual memory, document/timeline counts alone, or old/uninspected screenshots do not substitute.

**Stop condition:** Do not enter the Phase 7 Policy Gate until the exact matcher matrix, V1 preservation, client/server canonical-intent parity, protected idle panel, visible understood-result copy, Cancel-empty proof, real applied figure/held-pose geometry and pixels, exact purple/green onion neighbor proof, strict visual mask, full Human and mocked AI paths, content/render equivalence, Creator-preservation matrix, Drawing regressions, proof restoration, and no-egress checks pass. Any suggestion/warning/shortcut before first send; text-labeled **Preview** submit; missing applied figure; counts-only or screenshot-only render claim; wrong/missing onion figure or tint; fat-frame/unrelated visual change; general fuzzy correction; accepted unsupported semantic/number; silent/defaulted explicit conflict; AI-only state/mutation power; direct UI mutation; immutable-boundary modification; hidden remote attempt; or unavailable repeatable browser proof blocks Phase 6.

**D-0033 status update:** The preceding stop condition is preserved as Phase 6 history only. D-0033 retires old Phase 7 and its Policy Gate, so no later task may enter that former gate regardless of the historical prerequisite text.

**Handoff requirements:** The correction Spec Executor records its exact dirty subset inside the unchanged 26-path ceiling; V1 preservation digests; typo-map validation; every A/R fixture result; client/server/canonical-intent/envelope parity; exact pre-send panel strings/styles/absence assertions; visible preview copy and no-mutation/Cancel-empty checkpoints; applied keyframe/hold/current/onion geometry, DOM attributes, transformed coordinates, pixel masks/colors, and reopened evidence; both visible paths and their base/content/render digests; exact-base/result visual masks at both viewports; declared bookkeeping differences; readiness/UI/request/result/root and Creator-preservation matrices; full acceptance and Drawing regression; proof-manifest/anchor-restoration hashes; and client/server network evidence in its Implementation Review Packet, then stops. Arthur and the Project Manager receive a new exact-worktree review copy under D-0021. Only after their explicit acceptance and exclusive transfer may the Control Plane Architect record the accepted implementation, complete section 10.1 closeout, and keep live mode off and Phase 7 not started.
...
**Systems intentionally unchanged:** Published Phase 5 V1 contract behavior/fixtures/proof/history and server route/raw-dispatch/availability behavior; OpenAI helpers/client; all Drawing contracts/prompting/runtime/executor/storage/memory; `StickFigureCanvas.tsx`, existing onion resolver/tints, Stick canonical document/storage schema/history/executor/transaction semantics and accepted animation outcome; Creator implementation; all non-panel visual presentation; dependencies/package scripts; config/environment; production/deployment settings.

### 10.10 Retired Phase 7 — Feature-flagged bounded OpenAI integration (superseded by SPEC-0004)

**Status:** **Superseded; retired; unauthorized; must not be implemented as written.**

> Everything in this section is preserved solely as historical evidence of the former proposal. Its observable outcome, dependencies, file list, implementation steps, proof commands, policy values, and stop conditions are void as implementation authority under D-0033. The replacement is SPEC-0004. D-0035 historically authorized only Phase 1; D-0036 accepted it, and GIT-033 published/integrated it at `086420e6b0cbe683adbb8f0024e65a2fc1d68d6d`. Phases 2–8 remain unauthorized.
...
> Under D-0033, every Phase 7-specific criterion below is retained as historical text only. It is not an active acceptance gate and cannot authorize old Phase 7 work.
...
| AC-03 UI scope | Before first send, the bottom-right panel preserves the original header/helper presentation and source-authored helper `Generate frames • Clean drawings • Animate faster`, placeholder `Chat here`, white 28×28 circular checkmark submit, and exact muted/unboxed idle sentence `Ask the assistant for help with your stick figure.` It shows no readiness/eligibility warning, suggestion chip, shortcut explanation, preview, Apply, Cancel, or Drawing task selector. Sending—not a text-labeled Preview button—requests the preview. Only after send may bounded readiness/eligibility/rejection/preview content appear, never credential names/values, exact mode, provider identity, environment names/values, or secrets. Reasoning/Task may remain absent. |
...
| AC-05 Preview | Only after authoritative intent/envelope and dry-run candidate validation, the preview shows exactly “Understood: one stick figure, a three-pose wave, 12 frames at 12 FPS. No changes have been made.” plus Apply/Cancel; project, history, storage, one-cell timeline, zero current-figure SVG geometry, and clipped empty-stage pixels are unchanged until Apply. Cancel preserves every one of those values and creates no figure. Client/server disagreement or any rejected prompt yields no preview. |
| AC-06 Apply | Apply revalidates the visible eligible empty starter and the Phase 4 internal canonical starter, then atomically projects the approved rig/figure into the existing editable timeline. It commits three complete independent key poses at zero-based 0/4/8 and nine holds at 1–3/5–7/9–11, performs one revision/generation increment plus one undo entry, and makes the real 11-joint/10-limb built-in visibly available through `activeContent.structureGraph`. It composes the shared transaction/manual mutation authority; command beats do not persist, no AI-only edit state exists, and the immutable Canvas renderer needs no change. |
| AC-07 Playback/onion rendering | Live DOM geometry and clipped-stage pixels prove three distinct body positions: Frame 1 through held Frames 2–4, Frame 5 through held Frames 6–8, and Frame 9 through held Frames 10–12, then wrap after index 11 at 12 FPS. On paused Frame 5, Onion shows one previous complete figure with `rgba(92, 63, 158, 0.58)` and Frame 1 geometry plus one next complete figure with `rgba(44, 122, 91, 0.56)` and Frame 9 geometry; exact attributes/coordinates bind to visible color pixels. OFF removes both without mutation. Counts, render-input digests, or screenshots alone are insufficient, and no held frame is claimed to own an independent pose. |
...
| AC-14 Honest errors | Before first send, section 8.7's exact idle presentation is the only message. After send, every applicable condition shows the specified scoped message or equivalent approved copy, never raw exception/provider output, and explicitly says no changes were made when true. |
...
| AC-19 Regression | Home/New/Stick empty starter, complete human-only wave, fresh Creator/Back, Stick timeline/history/save/onion, Drawing project flows, and Drawing Generate Frames pass their required protected proofs. Exact-base/result evidence at `1440×900` and `1024×768` proves no unrelated font/color/size/spacing/layout/panel dimension/button size/timeline/canvas shell/right-panel/workspace/navigation or fat-frame change; only the bottom-right Stick AI states and intentional post-Apply content may differ. |
...
| AC-22 Evidence | Phase 1.5 has one independently valid strict 49-artifact proof bundle bound to its base SHA, tester-core/fixture/font hashes, exact initial action outcomes, both viewports, final Drawing success/input/settled-pixel evidence, all 37 negative cases, click/visible/screenshot/console/browser+server+WebSocket-network/unchanged-product/production-exclusion evidence, success/failure/`SIGINT`/`SIGTERM` cleanup, anchor preimage/replacement/restoration, temporary font-setup cleanup, and final diff allowlist. Every later browser phase adds only a versioned owning-phase extension, reruns all earlier accepted plans, and binds its own state evidence. Phase 6 appends the sixth `phase-6/v1` record with V6 schemas/registry/result, exact reported dirty subset inside the unchanged 26-path ceiling, protected idle-panel assertions, Cancel-empty geometry/pixels, applied keyframe/hold current-figure geometry/pixels, exact onion DOM/tint/color proof, and exact-base/result visual masks while the first five records and v2–v5 semantics remain unchanged. No undefined scenario convention, counts-only/screenshot-only render claim, temporary routable proof page, permanent anchor replacement, `app/layout.tsx` or unrelated product font/style change, or retroactive change to accepted tester behavior remains. |
| AC-23 Manual/AI equality | The exact manual action fixture and AI command fixture retain their byte-identical canonical command-starter foundation and produce byte-identical canonical `StickAnimationContentV1`, identical `animationContentDigest`, and identical resolved render-input digests for displayed Frames 1/5/9. The current UI adapter begins from the published visible empty starter and may not treat the internal command starter as pre-Apply rendered content. The projection includes rig template/role topology, figure label, fixed line-head rule, all joint coordinates, keyframe indexes, hold owner indexes, FPS, and timeline length; it excludes only the enumerated IDs/revision/history/session/storage/request bookkeeping. Phase 2 proves the no-AI visible construction, Phase 3 proves its history/save/reopen, Phase 4 proves shared mutation authority, and Phase 6 compares the real mocked AI result with the proven manual result plus visible geometry/pixels. |
...
| REG-15 | Availability UX and live-proof readiness | New same-origin GET and panel state could leak server configuration, pre-empt the user, alter Drawing presentation, or accidentally claim/rearm paid authority | Exact response allowlist/no-store/no secret fields; silent initial GET leaves the exact idle panel unchanged; checking/unavailable/eligibility copy appears only after send; repeated GET is read-only; absent/consumed/restarted authority stays unavailable; shared-shell defaults and original helper/composer/checkmark dimensions match current Drawing appearance. |
...
| REG-17 | Human/AI content equality | The AI executor could drift into a private document builder, fail to publish into the live render source, or produce different hold/pose semantics | Phase 1 pure projection equality, Phase 2 real no-AI construction, Phase 3 human history/save/reopen, Phase 4 shared-mutation trace, and Phase 6 paired visible contexts must all match the same `animationContentDigest` and Frame 1/5/9 render-input digests; Phase 6 additionally binds those inputs to live current/onion SVG geometry and clipped-stage pixels. |
...
| REG-19 | Original Stick AI panel and strict visual shell | The first rejected Phase 6 review copy changed the bottom-right panel presentation and counts-only proof missed absent canvas content | Exact-base/result proof at both viewports binds the protected original helper/header styling, `Chat here`, 28×28 white checkmark, exact unboxed idle copy, and absence of pre-send warning/chip/shortcut/output; visual masks reject every unrelated pixel/layout delta and fat-frame regression. Preview/Cancel prove empty stage; Apply/holds/onion prove actual geometry and color pixels. |
...
- screenshots at the applicable phase starter (the current Phase 6 starter is empty), each blank keyframe before Start Pose, manual keyframes 0/4/8, paired AI keyframes 0/4/8 when applicable, representative held frames, a selected head/body/arm/leg joint, the derived line head after correction, and after reopen; no screenshot may substitute for bound state/content evidence;
- for Phase 6, exact-base/result screenshots and computed rectangle/style records at both viewports; an explicit allowed-region mask limited to the bottom-right Stick AI panel and intentional post-Apply canvas/timeline content; and zero unexplained pixel/layout delta outside that mask;
- for Phase 6, pre-send DOM/text/style/absence assertions; pre-Apply and post-Cancel zero current-figure geometry plus identical clipped-stage pixel digests; post-Apply non-onion SVG line/circle attributes bound to the canonical resolved pose and transformed screen coordinates; clipped-stage non-background pixel masks for Frames 1/5/9 and representative holds; exact previous/next onion node counts, sides, tint attributes, neighbor-geometry digests, visible purple/green pixel clusters, and OFF cleanup. A timeline/document count, render-input digest, or screenshot without these cross-bindings is insufficient;
...
### 14.21 Phase 6 first visible review rejected; correction specified 2026-08-21

- Exact canonical basis: D-0027's seven-document typo-friendly activation was published in commit `f46ed3b13e6bca3a09c9b2926c972bea8c331f2c`, parent `da7c258078a0240da32018b9ecfa7f33ae27d5ac`, message `Authorize SPEC-0001 Phase 6 typo-friendly prompts`. Canonical `main` at that commit contains no Phase 6 runtime implementation.
- Human-review result: Arthur directly rejected the first unpublished Phase 6 review copy and supplied the corrected visible outcome. Arthur and the Project Manager accepted the seven-document correction, and the Control Plane Architect independently closed out its canonical-source feasibility and lifecycle record. The rejected implementation remains isolated in the separate stopped executor worktree and is not inspected, accepted, propagated, staged, committed, published, or treated as evidence by this documentation correction.
- Rejected reviewed behavior: the review copy did not visibly render the real built-in figure after Apply despite creating timeline cells, and its bottom-right panel exposed pre-action guidance/eligibility UI, a suggestion shortcut, and a visibly text-labeled Preview control instead of the original panel presentation.
- Corrected owner outcome: the user must act first. Initial presentation retains the original helper/header styling, `Chat here`, white circular checkmark send control, and exact muted Stick idle sentence with no warning/chip/shortcut/output. Preview remains explicit and non-mutating; Cancel creates no figure. Apply must visibly render the real built-in graph on all three key poses/holds, and paused onion must visibly render exact purple previous and green next figures.
- Fresh source trace at `f46ed3b…`: `WorkspaceAiPanelShell.tsx` contains the source-authored helper `Generate frames • Clean drawings • Animate faster`; its separator is `U+2022 BULLET`, the exact existing source glyph for Arthur's dot-separated wording, so it must not be normalized to a different character. `DrawingAiPanel.tsx` supplies `Chat here`, the 28×28 white circular checkmark, and an unboxed muted empty state; `StickFigureRightPanel.tsx` currently mounts that Drawing panel read-only. `StickFigureWorkspace.tsx` resolves canvas content from `activeContent` and already passes onion overlays; Phase 4's editable-wave projection installs 11 joints/10 limbs in `structureGraph` with `figures: []`. `StickFigureCanvas.tsx` already renders those SVG nodes and exposes exact onion side/tint attributes. `stickProjectHistory.ts` owns the accepted purple/green constants and distinct-neighbor resolution.
- Feasibility/boundary result: the existing 26-path Phase 6 ceiling is sufficient. `StickFigureWorkspace.tsx`, the new panel/adapter, and the already authorized proof paths can correct publication/presentation and prove the result; `StickFigureCanvas.tsx` and the onion resolver/tints remain immutable. The ceiling becomes a maximum authorization set with an exact reported dirty subset so the shared shell stays unchanged if no compatibility edit is necessary. No new implementation phase is added.
- Lifecycle: Phase 5 remains **Verified, published, and integrated** at `9bbcc1df2fe4c79c0947601d0ea6274a85732d85`. Phase 6 is **Approved; Authorized; correction required**; no Phase 6 runtime is accepted/Verified/published/in canonical main. Phase 7 remains **Unauthorized; Not started**. This accepted Control Plane Architect closeout changes documentation only and performs no runtime/test/fixture/script/proof/package/config/environment/database/external/Git publication work.

### 14.22 Corrected Phase 6 implementation accepted and technically Verified 2026-08-21

- Ownership and lifecycle: the correction Spec Executor stopped completely, Arthur reviewed the exact corrected app and accepted it, and the Control Plane Architect completed closeout from implementation base `f46ed3b13e6bca3a09c9b2926c972bea8c331f2c`. The accepted 25 technical paths plus eight records were then published/integrated in exact canonical-main commit `caa6c2d946780f384d0a8c58f4ea75a771483bcd`, parent `f19955c336be43b19b2c4cc13d16abcc7ded7247`, message `Implement SPEC-0001 Phase 6 deterministic Stick chat`. Phase 6 is **accepted, technically Verified, published, and integrated**. Phase 7 remains **Unauthorized; Not started**.
- Accepted visible result: the original helper `Generate frames • Clean drawings • Animate faster`, `Chat here`, original white circular checkmark sender, and exact idle sentence `Ask the assistant for help with your stick figure.` remain visible before first send, with no warning, suggestion, preview, Apply, or Cancel. The bounded typo-friendly V2 request produces a non-mutating preview; Cancel is a no-op; Apply installs the canonical 12-frame/12-FPS three-pose wave exactly once.
- Accepted rendering and continuity: the full 10-limb/11-joint figure and exact purple/green onion neighbors render inside the responsive stage. Creator → Back preserves the same Stick project, figure, history, storage, selection, and view root; the remounted canvas rebinds its stage observation. Atomic Undo after Back removes the entire applied wave, Redo restores it, and manual-edit history plus Save/Open survive the round trip.
- Root causes: the initial missing figure/onion came from drawing canonical `960×594` coordinates 1:1 into a smaller responsive SVG without a `viewBox`, clipping geometry outside the stage. The later Creator → Back disappearance came from the `ResizeObserver` remaining attached to the pre-Creator detached canvas, leaving the remounted canvas with stale dimensions after a viewport change even though the figure/history still existed.
- Frozen technical boundary: exactly 25 manifest-bound technical paths changed inside the published 26-path ceiling; `src/components/workspace/ai/WorkspaceAiPanelShell.tsx` is the permitted but intentionally unchanged 26th path. Proof-manifest SHA-256 is `c0d13c9234087855784ad805410667e9d583ac0f03dbe847257db778fea39297` (48,147 bytes), browser-runner SHA-256 is `b1be9c83951f027687e2b50fa7df702ffa09b2e8213a7c30b6c7b0e9fa6220bd`, and the framed 25-path aggregate is `3bf3290f09f72787fa28db4f1a96003ab5f13e846358361e5805812904078aa2` over 1,065,268 input bytes.
- Proof result: 15/15 receipts and 88 artifacts pass. The V2 matcher/UI adapter records 338 assertions, 15 accepted visible requests, 36 guarded rejections, and 13 visible rejections. Browser proof binds the empty Preview/Cancel stage, 10-limb/11-joint Apply result, distinct Frames 1/5/9 and holds, exact onion tints/full overlays, the real Creator → Back route at `1440×900` then `1024×768`, atomic Undo/Redo, manual-edit history, later Save/Open, protected Drawing/Creator/Phase 1–5 behavior, TypeScript, measured lint non-regression, diff/scope/index/self-tests, independent manifest validation, zero non-loopback/provider attempts, and cleanup.
- Non-goals preserved: no Phase 7, OpenAI/provider/search/Supabase/paid/deployment behavior, broad visual redesign, Drawing change, or unrelated editor feature was added. Publication remains a separate explicit Control Plane Architect task.
...
| --- | --- | --- |
| Phase 6 activation publication | Pass | Exact seven-document commit `f46ed3b13e6bca3a09c9b2926c972bea8c331f2c`, parent `da7c258078a0240da32018b9ecfa7f33ae27d5ac`, message `Authorize SPEC-0001 Phase 6 typo-friendly prompts`. This published authorization only; it contains no Phase 6 runtime. |
| Phase 6 first human review | Rejected / unpublished | Arthur rejected the separate executor worktree's visible result. No rejected runtime byte or proof is accepted, copied into this worktree, published, or claimed as current behavior. Phase 6 is correction-required. |
| D-0028 source trace and file feasibility | Pass for specification evidence | At clean `f46ed3b…`, the original helper/composer/checkmark presentation, empty editable starter, Phase 4 editable-wave projection, active `structureGraph` canvas source, existing SVG node path, and exact onion constants/attributes were read directly. This proves the unchanged 26-path ceiling can support a Workspace/panel/proof correction while Canvas/onion source stays read-only. It does not prove the future corrected runtime. |
| D-0028 Control Plane Architect closeout | Pass; published | The seven-document correction was published in canonical record commit `f19955c336be43b19b2c4cc13d16abcc7ded7247`, parent `f46ed3b13e6bca3a09c9b2926c972bea8c331f2c`, message `Record D-0028 Phase 6 correction`. |
| Corrected Phase 6 human/visual/browser acceptance | Pass; Verified/published/integrated | Arthur accepted the corrected app. Fifteen receipts and 88 artifacts bind the protected initial panel, Preview/Cancel empty state, real 10-limb/11-joint Apply result, distinct keyframes/holds, exact purple/green onion overlays, responsive geometry, same-root Creator → Back continuity, atomic Undo/Redo, manual-edit history and Save/Open, inherited protections, zero egress, and cleanup. Proof SHA-256 is `c0d13c9234087855784ad805410667e9d583ac0f03dbe847257db778fea39297`; runner SHA-256 is `b1be9c83951f027687e2b50fa7df702ffa09b2e8213a7c30b6c7b0e9fa6220bd`. The exact 33-path package was published at `caa6c2d946780f384d0a8c58f4ea75a771483bcd`, and the clean committed permanent browser plan passed before integration. |
...
| Approval/status consistency | Pass for current published implementation state | SPEC-0001 remains Approved and active through Phase 6. Phase 1, historical Phase 1.5, §10.4A, §10.5A, and Phases 3 through 6 are published/integrated. D-0027's Phase 6 activation is published at `f46ed3b13e6bca3a09c9b2926c972bea8c331f2c`, D-0028's corrected requirements at `f19955c336be43b19b2c4cc13d16abcc7ded7247`, and D-0029's accepted exact 25-path Phase 6 result plus eight records at `caa6c2d946780f384d0a8c58f4ea75a771483bcd`. Old Phase 7 is retired and not started. SPEC-0002 remains complete/protected; SPEC-0003 remains separate Proposed/inactive. |
| D-0033/D-0035 Phase 7 transition | Pass for the docs-only transition and current successor lifecycle | The preceding approval/status row is retained historical evidence. D-0033 preserves Phase 1–6 and retires old Phase 7/its Policy Gate; D-0034 records the complete direction; and D-0035 historically authorized only SPEC-0004 Phase 1. D-0036 later accepted that result, and GIT-033 published/integrated it at `086420e6b0cbe683adbb8f0024e65a2fc1d68d6d`. Phases 2–8 remain unauthorized, and old Phase 7 must not be implemented. |
...
Arthur accepted OD-01 through OD-06 and OD-09 on 2026-08-11. D-0027 later supersedes only OD-01's prompt-input boundary; D-0028 supersedes the initial suggestion-chip/panel presentation and strengthens visible canvas/onion proof without changing the accepted animation outcome.
...
| --- | --- | --- | --- |
| OD-01 | Make the first applied outcome one built-in figure performing three body positions across 12 frames at 12 FPS: keyframes at displayed Frames 1, 5, and 9, with held frames between them. A human must be able to build this exact content without AI; the golden sentence and section 4.3's typo-friendly deterministic grammar are only a faster route. The current Phase 6 visible starter stays empty until Apply; Preview/Cancel create no figure. Do **not** create 12 separate body positions. | Twelve unique positions or smoother motion would require more keyframes, frame-by-frame authoring, or interpolation/tween rules and is later work. D-0027 replaces exact-sentence dependence with bounded paraphrases/safe corrections; D-0028 removes the pre-send suggestion chip and requires actual applied/onion rendering proof while continuing to reject broader semantics/counts/FPS. |
...
#### Retired Phase 7 owner gates (historical only)

No row below was accepted. D-0033 retires the former Phase 7 Policy Gate, so these rows are no longer pending SPEC-0001 gates and cannot be implemented or accepted piecemeal. D-0034 captures the replacement direction in SPEC-0004. D-0035 historically authorized only Phase 1; D-0036 accepted it, and GIT-033 published/integrated it at `086420e6b0cbe683adbb8f0024e65a2fc1d68d6d`. Its named later owner gates and Phases 2–8 remain separate and closed.
...
Decision D-0009 records the original product/phase approval boundary: OD-01–OD-06 and OD-09 are accepted exactly; OD-07/08/10/11 are approved engineering rules/prerequisites; OD-12–OD-14 were explicitly deferred and never accepted. D-0010 supersedes only OD-10's former combined executor/control-plane mechanics. D-0017 governs the §10.5A correction, now Verified/published/integrated at `edfb3dea023119b91336e6e5da645d4982a9f068`. D-0018 fixes the exact Phase 3 Stick onion-skin behavior. D-0019 authorized Phase 3; D-0020 accepts its exact tester-registration/Creator-preservation boundary at `54234b7c7b95201e274975a804859fa9c36806a1`; and Phase 3 is published/integrated at `3fe3a5487389647b67216e9466121e00f1a73856`. D-0022 authorized only Phase 4 and accepted its exact 22-path proof-feasibility correction. D-0024 authorized only Phase 5's section 10.8 provider-free server/mock boundary. D-0027 accepts the section 4.3 typo-friendly Phase 6 outcome and published 26-path feasibility ceiling while leaving Phase 5 V1 history immutable. D-0028 records the rejected first Phase 6 review and corrects only Phase 6's original-panel presentation, user-first reveal sequence, visible Apply/Cancel/onion outcome, and proof. D-0029 records Arthur's acceptance of the corrected exact 25-path Phase 6 result and same-root Creator continuity. D-0033 retires the former Phase 7 and its Policy Gate without accepting OD-12–OD-14; D-0034 records the complete SPEC-0004 direction; D-0035 historically authorized only its unchanged Phase 1; D-0036 accepted it; and GIT-033 published/integrated it at `086420e6b0cbe683adbb8f0024e65a2fc1d68d6d` without opening Phases 2–8 or any later owner gate.
...
- **SPEC-0004 — One-Time Stick Figure AI Animator** is the only requirements home for the design that replaces the retired SPEC-0001 Phase 7. D-0035 authorized only Phase 1; D-0036 accepted it; and GIT-033 published/integrated it at `086420e6b0cbe683adbb8f0024e65a2fc1d68d6d`. Phases 2–8 carry no implementation authorization today.
...
Final spec status is **Approved historical implementation authority through Phase 6 only**. **Phase 1, historical Phase 1.5, §10.4A, §10.5A, and Phases 3 through 6 are Verified, published, and integrated.** D-0027's typo-friendly Phase 6 activation was published at `f46ed3b13e6bca3a09c9b2926c972bea8c331f2c`, D-0028's visible correction record at `f19955c336be43b19b2c4cc13d16abcc7ded7247`, and D-0029's accepted exact 25-path implementation plus eight records at `caa6c2d946780f384d0a8c58f4ea75a771483bcd`. Proof-manifest SHA-256 is `c0d13c9234087855784ad805410667e9d583ac0f03dbe847257db778fea39297`; runner SHA-256 is `b1be9c83951f027687e2b50fa7df702ffa09b2e8213a7c30b6c7b0e9fa6220bd`; framed 25-path aggregate is `3bf3290f09f72787fa28db4f1a96003ab5f13e846358361e5805812904078aa2`. Phase 7 is **Superseded by SPEC-0004; retired; and must not be implemented**. SPEC-0002 and SPEC-0003 remain complete/protected.

Current next product step: Arthur and the Project Manager may discuss SPEC-0004 Phase 2. Phase 1 is Verified, published, and integrated at `086420e6b0cbe683adbb8f0024e65a2fc1d68d6d`; Phase 2 has not started and requires a new explicit Arthur authorization before one dedicated executor may begin. Phases 2–8 remain unauthorized.
...
Arthur's published shell correction remains recorded as D-0017; D-0018 remains the exact onion contract; D-0019/D-0020 govern Phase 3; D-0022/D-0023 govern Phase 4; D-0024/D-0025 authorize and bound Phase 5; D-0026 accepts its exact technical result; D-0027 is the published Phase 6 typo-friendly activation; D-0028 governs its rejected-review correction; and D-0029 accepts the corrected implementation and same-root Creator continuity. The exact Phase 6 closeout records are `docs/CURRENT_STATE.md`, `docs/DECISIONS.md`, `docs/SESSION_HANDOFF.md`, `docs/TODO.md`, `docs/changelog.md`, `docs/specs/0001-first-reversible-ai-stick-animation.md`, `docs/specs/README.md`, and deterministically generated `project/project_structure.txt`. `AGENTS.md`, `docs/PROJECT_MANAGER_CONTEXT.md`, all accepted technical/proof bytes, package/configuration bytes, SPEC-0002, the separate SPEC-0003 worktree, the rejected historical review state, and recovery material remain preserved. Every later task must establish exact current Git state rather than infer a future HEAD from this specification.
