...
Decision links: accepted SPEC-0001 decision D-0009, permanent role-separation decision D-0010, Phase 1.5 approval decision D-0011, protected-Drawing correction decision D-0012, and still-broader pending decisions P-0004, P-0005, P-0007, and P-0008 in `../DECISIONS.md`
...
Last verified branch/commit: Phase 1 corrections and renewed offline proof were completed from exact base `832d1f93630d7093514af3e81399077ebed696b4` and published in canonical-main commit `21a88feb65cf1cc51138c9ad4879b962ee468569`; D-0010 was published and integrated in `2029fd07e14b6f48feb6d04e02dbd52ec683d55d`; D-0011 was published and integrated in canonical-main commit `a35a268764c21eedffcf3d82b59718699b62d4d0`

Arthur approved this specification on 2026-08-11. OD-01 through OD-06 and OD-09 are accepted exactly as written; OD-07, OD-08, OD-10, and OD-11 are approved engineering rules/prerequisites; OD-12 through OD-14 remain deferred to the separate Phase 7 Policy Gate. That approval authorized Phase 1, which is Verified, published, and integrated into canonical `main` as of 2026-08-13. On 2026-08-14 Arthur approved the **Phase 1.5 — Permanent Automatic Browser Tester** amendment in D-0011. The first Phase 1.5 Executor attempt is now **Blocked; stopped; unaccepted; unpublished** because its corrected fail-closed tester exposed a protected Drawing Generate Frames settlement regression outside the approved runtime allowlist. On 2026-08-15 Arthur approved the narrow Drawing diagnostic/correction expansion in D-0012; the correction is **Approved; Authorized; Not started/resumed**. Phases 2–7, the Phase 7 Policy Gate, and every live/paid/external request remain unauthorized and not started.
...
| Phase 1.5 protected Drawing settlement | The corrected mocked tester observed at least 100 red pixels after `applyGeneratedFrameToWorkspace`, then observed zero qualifying pixels after the AI status settled. Both empty-output and explicit tester-only success-text variants reproduced the disappearance. Source tracing shows two remaining clearing boundaries—later `DrawingWorkspace` bitmap resolution/restore and `DrawingCanvas` dimension/reset—but current evidence does not identify the first writer. | The generated bitmap remains in the real Drawing timeline and stays visibly rendered after final success, after the textarea and submit become usable, and through a stable final screenshot. | The current Phase 1.5 result is blocked; the former false-positive manifest is invalid, no replacement manifest exists, and the exact clearing writer requires a diagnostic trace before any behavior patch. | A diagnostic-only run binds the canonical frame bitmap, every relevant render caller/resolution, canvas identity/dimensions, pixel checkpoints, and UI-settlement time; only the proven narrow writer may then be corrected and the complete Phase 1.5 proof rerun from clean owned output. |
...
SPEC-0001 remains Approved and active. Phase 1 is Verified, published, and integrated into canonical `main`. D-0011 approved the original Phase 1.5 tester boundary, but the stopped Executor result is blocked, unaccepted, and unpublished. D-0012 approves and authorizes the conditional Drawing correction as Not started/resumed; implementation may begin only in a separately authorized Plan-mode correction task after this approval record is reviewed, separately published, and integrated. Phase 2, Phases 3–7, the Phase 7 Policy Gate, paid/live requests, and implementation inside this documentation task remain unauthorized.
...
| 1.5 — Permanent Automatic Browser Tester | **Blocked; stopped Executor result unaccepted/unpublished; protected-Drawing correction Approved/Authorized but Not started/resumed** | One permanent developer command opens the real app offline, proves Home → New → Stick, fresh Stick → Creator → Back, and mocked Drawing Generate Frames through final settled pixels/input usability, emits repeatable evidence, proves cleanup, and proves no tester UI/route/import/asset or visible source/style change ships in the production result. | Phase 1 Verified/published/integrated; D-0010 integrated at `2029fd07e14b6f48feb6d04e02dbd52ec683d55d`; D-0011 integrated at `a35a268764c21eedffcf3d82b59718699b62d4d0`; D-0012 approval record separately reviewed/published/integrated; separately authorized Plan-mode Phase 1.5 correction Spec Executor | Stop first unless diagnostics identify the first clearing writer inside the exact two-file boundary. Then stop until the corrected tester starts from clean owned proof output, retains the generated pixels through final settlement/input usability at both viewports, passes all 37 negatives and every original offline/cleanup/production/Phase-1 gate, and emits an independently valid strict 49-artifact proof manifest. |
...
**Status:** **Verified, published, and integrated into canonical `main` on 2026-08-13 after corrected PM review.** D-0011 later authorized the original Phase 1.5 boundary; its stopped Executor result is now blocked/unaccepted/unpublished, and D-0012 authorizes the correction as Not started/resumed. Phase 1 did not implement Phase 1.5, and Phases 2–7 remain unauthorized and not started.
...
**Status:** **Blocked; correction Approved and Authorized but Not started/resumed.** D-0011 authorized the original tester boundary, but the stopped Phase 1.5 Executor result is unaccepted and unpublished. The corrected tester failed closed on the protected Drawing settlement requirement, invalidating the earlier false-positive proof; no valid current proof-manifest SHA exists. D-0012 approves the conditional two-file Drawing diagnostic/correction expansion. A separate Plan-mode Phase 1.5 correction Spec Executor may resume only after D-0012 is reviewed, separately published, and integrated under D-0010. Phase 2, automatic dependency/browser/font downloading, external services, Git publication, and every other product feature remain unauthorized.
...
- Phase 1.5 gate: before Phase 2, the currently blocked tester must first diagnose and correct only the protected Drawing settled-pixel loss inside the owner-approved two-file ceiling, then become Verified/integrated through the complete lifecycle. Its tested-process server-egress guard and tester-only exact offline-font response strategy may not permanently change `app/layout.tsx`, application styling, font selection, or unrelated visible behavior and must restore all temporary setup byte-for-byte. Credentials are also scrubbed and `NEXT_TELEMETRY_DISABLED=1`, but those are defense in depth—not egress proof.
...
| Phase 1.5 stopped Executor evidence | Failed / blocked | Read-only failure SHA matches `53d34094cff90d2864dd2e5bfdb09cb887bb60326806e8a048e13072a6d6422b`. Transient red pixels were observed, but final settlement/input usability, the complete 37-case matrix, and strict 49-artifact manifest were not reached. No valid current Phase 1.5 proof-manifest exists. |
...
| Approval/status consistency | Pass | SPEC-0001 remains Approved and active. OD-01–OD-06 and OD-09 are accepted; OD-07/08/10/11 retain their accepted safety/process boundary; OD-12–OD-14 remain deferred. Phase 1 is Verified/published/integrated. The Phase 1.5 Executor result remains Blocked/unaccepted/unpublished with no valid proof manifest; D-0012 makes only its protected-Drawing correction Approved/Authorized/Not started-resumed. Phases 2–7 remain Unauthorized/Not started. |
| Documentation/control-plane finishing gates | Pass | The seven canonical D-0012 approval/control-plane files were updated, `bash scripts/update_memory.sh` regenerated the canonical tree without changing it, its later `--check-only` passed, final diff checks passed, and the index/untracked set remained empty. `docs/DECISIONS.md` records D-0012; `AGENTS.md` and `docs/PROJECT_MANAGER_CONTEXT.md` remain unchanged because Arthur changed no PM-process rule. |
...
Final spec status is **Approved** and active. **Phase 1 is Verified, published, and integrated into canonical `main`. Phase 1.5 is Blocked; its stopped Executor result is unaccepted/unpublished with no valid proof manifest, and its protected-Drawing correction is Approved/Authorized but Not started/resumed under D-0012.** Phases 2–7 remain Unauthorized/Not started.

Exact next step: Arthur and the Project Manager review this Control Plane Architect approval packet, followed by a separate publication instruction if accepted. Only after D-0012 is published/integrated may a separately authorized Phase 1.5 correction Spec Executor take exclusive ownership of the stopped worktree, diagnose first, make the proven narrow correction, and rerun the entire proof from clean owned output. Its packet does not authorize propagation or publication. Phase 2 cannot begin until Phase 1.5 is technically Verified with a valid 49-artifact manifest, accepted, propagated by a later Control Plane Architect, separately published, integrated into canonical `main`, and Arthur separately authorizes Phase 2. The Phase 7 Policy Gate, every external lookup, and any paid/live request remain forbidden.
...
`docs/DECISIONS.md` additionally records D-0012, making seven control-plane paths in the approval-propagation diff. `AGENTS.md`, `docs/PROJECT_MANAGER_CONTEXT.md`, runtime/tester files, and `project/project_structure.txt` remain unchanged unless deterministic memory regeneration proves otherwise. No file is staged, committed, or pushed in this task.
