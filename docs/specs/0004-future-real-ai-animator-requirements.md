...
> **Current supersession boundary (D-0055).** SPEC-0004 Phases 1, 2, and timing-only Phase 2.5 remain Verified, published, and integrated. Phase 2.6 remains rejected, unpublished, and superseded historical evidence. Under D-0055, unfinished Phases 3–8 are Superseded by future SPEC-0008, inactive, and must not be implemented. The former dependency on completion of all eight SPEC-0005 phases is closed, not paused. Unfinished scope/commands/entry gates below are historical design only and cannot authorize execution or define future SPEC-0008.

> Current lifecycle update (D-0058): GIT-050 published SPEC-0006 Phase 1 activation at `3b784cc6a68ff6f10fa390d96b81376b46e54b44`; Arthur/PM accepted the exact 11-path hidden contract/read-only migration result, which is technically Verified pending GIT-051 publication/integration. Phases 2–7 remain Unauthorized; Not started. Future SPEC-0007/0008 remain absent and unauthorized. No new AI/provider/model/video/tracking, paid service, or deployment is authorized. Completed/superseded SPEC-0004/0005 statuses do not change.
...
**Foundation entry gate — closed.** D-0055 supersedes this phase. Completing another spec cannot reopen it; no executor may start under this historical plan.
...
**Scope.** Build the versioned parameterized $0 catalog for wave, jump, walk, run, punch, kick, turn, crouch, and nod; broad tested casual wording, safe defaults, negation, ambiguity handling, confidence decision, and exact free-versus-needs-Terra disposition. Every accepted complete recipe must emit one complete bounded ordered `stick.movement-goal/v1` sequence for the integrated SPEC-0005 engine before local materialization, preserving base pose, landmark facing/contact, explicit simultaneous/sequential parts, handoffs, and completion. It may not emit raw joint coordinates, flatten the sequence, or bypass the shared pose/mechanics/path pipeline. Preserve all current Phase 6 wave and approved typo behavior.

**Entry gate.** All eight SPEC-0005 phases and this spec's Phase 3 are integrated; every catalog parameter, semantic movement field, and timing/path choice has an explicit supported range and safe response. Parameters incompatible with the actual one-layer/multi-figure limit are rejected/deferred, not quietly approximated. Recognition of “robotic,” “mechanical,” “stepped,” “constant pace,” or equivalent wording belongs here, not SPEC-0005, and may emit `mechanical_robotic` or `mechanical_stepped` only on a complete safe match; when the preserved Phase 2.5 timing sidecar is used, its corresponding historical `motionIntent` remains `mechanical_explicit`. Free recipes may not bypass the shared SPEC-0005 planner door or engine with recipe-specific executors.
...
**Visible review/proof.** Offline structured-output, timing-contract validation, redaction, invalid/refusal/timeout/stale/over-budget/no-op, route isolation, and network-denial proof pass first. Natural-language recognition of robotic/mechanical/stepped/constant-pace wording belongs to this Terra phase and may emit only the bounded SPEC-0005 `mechanical_robotic` or `mechanical_stepped` style, with the preserved Phase 2.5 sidecar's historical `mechanical_explicit` intent when applicable. A paid real call, if later explicitly authorized, is separate from this phase's ordinary offline proof and must obey the recorded one-use/budget policy. No follow-up AI editing.
...
Next step is separate GIT-051 publication/integration of accepted SPEC-0006 Phase 1. Phase 2 remains unauthorized. This historical spec grants no implementation authority; follow the approved SPEC-0006 only.
