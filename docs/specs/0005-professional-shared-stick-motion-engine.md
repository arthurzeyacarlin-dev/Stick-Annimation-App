...
Status: **Approved. Phase 1 is Verified, published, and integrated. The first former Phase 2 implementation was visually rejected and is closed without acceptance/publication. GIT-041 published the eight-phase restructure at `46b97556ec4e9c9249a1ff354c546f7b1c32ea4d`. The current Phase 2 body-safety implementation exists only as a stopped, unpublished, publication-blocked eight-path result; its correction is not started and is not accepted, Verified, or published. Phases 3–8 are Unauthorized; Not started.**

Owner: Arthur
Spec role: Spec Architect
Created: 2026-09-04
Last updated: 2026-09-05
Decision links: [D-0043 through D-0047](../DECISIONS.md)
...
The distinct restructured Phase 2 executor later implemented the body-safety foundation from published GIT-041 base `46b97556ec4e9c9249a1ff354c546f7b1c32ea4d`. Its stopped `/7dfc/` bytes and `ea7ab9c5…` manifest are preserved, but the PM publication review found two specification defects: Phase 3 could not obtain checked important poses before baking, and Phase 4 was required to support airborne mechanics without authority to extend the safety module that currently rejects airborne frames. D-0047 corrects those contracts. This is not a claim that the current result is accepted, Verified, published, or ready to resume without a separately authorized correction executor.
...
The Phase 2 safety kernel is planner-independent and action-name-independent. It receives only fixed topology/neutral metrics, semantic limb/posture/support intent, candidate poses, final baked frames, and declared continuity/path/contact context. Planner identity, prompt text, action label, fixture ID, materializer name, or a caller-supplied “trusted” flag is neither an input to nor an exception from safety. The current built-in humanoid has no unusual-motion escape hatch. Deliberate hyperextension, reversed bends, or unusual left/right anatomy require a future separate custom/unusual-body spec and contract that explicitly chooses performer-anatomical versus viewer/facing left/right semantics and defines the permitted ranges, display, and safety ownership. SPEC-0005 may not silently authorize or prebuild that override.
...
**Execution path.** Neutral built-in body + semantic chain intent + enumerated two-bone candidates → checked immutable important-pose selection → caller builds provisional local technical in-betweens from exactly those returned poses → final rounded frames after any repair → final completion revalidates binding, the selected poses, every rounded/post-repair frame, and whole-animation semantic continuity → one safety-qualified final candidate or fail-closed result. Phase 2 implements and proves the safety-owned steps and their integration seam; Phase 3 supplies the first actual movement-goal pose maker.
...
**Implementation boundary and two-stage public safety API.** `stickFigureBodySafety.ts` owns all §3.2–3.3 constants and calculations, two-circle IK enumeration, hard-candidate validation, deterministic full-sequence selection, final-frame validation, and final-animation validation. It may export data types/constants plus exactly two callable safety operations:

1. `selectStickBodySafetyImportantPoses` accepts a strict `stick.body-safety-selection/v1` request and the captured starter document. The request contains the exact `projectId`, `transactionId`, `baseDocumentRevision`, and `baseDocumentDigest` binding; neutral metrics; semantic chain/support/posture context; ordered important-pose candidate sets; and bounded animation intent. It contains **no `finalFrames` and no `candidateDocument`**. It independently recomputes the starter/base binding, validates every candidate, selects the canonical full important-pose sequence, and returns a deep-cloned/deep-frozen checked selection containing that same binding, the canonical request digest, the selected unrounded poses and their semantic/frame/branch context, qualification evidence, and a canonical selection digest. It returns no final document and cannot expose Preview. The straight-neutral-knee stationary exception is selection-qualified only from a bounded first/last rest endpoint or an important-pose interval whose selected poses are geometrically identical and whose rest semantics are independently checked; there is no trusted `stationary` flag. Finalization must independently re-evaluate the exception against all baked final frames.
2. `finalizeStickBodySafetyCandidate` is the only final body-safety completion operation. A strict `stick.body-safety-completion/v1` request carries the original selection request, the exact returned checked selection, final rounded/post-repair frames, candidate document, and animation intent. The finalizer recomputes the base binding and selection-request digest, reruns candidate validation and deterministic selection from the original request, compares the canonical selected bytes/context/digest, verifies that every important final frame derives from that checked selection, validates every rounded/post-repair frame, revalidates whole-animation semantics/recovery/continuity/loop behavior, and only then returns one cloned/frozen safety-qualified final candidate.

The binding and digests are comparison evidence, never trusted capabilities. There is no caller-asserted checked token, boolean `skip`, `trusted`, planner, action, fixture, materializer, custom-rig override, or selectable legacy bypass. Helpers that enumerate, solve, validate, or return unchecked geometry stay module-private. The closed failure reasons are `non_finite_geometry`, `segment_length`, `stage_bounds`, `elbow_bend`, `knee_bend`, `branch_singularity`, `branch_flip`, `torso_head`, `body_crossing`, `clearance`, `support_contact`, `continuity`, `overshoot`, `loop_snap`, `unrequested_motion`, `recovery`, `balance`, `semantic_continuity`, `no_safe_sequence`, and `integration_bypass`; a failure also identifies `important_pose`, `final_frame`, `final_animation`, or `integration`, plus the applicable frame/transition/roles. When several rules fail, evaluation follows §3.3 rule order, then frame/transition order, then anatomical role order, so the reported failure is deterministic.

`stickFigureMotionEngine.ts` may expose exactly one SPEC-0005 final candidate door, and it can return only a successful finalization result from the API above. `stickFigureCommandExecutor.ts` must internally route a SPEC-0005 candidate through that door and must reject caller-selected materializers, including current SPEC-0004 compatibility literals, for any movement-goal/body-safety candidate. Neither a selection result nor unchecked/private helper output may reach Preview. The published SPEC-0004 compatibility entrypoints remain frozen for their historical routes only.
...
**Entry gate.** Phase 1 remains accepted/published at `2436a9414221e8ee7ef40151284cb8f4e069e828`; GIT-041 published D-0046 at `46b97556ec4e9c9249a1ff354c546f7b1c32ea4d`. The distinct current `/7dfc/` executor is completely stopped at detached `46b97556…`, with empty index, exactly the eight authorized Phase 2 paths, and preserved manifest `ea7ab9c5a142843b109ad49924f4709660347265099740312a666eca07972cd4` (17,826 bytes). Those bytes are implemented-but-publication-blocked and remain read-only until this D-0047 correction is independently reviewed, published, and a later instruction explicitly authorizes the same-worktree correction executor. The old rejected pose-maker remains removed and unavailable.

**Proof independence.** The fixture catalog and `validateStickBodySafety.ts` form the independent/body-rule oracle and are authored before or separately from the runtime route. The oracle may import only the stable project contract/topology primitives, not `stickFigureBodySafety.ts`, `stickFigureMotionEngine.ts`, or the executor. It independently recomputes angles, lengths, crossings, semantic distances, continuity, and expected pass/fail reasons. Runtime property/metamorphic tests preserve the floor of at least 10,000 fixed-seed valid/invalid candidates **plus** at least 10,000 mirrored counterparts; exact engine-produced coordinates may be recorded only as secondary regression evidence. `validateStickBodySafetyIntegration.ts` must execute the actual order `select checked important poses → build provisional local technical in-betweens from the returned poses → finalize/revalidate → final candidate door`, prove transaction fail-closed behavior, and source/AST-prohibit planner/action/fixture/trust/skip/materializer bypass. The provisional builder is test-local technical fixture logic only: it is not the Phase 3 pose maker, new application motion, duplicate IK/selection logic, or a product export.

**Required positive/property cases.** Neutral straight-knee rest endpoint transitioning into one selected safe flexion branch; a wholly stationary neutral straight-knee interval; safely flexed mirrored arms and legs; both safe IK branches considered with the continuity-consistent branch selected; relaxed unspecified limbs staying near transported neutral; monotonically recovering poses; conservative double- and single-support balance floors; small directional proportional balance-limb response; exact segment preservation before/after rounding; safe final frames surviving the same result after a no-op contact-repair pass; deterministic byte-identical sequence choice; safe left/right translated placements; role-swapped mirrors; grounded rise/lower with fixed contact; and a no-safe-candidate failure with no Preview candidate. The translation, direction, mirror, rise, and lower cases prove only 2D placement/body safety. They do not prove travel, locomotion, takeoff, airborne motion, landing, or natural-action quality.

**Required negative cases.** The exact rejected reach alternative-branch issue; impossible reach; zero-length and sub-two-pixel rounded segments; stretched segment; elbow branch flip; knee branch flip; singular straight-through elbow/knee flip; moving-knee near-lock, dead-corridor bend, or inversion; head/torso inversion, kink, or snap; unsafe in-between with individually safe endpoints; unsafe geometry introduced only by integer rounding; unsafe geometry introduced only by contact repair; `recover` moving away from rest; W arms from unspecified limbs; large unsolicited hip lowering/body compression; unsafe hand/body/head crossing; unsupported/false contact or foot slide/lift; unbalanced grounded pose; large unexplained joint jump; path overshoot; declared-loop last-to-first teleport; invalid mirror asymmetry; unreachable target/no safe candidate; and planner identity, action label, fixture ID, prompt text, alternate materializer, or forged trust metadata attempting to bypass the same rejection. Named integration negatives additionally include tampered checked-selection bytes/digest, stale project/transaction/revision/document binding, a guessed or selection-mismatched candidate document, an unsafe rounded frame, an unsafe post-repair frame, selection-result-to-Preview exposure, any exported unchecked helper, caller-selected legacy materialization, and every trusted/skip/action/fixture shortcut. Each must fail at the named important-pose/final-frame/final-animation/integration stage with no Preview and no document/history/storage/latch mutation.

**Required inherited proof.** On the live Phase 2 dirty tree, run `scripts/validateStickMotionQualityBaseline.ts`, `scripts/validateStickFigureAiContracts.ts`, `scripts/validateStickFigureCommandTransaction.ts`, `scripts/validateStickFigureMotionEngine.ts`, `scripts/validateStickFigureActionTiming.ts`, `scripts/validateStickFigureAiUiAdapter.ts`, `scripts/validateStickHistoryPersistence.ts`, and `scripts/validateStickPoseTimeline.ts`. These current focused validators must preserve the published SPEC-0005 Phase 1 reference and accepted SPEC-0004 Phase 1/2/2.5 outputs, defaults, timing formulas, editability, transaction/history/storage/onion/Creator behavior, and zero-egress boundary. Historical proof manifests and successful receipts remain immutable audit records with their published hashes; do not claim to rerun their activation-HEAD/old-dirty-set/live-process checks against new approved source extensions. Run the permanent browser tester in a tester-owned isolated temporary clean clone of the exact D-0047 correction-publication base, then clean that clone/process up, to protect unrelated app behavior. State explicitly that current Phase 2 runtime coverage comes from the dirty-tree safety/integration validators because no normal app route invokes it yet. This permanent test is separate from the one later ordinary `/7dfc/` smoke-only root. Legacy compatibility revalidation is not permission for any future planner to use a legacy materializer.

**Review decision.** Phase 2 is technical-only because it intentionally produces no new user-visible action and changes no normal app presentation. After every corrected technical gate passes, retain or revalidate exactly one ordinary regression-review instance directly from the exact unpublished executor worktree on a loopback-only non-`3000` root URL. The currently preserved `http://127.0.0.1:57795/` process is read-only historical state during this specification task. A later explicitly authorized same-worktree correction may verify and reuse it; if it has exited or cannot represent corrected source, that later executor may verify/stop the exact old process and then start **one** replacement in the same `/7dfc/` worktree with new manifest binding. Old and replacement processes may never overlap, and no second app copy is permitted. Do not duplicate or inject source, preload a new action/project, add a blue box, picker, overlay, test control, fixture, query/hash flag, special route, public asset, or permanent review UI. Arthur's smoke check answers only whether the existing ordinary application still looks and works normally; because no normal route invokes the new safety seam, it is not evidence that Phase 2 safety works and it is not a new-motion acceptance gate. Arthur reviews the first new safety-gated whole-body motion in Phase 3: reach-high-right, centered compression, left support shift, and neutral recovery, each through two full loops and important-pose scrubbing.
...
**Proof.** One new independently validated **correction** manifest binds the exact corrected base/HEAD/index/allowlist, source/fixture hashes, independent-oracle provenance, fixed property seeds/case counts, two-stage API order, pass/fail reason matrix, source/AST bypass scan, transaction/legacy regression receipts, TypeScript/scoped lint/full-lint non-regression, both diff checks, permanent tester, zero-egress ledger, and cleanup. It must not overwrite or relabel the historical `ea7ab9c5…` manifest; that hash is an input audit anchor only and proves neither correction nor acceptance. After corrected gates pass, the new manifest also binds the one ordinary regression-review root URL/port, launch command, PID/process group, exact executor-worktree path and source identity, startup/network logs, `existing_app_regression_smoke_only` purpose, false safety/new-motion claims, and bounded cleanup instructions. The manifest cannot label visible quality or Phase 2 safety human-accepted because Phase 2 has no new visible quality claim and the ordinary app does not invoke the safety seam.
...
**Execution path.** Strict internal movement goal → binding/field validation → neutral body metrics → ordered chain solver → enumerate both two-bone limb solutions and whole-body candidates → call Phase 2 `selectStickBodySafetyImportantPoses` → bake provisional in-betweens from exactly the returned checked immutable poses with the existing published SPEC-0004 Phase 2.5 timing/baker → call Phase 2 `finalizeStickBodySafetyCandidate` to recompute/revalidate binding, selection, every rounded frame, and semantic continuity → the one motion-engine final candidate door → independent ordinary keyframes → existing transaction/Preview.
...
Ignored output: `output/spec-0005/phase-3/**`. No UI, old fixture, provider, package/config, safety-module/kernel, or control-plane edit. Phase 3 must consume the published checked-selection API as-is; it may not duplicate IK enumeration/sequence selection, export an unchecked helper, guess selected poses, or add a second finalizer/Preview door.
...
**Entry gate.** Corrected Phase 2 is technically accepted, CPA-closed, and separately published/integrated; its accepted two-stage API, exact source, independent oracle fixtures, reason matrix, current safety/property/integration validators, and canonical acceptance record must revalidate. The correction manifest proves selection-before-bake and final revalidation; the earlier `ea7ab9c5…` manifest remains historical non-acceptance evidence only. Phase 3 still requires a later explicit owner authorization after that publication.
...
**Proof.** Independent semantic input cases and expected qualitative invariants, solver unit/property checks, mirrored/metamorphic cases, mutation tests, source/AST proof that Phase 3 calls checked selection before bake and the sole finalizer afterward, four full-playback browser flows, one-frame manual edit/Undo/Redo/Save/Open, permanent tester, manifest, and Arthur review. Tampered selection, stale binding, guessed/reordered important poses, unsafe rounded output, duplicate IK/selector logic, unchecked helper access, or any alternate Preview route must fail. The solver may have deterministic coordinate snapshots as regression evidence, but they are secondary to independent semantic/physical assertions.
...
**Stop boundary / later relationship.** Stop after the four grounded micro-movements. These cases prove safe 2D placement, not travel, takeoff, airborne motion, landing, gait, or final action quality. No airborne mechanics, natural paths, gait, final actions, planner connection, or Phase 4 work. Phase 4 explicitly extends the shared safety module under its own ceiling; it does not fork, bypass, or weaken Phase 2's bend, continuity, or manual-ownership rules.
...
- the two-stage checked immutable important-pose selection then final completion/revalidation safety API, with exactly one final candidate/Preview door;
...
GIT-041 is complete at canonical `46b97556ec4e9c9249a1ff354c546f7b1c32ea4d`. The exact next lifecycle action is independent PM review of this D-0047 docs-only correction, followed by separate publication of only its reviewed control-plane paths. Phase 2 is **not ready or authorized to execute again** until that publication is cleanly verified and Arthur/PM explicitly authorize a same-worktree correction executor.

That later executor must take exclusive ownership of the preserved `/Users/arthurcarlin/.codex/worktrees/7dfc/stick-animation-app`; verify it is stopped, detached at `46b97556…`, index-empty, dirty only at the exact eight Phase 2 paths, and byte-bound to historical manifest `ea7ab9c5a142843b109ad49924f4709660347265099740312a666eca07972cd4`; verify the one preserved ordinary server at `http://127.0.0.1:57795/` without mutating it; and compare every byte before any Git-state change. Because the docs-only publication and eight Phase 2 paths do not overlap, the later authorization may permit advancing only that detached worktree basis to the new correction-publication SHA while preserving the exact eight dirty bytes. Hash all eight paths before and after and stop on any overlap, extra dirty path, staged byte, source change, or server/process mismatch. This is a future bounded reconciliation step, not authorization in this specification task.

Only after the basis is reconciled may the correction executor patch those same eight paths, create a new independently validated correction manifest without overwriting the historical one, and prove the two-stage order and all negatives above. Reuse the one existing ordinary app process if its exact source/process identity remains valid. If it exited or corrected-source rebinding requires restart, a later explicit same-worktree correction authorization may verify/stop the exact old process and then start one replacement in `/7dfc/`; never run two concurrently or create a second app copy. Return the Implementation Review Packet and stop. Do not start Phase 3, mutate/delete the preserved worktree or historical evidence, touch the rejected SPEC-0004 `/8de8/` history, contact a provider, deploy, or expand scope. A later accepted-result CPA takeover/publication follows the ordinary sequential lifecycle and must not lose the preserved `/7dfc/` bytes.
