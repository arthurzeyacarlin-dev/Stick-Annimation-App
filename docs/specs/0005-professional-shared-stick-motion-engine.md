...
D-0054 supersedes older plural-copy delivery wording only: use one active review app copy per milestone, with cases loaded sequentially and clear clickable instructions in the PM message. Never keep concurrent per-case servers/copies or add review UI to the product. After accepted propagation/publication/clean verification, preserve proof and remove the exact obsolete server/worktree/local branch. After rejection and selection of a replacement path, preserve and verify unique dirty bytes before removing the rejected server/worktree/obsolete branch, then start the next fresh executor. Follow AGENTS' exact-target and ownership checks; never remove canonical main or the active PM worktree. The existing numerical, technical, and visible acceptance gates are unchanged.
