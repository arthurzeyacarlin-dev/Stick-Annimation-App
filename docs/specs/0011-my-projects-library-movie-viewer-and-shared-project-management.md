...
Decision links: [D-0110](../DECISIONS.md), [D-0111](../DECISIONS.md), [D-0112](../DECISIONS.md), [D-0113](../DECISIONS.md), [D-0114](../DECISIONS.md), [D-0115](../DECISIONS.md), [D-0116](../DECISIONS.md), [D-0117](../DECISIONS.md), [D-0118](../DECISIONS.md)
TODO IDs: `PLAN-011`, `SPEC-011`, `LIBRARY-001`, `GIT-084`, `GIT-085`, `VIEWER-001`, `GIT-086`, `GIT-087`, `PROJECTS-001`, `GIT-088`
...
## 7. Phase 2 — Movie Viewer (Verified/published/integrated/synchronized/proof-preserved; review server stopped)
...
The executor may implement only §7 inside the Phase 2 boundary. It must satisfy every §7.1 entry gate before geometry changes, build and independently validate the §11 immutable manifest, return the full Implementation Review Packet, and stop. At this Phase 2 activation point, Phase 3 remained Unauthorized/Not started; D-0117/GIT-087 now supersede only that lifecycle state after Phase 2 fully closed.
...
The full repository build still compiles before stopping at the inherited untouched `app/dev/ai-costs/lifetime/page.tsx` `PageProps/searchParams` type failure; the focused Home plus Terra route production build passes. Physical devices, non-Chromium browsers, native OS audio/permission UI, native Finder/new encoded output and a fully hydrated 64-project full-resolution collection remain unproven. The ignored `.env.local` was not included in proof or publication, its contents were not recorded, and byte identity is not claimed. Phase 3 is not implemented or authorized. D-0116/GIT-086 publish and integrate Phase 2 exactly as accepted.

### 7.12 Phase 2 publication and preservation record — D-0116/GIT-086

GIT-086 is exact 29-path commit `faf2965cb51d47ebcd2b68c9378db8a39486915d`, parent `e6f6b1d7887bbbfaed7a9b00b8b472c5b28fe834`, message `Complete SPEC-0011 Phase 2 movie viewer`: the exact 15 accepted technical paths plus 14 reviewed control-plane/tree paths. Canonical `main`, local `origin/main` and live GitHub `main` synchronized cleanly after the normal push.
...
### 8.0 Activation record — D-0117/GIT-087

Arthur explicitly authorizes publication of this Phase 3 activation and, only after clean canonical/local-origin/live-GitHub synchronization, exactly one fresh dedicated Plan-mode Spec Executor from the exact GIT-087 SHA. The activation commit contains control-plane records only and implements no Phase 3 runtime, fixture, test, proof, package/dependency, schema, credential/environment, AI/Terra, provider, recovery, player, editor, Export or deployment byte.

The executor may implement only §8 inside the Phase 3 boundary. It must first trace the live My Projects/Open Project collection, V2 repository/storage Save/Save As, recovery metadata, open viewer/editor ownership, legacy hydration and asset-reference paths; build and independently validate the §11 immutable manifest; return the full Implementation Review Packet; and stop.
...
### 8.11 Accepted Phase 3 result — D-0118

Arthur accepted the final ordinary app result and authorized sequential Control Plane Architect propagation. The stopped executor result is exactly 14 technical paths from detached base/HEAD, local `main`, and local `origin/main` `b57ff995df9bb351dacb9a6a79b8cbd23203e988`, with an empty index. Immutable 46,868-byte `output/spec-0011/phase-3/proof-manifest.json` is PASS at SHA-256 `92ae122c227a36fd8a637b1922b1bca139224be4caf0e42d3a147f2a43147aaa`, source digest `05ee661120e7751658b9ea2e5269c7cc1cbc666645568af1db7b1ff1f08074d1`; fresh strict validation returned VALID with 18 assertions and 12 rejected mutation classes.

The accepted implementation uses one `ProjectManagementCommandOwner` on My Projects and Open Project. Search is NFC-normalized/case-insensitive and all four sorts are local. Rename is native-only and publishes one metadata-only next revision with CAS. Duplicate hydrates valid native or legacy input through the existing Save As path while leaving legacy source bytes unchanged. Delete is native-only, binds exact head/revision/digest, blocks active viewer/editor/recovery conflicts, removes only the target project/history, and removes only assets proven unreferenced while preserving shared assets. Web Locks plus localStorage heartbeat leases serialize active ownership; the editor lease follows Save/Save As identity. BroadcastChannel/storage events are advisory invalidation only and every consumer rereads authoritative storage.
