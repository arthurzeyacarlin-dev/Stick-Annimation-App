...
| [`SPEC-0013 — Version 1 Home Finalizer Removal and Shorter Home Scroll`](0013-version-1-home-finalizer-removal.md) | **Phase 1 Verified and accepted; publication authorized/in progress** | D-0135 accepts the exact seven-path result from base `8ddc534…`. Immutable manifest SHA-256 is `bf8e51c…db63`; Export is the only Tools/final Home card, the existing scrollbar remains unchanged, the stale Assistant Finalizer fact is corrected, live Terra connectivity passed and port 58080 is closed. Exact GIT-097 publication/integration/proof preservation/D-0054 cleanup is pending. No Phase 2 exists. |
