...
| [`SPEC-0012 — Diamond Animator Guidance Assistant`](0012-diamond-animator-guidance-assistant.md) | **Proposed; owner outcome and six-phase direction accepted; all implementation unauthorized/not started** | D-0120 defines the separate guidance-only Assistant: responsive shell; bounded 50-session local persistence; fixed-Terra catalog guidance; optional hosted search with sources/truthful activity; separately gated microphone dictation; and beta hardening. The planning package authorizes no executor, runtime change, live/paid call, Git publication or deployment. Phase 1 is ready only after reviewed planning publication/synchronization and Arthur's separate authorization. |
...
D-0120 creates Proposed SPEC-0012 from Arthur's accepted six-phase guidance-Assistant direction. No GIT-089 planning publication or Phase 1 authorization exists yet. SPEC-0012 is independent from paused SPEC-0008: it cannot mutate animation/project content and cannot resume any Animator phase.
