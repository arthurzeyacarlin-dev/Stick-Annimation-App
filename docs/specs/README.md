...
| [`SPEC-0012 — Diamond Animator Guidance Assistant`](0012-diamond-animator-guidance-assistant.md) | **Active; Phase 1 fully closed; combined Phase 2 accepted/technically Verified/control-plane propagated, publication pending; original Phase 3 absorbed; Phases 4–6 unauthorized/not started** | D-0124 binds the exact 34-path sessions/fixed-Terra result and Arthur's removal of cumulative daily/monthly local Assistant caps while retaining per-request/token/timeout/concurrency/dedupe/cancel/usage/local-only/no-tools safeguards. Final manifest SHA-256 `b6251390…350c`, source digest `304ef207…8d7`; GIT-091 remains a separately authorized future publication/cleanup task. |
...
D-0120 created SPEC-0012; GIT-089 published the plan. D-0121–D-0123/GIT-090 fully close Phase 1. D-0124 accepts combined Phase 2 and records that the original Phase 3 fixed-Terra scope was moved forward and satisfied there. SPEC-0012 remains independent from paused SPEC-0008 and cannot mutate animation/project content or resume any Animator phase.
