...
D-0050's first Phase 2 v2 plan is published in GIT-045 at `a4589664…`; D-0051 requires GIT-046 before one fresh safety-correction executor becomes authorized. Its independent human-authored oracle must cover every per-landmark front/left/right facing, plane/guide/band, valid mirror and through-front turn, constant and future-changing foot/hand/pelvis contacts, grounded/airborne classification, neutral/non-neutral base completion, single/multiple active roles, final rounded context, forbidden extras, and paired false-accept/false-reject mutations. Explicit injury, deformation, or unusual-motion wording never bypasses anatomy. It retains the v1 fixed-seed 10,000 cases plus 10,000 mirrors and adds a separate v2 10,000 plus 10,000 mirror corpus across those contexts. Hard failures stop before Preview; safe later-owner cases reach `unsupported_owner_phase` rather than a false anatomy rejection. The implementation ceiling remains exactly eight tracked paths plus ignored `output/spec-0005/phase-2-natural-safety-correction/**`; prior proof and rejected worktrees stay read-only.
