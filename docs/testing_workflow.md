...
The accepted D-0047 correction manifest is `output/spec-0005/phase-2/d0047-correction/proof-manifest.json`, 17,520 bytes at SHA-256 `e8a3fbbbb1a1cdf63e5d3648422def4736a4a1bf212ecc27335d2aa842db7874`; independent validation passed 168 checks. Runtime integration passed 20,305 assertions, 81 named negatives, and 10,000 generated cases plus 10,000 mirrors. The separate oracle imports no runtime safety/engine/executor modules and passed its own 10,000 cases plus 10,000 mirrors and 68 negatives. TypeScript, scoped lint, full-lint non-regression (5 errors/72 warnings with zero changed-path findings), inherited validators, diff/scope checks, and the isolated permanent tester passed. The tester recorded 40 operations and 13 screenshots with zero real API/non-loopback/provider attempts and complete cleanup. The earlier screenshot-drift attempt is preserved only as unproven history. Arthur's ordinary-app smoke is separate human evidence and does not prove safety or motion. The manifest-recorded process was stopped and port `57795` was closed.
...
Phase 3 is authorized by D-0049 only for the existing grounded non-locomotion whole-body pose contract and remains Not started until GIT-044 establishes its exact fresh Plan-mode executor base. Phases 4–8 remain unauthorized and later add mechanics/weight/contact, paths/timing/gravity, locomotion, semantic actions, and the shared provider-free planner door in order. At boot, each visible phase verifies that its clean canonical base contains the preceding accepted source/control-plane anchors and recorded manifest hashes; after edits, it reruns inherited current focused suites and keeps complete ordinary independent editable frames plus all protected transaction/editor behaviors. It does not claim to rerun an old manifest validator whose HEAD/dirty/live-process contract belongs to another stopped worktree. A failed visible review rejects the phase even when all technical commands pass.
