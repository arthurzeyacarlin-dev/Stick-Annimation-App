...
## Approved drawing-only SPEC-0007 proof profile — corrected Phase 1 authorized after rejected-result cleanup

[`SPEC-0007`](specs/0007-manual-editor-completion-and-ai-ready-tools.md) is Approved/active under D-0074. Corrected Phase 1 is Authorized/Not started; Phases 2–5 remain unauthorized. Each of the five phases requires a fresh exact-base Plan-mode executor worktree, deterministic command/contract tests, desktop `1440×900` DPR1 and compact `390×844` DPR2 real-browser flows, relevant 200% zoom/reduced-motion/accessibility checks, performance/memory/size receipts, Save/Open/Undo/Redo/failure proof, protected SPEC-0006 regressions, zero unexpected external/real API/provider requests, an independently mutation-tested technical manifest, exact scope/diff checks, and an empty index.
...
The destructive-command matrix must cover current Eraser, Knife removal, Delete selection, Remove Frame/minimum guard, confirmed Delete Layer, confirmed Clear Canvas, Delete symbol instance, unreferenced Delete symbol definition plus referenced rejection, and Remove Attached Sound. No current asset-catalog delete control is claimed; Phase 4 may add explicit deletion of an unreferenced catalog entry. Painting, Fill, and transforms are targeted mutations rather than delete commands and must prove unrelated retention.
