...
### Decision-complete SPEC-0004 Phase 2.6 proof boundary

D-0041 fixes the final shared `stick.action-foundation/v1` phase contract; there is no Phase 2.7. GIT-037 has published/integrated the exact accepted Phase 2.5 seven paths and reviewed control-plane/tree paths. D-0042 records Phase 2.6 as decision-complete/Not started and awaiting Arthur's separate implementation authorization. Once authorized, one fresh Plan-mode Phase 2.6 executor owns only `src/lib/ai/stickFigureActionFoundation.ts`, the two shared motion/executor runtime files, the one `v4` fixture, and the four Phase 2.6 proof/validator files listed in SPEC-0004. Every other runtime/UI/fixture/proof/control-plane path is read-only.

The focused and browser proof must cover exact 12-FPS wave, jump/hop, bow, dodge, walk, and run samples plus slow-motion hop and general `reach_up`; action-pure ordered beats; planted/contact/support/weight/balance/landing recovery; no unrequested prelude or extra action; and required negatives including wave-with-clap/hop, jump-with-wave, and hop-with-head-nod/shake while permitting mechanically tied balance motion. Both pre-bake and post-bake validation fail closed before Preview. Every successful output slot remains an independent ordinary editable keyframe with exact no-op/Apply/Undo/Redo/Save/Open/manual-edit ownership.
...
Near the end, after current Phase 2.6 runtime/browser proof is green, `npm run test:spec0001-browser` must pass before any link reaches Arthur. Human review then uses exactly six ordinary unpublished non-3000 copies with no blue box, query/hash flag, picker, overlay, special control, product route/import/asset, or provider/API/external request. Phase 2.6 adds no normal chat/Pretend-AI/Terra routing, UI, background, layer, color, custom rig, multiple figure, post-Apply work, deployment, or publication.
