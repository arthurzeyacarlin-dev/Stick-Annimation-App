...
## Current SPEC-0006 Phase 7 accepted evidence

Phase 7 is Arthur-accepted and technically Verified under D-0069 pending GIT-061 publication. The stopped executor produced exactly 11 technical paths from GIT-060 base/HEAD `cbe16411a0f83d3b86136f41d0a66d1874d009aa` with an empty index. Immutable manifest `output/spec-0006/phase-7/proof-manifest.json` is 50,224 bytes at SHA-256 `c6202f6deab4d3bcc75af3ad6699fc2dc70fc58723eccaa8337b1a2aef74bc51`, PASS/VALID with 11 source and 16 evidence bindings. Arthur accepted the ordinary app at port 56770; D-0069 records that later lifecycle fact without rewriting the executor snapshot.
...
The strict Phase 7 validator is run before CPA record edits because it requires the exact 11-path executor dirty set. After propagation, CPA must instead verify the immutable manifest SHA/size and all 11 source plus 16 evidence bindings directly, then prove the exact 25-path publication-ready set and empty index. Listener PID 34120 at port 56770 and the accepted worktree remain preserved. Do not regenerate or rewrite the manifest/finalization merely to change `humanAcceptance: pending Arthur` or `controlPlaneUpdated: false`; D-0069 is the canonical later lifecycle record.
