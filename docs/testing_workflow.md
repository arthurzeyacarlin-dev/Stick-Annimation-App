...
Phase 2 implements only the planner-independent `humanoid-11-v1` Body Safety Gate/Foundation and its non-bypassable motion-engine/executor seam. The safety module is the sole new SPEC-0005 finalizer. Existing SPEC-0004 `phase-1-holds`, `phase-2-baked-motion`, and `phase-2.5-timed-motion` materializers remain frozen compatibility paths and may not become planner doors. The executor must prove the exact movement goal → enumerate candidates → important-pose safety → mechanics/required insertion → path/timing bake → post-rounding/post-contact safety → final semantic/continuity → Preview ordering at the available Phase 2 seam and fail closed when required later-stage data is absent.

Its independent oracle may not import `stickFigureBodySafety.ts`, `stickFigureMotionEngine.ts`, or `stickFigureCommandExecutor.ts`. With a fixed recorded seed, it must generate at least 10,000 valid/invalid/mirrored property cases against the spec's exact coordinate, topology, bend, torso/head, crossing/clearance, support/contact, continuity, loop, unsolicited-motion, recovery, and balance rules. Source/AST integration checks must reject direct Preview exposure, missing important-pose validation, missing post-rounding validation, a planner-selected validation bypass, or legacy materializer reuse as a planner door.

Required explicit negative evidence includes impossible reach, zero-length and sub-two-pixel rounded segments, elbow and moving-knee near-locks, forbidden knee dead-corridor bends, bend-sign flips through tangent, head/torso inversion or over-rotation, hand-through-head/torso, unsupported or underground contact, foot slide/lift, unsafe inter-frame displacement/rotation, overshoot, loop snap, unsolicited W-arms or hip drop, nonconverging recovery, unbalanced grounded frames, invalid mirror asymmetry, and bypass attempts. Phase 2's manifest records new-motion/full-playback human review as not applicable. After technical gates pass, it additionally binds exactly one ordinary root URL/port, launch command, PID/process group, exact executor-worktree path, base/HEAD/index/eight-path dirty source identity and hashes/sizes, startup/network logs, purpose `existing_app_regression_smoke_only`, explicit `phase2SafetyDemonstrated: false` and `newMotionReviewed: false`, and cleanup instructions. It must reject a second review process/copy, fixture injection, a preloaded new action/project, query/hash flag, blue box, picker, overlay, tester control, special route/asset/import, or any product/UI source change for review. Arthur's smoke result concerns only existing app behavior and is not safety evidence.

Inherited regression receipts come from the current live-tree focused validators: `validateStickMotionQualityBaseline.ts`, `validateStickFigureAiContracts.ts`, `validateStickFigureCommandTransaction.ts`, `validateStickFigureMotionEngine.ts`, `validateStickFigureActionTiming.ts`, `validateStickFigureAiUiAdapter.ts`, `validateStickHistoryPersistence.ts`, and `validateStickPoseTimeline.ts`. They protect SPEC-0005 Phase 1, SPEC-0004 Phases 1/2/2.5, Preview/Cancel/failure no-op, one atomic Apply, manual editability, Undo/Redo, Save/Open, onion, Creator, one-shot/no-post-Apply behavior, and zero provider/API/external traffic. Historical phase manifests/receipts retain their published hashes as immutable audit records; their activation-HEAD, former exact dirty-set, and removed live-process checks are not rerun or claimed current. The permanent tester runs in an isolated clean clone of the D-0046 publication base for unrelated app regression, while the new Phase 2 safety/integration validators cover the approved dirty runtime because no normal app route invokes it yet.
