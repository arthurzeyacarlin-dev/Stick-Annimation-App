...
D-0050 authorizes one fresh Phase 2 v2 safety-correction executor after GIT-045. Its independent oracle must add performer-facing projection, front/profile limb-plane resolution, joint-guide half-planes, every named safe flexion band and preferred range, backward-bend and sudden-flip negatives, the exact controlled projected-sign-transition rule, forbidden-extra-movement cases, and final rounded-sequence rejection. It retains the v1 fixed-seed 10,000 generated cases plus 10,000 mirrors and adds a separate v2 10,000 plus 10,000 mirror corpus. Hard failures must stop before Preview. The implementation/integration ceiling is exactly the eight tracked paths and ignored `output/spec-0005/phase-2-natural-safety-correction/**` root named in the spec; prior Phase 2 proof files stay read-only, and no rejected Phase 3 byte or worktree may be used.

Corrected Phase 3 must use the independent human-written `SPEC0005-HUMANOID-NATURALNESS-v1` quality/mutation suite. Hard safety and schema gates pass at 100%. The visible rubric weights prompt match 25, comfort 15, line of action 15, balance 15, silhouette 15, continuity 10, and minimum unrequested movement 5; every reviewed probe must score at least 85, the set mean at least 90, and Arthur must accept every probe. The named suite covers wave, walk, run, run-then-wave, punch, jump, mirrors, near-limit poses, and extra-action mutations. Phase 3 can satisfy only its bounded grounded pose probes; full walk/run and full action sequences remain Phase 6 and Phase 7 gates. Sparse landmarks/guides are sampled evidence and steering constraints, never automatic stops or easing boundaries.

Phase 4's future proof ceiling is exactly twelve paths, including the safety module and both current safety validators. Phase 2 continues to reject airborne. Phase 4 selection may accept airborne only from the complete ordered important-pose/mechanics sequence; completion rechecks the full baked sequence. Both require grounded anticipation/takeoff, no planted anchor during flight, contact/compression/recovery afterward, no penetration, and all unchanged bend/continuity/semantic/manual-ownership rules. A caller airborne/trust/skip flag is always a bypass failure.

GIT-044 published the former D-0049 Phase 3 authorization, but Arthur rejected that executor's visible result; technical green status did not satisfy natural-motion acceptance, and none of its bytes may be reused. Corrected Phase 3 is decision-complete but unauthorized until Phase 2 v2 is accepted, control-plane closed, separately published/integrated, and Arthur explicitly authorizes the next executor. Phases 4–8 remain unauthorized and later add mechanics/weight/contact, paths/timing/gravity, locomotion, semantic actions, and the shared provider-free planner door in order. At boot, each visible phase verifies that its clean canonical base contains the preceding accepted source/control-plane anchors and recorded manifest hashes; after edits, it reruns inherited current focused suites and keeps complete ordinary independent editable frames plus all protected transaction/editor behaviors. A failed visible review rejects the phase even when all technical commands pass.
