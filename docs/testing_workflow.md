...
Official/recovery storage hashes were identical before/after the desktop library/viewer, Export shared-player and changed-source retry flows. Proof recorded zero external/provider/AI/paid requests, zero credit changes, zero official/recovery writes, zero page errors and zero console errors. Export Phase 1/2 oracles and recovery Phase 1/2/3 contracts passed. The inherited SPEC-0009 browser replay reaches the same recorded frame-3 mismatch; it remains a Phase 2 reproduce/reconcile gate, not a Phase 1 regression. Export Phase 3 reached its substantive checks and then stopped only at its historical exact-dirty-allowlist gate because the current Phase 1 paths are intentionally outside that older allowlist.
...
Every later authorized phase must bind an immutable ignored manifest to its exact base and dirty allowlist; source/evidence hashes; before/after official repository, recovery, history/workspace and Export digests; deterministic fixtures; fault/two-tab concurrency; zero network/provider/AI/credit activity; required viewport/keyboard/accessibility/fullscreen results; performance/resource ceilings; focused type/lint/build/diff/index checks; protected regressions; and negative manifest mutations. Executor flags remain `humanAcceptance: pending Arthur`, `controlPlaneUpdated: false`, and `gitPublication: false`. The complete fixture matrix, ceilings, acceptance flows, and stop gates live in [`SPEC-0011`](specs/0011-my-projects-library-movie-viewer-and-shared-project-management.md).
