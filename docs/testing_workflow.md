...
### Authorized SPEC-0004 Phase 2 proof boundary

D-0037 authorizes Phase 2 only after its docs activation is published. The executor owns exactly the eight paths listed in SPEC-0004 plus ignored `output/spec-0004/phase-2/**`. It must preserve the unchanged Phase 1 `v1` plans, default materializer/transaction behavior, normal Stick chat/UI, and all Drawing paths.
...
The recorder writes `output/spec-0004/phase-2/proof-manifest.json`; the independent validator recomputes exact hashes/sizes/base/HEAD/branch/dirty ceiling/empty index and rejects any one-byte/count/path/status/base/command mutation. The executor returns its Implementation Review Packet and stops before docs, stage, commit, or push. Arthur/PM review, Control Plane Architect closeout, and later publication remain separate.
