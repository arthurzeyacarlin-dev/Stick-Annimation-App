...
## SPEC-0009 later-phase proof contract

D-0097 authorizes Phase 2. It must prove one local supported-browser Mediabunny-assembled H.264/AAC MP4 encode, 720p/1080p, inherited saved FPS/duration, persisted project-owned background truth with white default, deterministic audio sync, truthful progress/cancel/failure, Finder location/partial-file semantics, post-write media inspection, no silent omissions and zero project mutation. Phase 3 remains unauthorized and must refresh mutable platform facts from official first-party sources before proving every versioned destination preset through the same local encode/write/inspect path, complete-animation contain/no-crop framing, stale-catalog handling, brand-asset provenance or neutral fallback, long/performance/accessibility gates and zero network/AI/credit behavior.
