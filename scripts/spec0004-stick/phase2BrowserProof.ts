...
const OUTPUT_ROOT = resolve(ROOT, "output/spec-0004/phase-2/browser");
...
}>(resolve(ROOT, "scripts/fixtures/spec0004-stick/v2/phase2-motion-cases.json"));
...
type ReviewWindow = Window & {
  __spec0004Phase2PortsV1?: ReviewPorts;
  __spec0004Phase2PrePreviewCheckpoint?: Record<string, unknown>;
};
...
const patchWorkspaceForSeparatePhase2Port = (copy: string) => {
  ...
  source = source.replace(
    ...
    "  const spec0004Phase2MaterializerRef = useRef(false);\n\n" + PHASE4_MACHINE_DECLARATION,
  );
  ...
  source = source.replace(PHASE4_CONSTRUCTOR, [
    "    const machine = new StickFigureCommandTransactionV1(",
    "      await createStickCommandWorkspaceRoot(parsed.starter, `${visible.workspaceInstanceId}:phase4`, visible.workspaceGeneration),",
    "      spec0004Phase2MaterializerRef.current ? {animationPlanMaterializer: \"phase-2-baked-motion\"} : undefined,",
    "    );",
  ].join("\n"));
  assert.ok(source.includes(WORKSPACE_ANCHOR), "review workspace injection anchor must exist exactly");
  const injection = [
    "  const spec0004Phase2PortsV1 = {",
    "    ...spec0004Phase1BrowserPortsV1,",
    "    previewFixture: async (fixture: unknown) => {",
    "      spec0004Phase2MaterializerRef.current = true;",
    "      try { return await previewStickCommand(fixture); }",
    "      finally { spec0004Phase2MaterializerRef.current = false; }",
    "    },",
    "  };",
    "  if (typeof window !== \"undefined\") {",
    "    (window as unknown as {__spec0004Phase2PortsV1?: unknown}).__spec0004Phase2PortsV1 = spec0004Phase2PortsV1;",
    "  }",
    "",
  ].join("\n");
...
const patchAiPanelForNormalAutoPreview = (copy: string, fixture: FixtureName) => {
  ...
  const fixtureValue = {starter, envelope: plans[fixture]};
  const helper = [
    timeout,
    `const SPEC0004_PHASE2_REVIEW_FIXTURE = ${JSON.stringify(fixtureValue)} as unknown as {starter: unknown; envelope: StickCommandBatchV1};`,
    "type Spec0004Phase2ReviewPorts = {",
    ...
    "  readCheckpoint: () => Promise<Record<string, unknown>>;",
    "};",
    "const spec0004Phase2ReviewPorts = () => (window as unknown as {__spec0004Phase2PortsV1?: Spec0004Phase2ReviewPorts}).__spec0004Phase2PortsV1 ?? null;",
  ].join("\n");
  ...
  const autoPreviewEffect = [
    cleanupEffect,
    "",
    "  const phase2ReviewStartedRef = useRef(false);",
    "  useEffect(() => {",
    "    let cancelled = false;",
    "    const prepare = async () => {",
    "      for (let attempt = 0; attempt < 240 && !cancelled && !phase2ReviewStartedRef.current; attempt += 1) {",
    "        const ports = spec0004Phase2ReviewPorts();",
    ...
    "        if (ports && binding) {",
    "          phase2ReviewStartedRef.current = true;",
    "          const before = await ports.readCheckpoint();",
    "          (window as unknown as {__spec0004Phase2PrePreviewCheckpoint?: Record<string, unknown>}).__spec0004Phase2PrePreviewCheckpoint = before;",
    "          const outcome = await ports.previewFixture(SPEC0004_PHASE2_REVIEW_FIXTURE);",
    "          if (!cancelled && outcome.accepted && outcome.outcomeCode === \"previewed\") {",
    "            setHasSentAttempt(true);",
    "            setPreview({envelope: SPEC0004_PHASE2_REVIEW_FIXTURE.envelope, binding, phase2Review: true});",
  ...
  source = source.replace(
    ...
    "    const outcome = preview.phase2Review\n      ? await spec0004Phase2ReviewPorts()!.cancelFixture(SPEC0004_PHASE2_REVIEW_FIXTURE)\n      : await adapter.cancel(preview.envelope);",
  );
  source = source.replace(
    "    const outcome = await adapter.apply(preview.binding, preview.envelope);",
    "    const outcome = preview.phase2Review\n      ? await spec0004Phase2ReviewPorts()!.applyFixture(SPEC0004_PHASE2_REVIEW_FIXTURE)\n      : await adapter.apply(preview.binding, preview.envelope);",
  );
...
const makeIsolatedCopy = (fixture: FixtureName) => {
  const copy = mkdtempSync(resolve(tmpdir(), `diamond-spec0004-phase2-${fixture}-review-`));
  cpSync(ROOT, copy, {
    recursive: true,
    filter: (source) => {
  ...
  patchWorkspaceForSeparatePhase2Port(copy);
...
const stopReviewServer = async (record: ServerRecord) => {
  ...
  if (existsSync(record.copy) && record.copy.startsWith(resolve(tmpdir(), `diamond-spec0004-phase2-${record.fixture}-review-`))) {
...
const readPorts = (page: Page) => page.evaluate(() => {
  const ports = (window as unknown as ReviewWindow).__spec0004Phase2PortsV1;
  if (!ports) throw new Error("Phase 2 review ports are unavailable.");
  return true;
});
...
const prePreviewCheckpoint = (page: Page) => page.evaluate(() => {
  const value = (window as unknown as ReviewWindow).__spec0004Phase2PrePreviewCheckpoint;
...
const captureEditableFrames = async (page: Page, frameCount: number) => {
  ...
  for (let frameIndex = 0; frameIndex < frameCount; frameIndex += 1) {
    ...
    assert.equal(points.length, motionCases.jointRoleOrder.length, `frame ${frameIndex + 1} exposes every ordinary joint`);
    frames.push(points);
  }
  return frames;
};
...
const runFixtureBrowserFlow = async (record: ServerRecord, viewport: {name: string; width: number; height: number}) => {
  const profile = mkdtempSync(resolve(tmpdir(), `spec0004-phase2-${record.fixture}-profile-`));
  ...
  const expected = motionCases.validCases.find((testCase) => testCase.name === record.fixture);
  assert.ok(expected, `${record.fixture} golden case must exist`);
  ...
  try {
    ...
    assert.equal(await digestCanonical(candidate), expected.motionCandidateDigest, `${record.fixture} Preview candidate matches frozen Phase 2 bytes`);
    assert.equal(candidate.layers.length, 1, `${record.fixture} candidate has one layer`);
    assert.equal(candidate.layers[0].cells.length, 12, `${record.fixture} candidate has twelve slots`);
    assert.ok(candidate.layers[0].cells.every((cell) => cell.cellType === "keyframe"), `${record.fixture} candidate is fully baked`);
    assert.equal(new Set(candidate.layers[0].cells.map((cell) => cell.frameId)).size, 12, `${record.fixture} candidate cells have unique IDs`);
    ...
    assert.equal(await timelineCells.count(), 12, `${record.fixture} timeline visibly contains twelve slots`);
    ...
    const appliedFrames = await captureEditableFrames(page, 12);
    ...
    const redoneFrames = await captureEditableFrames(page, 12);
    ...
    await page.waitForFunction(async () => (await (window as unknown as ReviewWindow).__spec0004Phase2PortsV1!.readCheckpoint()).onionEnabled === true);
    assert.ok(await page.locator("[data-onion-side]").count() > 0, `${record.fixture} onion rendering remains active`);
    await page.getByRole("button", {name: "Onion", exact: true}).click();
    await page.getByRole("button", {name: "Play", exact: true}).click();
    await page.waitForFunction(async () => (await (window as unknown as ReviewWindow).__spec0004Phase2PortsV1!.readCheckpoint()).playbackState === "playing");
    await page.getByRole("button", {name: "Pause", exact: true}).click();
    await page.waitForFunction(async () => (await (window as unknown as ReviewWindow).__spec0004Phase2PortsV1!.readCheckpoint()).playbackState === "paused");
    ...
    await page.waitForFunction(async (digest) => (await (window as unknown as ReviewWindow).__spec0004Phase2PortsV1!.readCheckpoint()).documentDigest !== digest, beforeManual.documentDigest);
    const afterManualFrames = await captureEditableFrames(page, 12);
    const differences: Array<{frameIndex: number; jointIndex: number}> = [];
    for (let frameIndex = 0; frameIndex < 12; frameIndex += 1) {
      for (let jointIndex = 0; jointIndex < motionCases.jointRoleOrder.length; jointIndex += 1) {
    ...
    const reopenedFrames = await captureEditableFrames(page, 12);
    ...
    return {
      ...
      viewport,
      previewCandidateDigest: expected.motionCandidateDigest,
...
const main = async () => {
  ...
  assert.equal(motionCases.fixtureVersion, 2, "motion fixture version is exact");
  assert.equal(viewportFixture.fixtureVersion, 2, "viewport fixture version is exact");
  assert.deepEqual(motionCases.validCases.map((testCase) => testCase.name), FIXTURE_NAMES, "exact four fixed plans are covered");
  ...
  try {
    ...
    console.log("SPEC-0004 Phase 2 real-browser proof passed.");
