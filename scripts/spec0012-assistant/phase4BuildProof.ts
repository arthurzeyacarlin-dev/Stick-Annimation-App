...
const output = resolve("output/spec-0012/phase-4/build"); mkdirSync(output, { recursive: true });
...
for (const kind of ["full", "focused"] as const) {
  const env = phase1Environment(`phase4-build-${kind}`);
  ...
  if (kind === "full" && result.status !== 0) {
    ...
    if (kind === "focused") { const manifest = JSON.parse(readFileSync(".next/server/app-paths-manifest.json", "utf8")); for (const route of ["/page", "/assistant/page", "/api/ai-animator/route", "/api/diamond-assistant/route", "/favicon.ico/route"]) assert.ok(manifest[route], route); }
