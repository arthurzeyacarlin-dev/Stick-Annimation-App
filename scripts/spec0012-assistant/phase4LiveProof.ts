...
const output = resolve("output/spec-0012/phase-4-final-correction/live"); mkdirSync(output, { recursive: true });
...
try {
  ...
  await page.getByLabel("Reasoning level").selectOption("low"); assert.equal(await page.getByLabel("Reasoning level").inputValue(), "low");
  const greeting = "Hi, who are you?"; const greetingStartedAt = Date.now(); await send(greeting);
  const greetingReply = page.getByRole("article", { name: "Assistant reply", exact: true }); await greetingReply.waitFor({ timeout: 58000 }); const greetingElapsedMs = Date.now() - greetingStartedAt; await greetingReply.locator('[data-assistant-reveal="complete"]').waitFor({ timeout: 5000 });
  const greetingText = await greetingReply.innerText(); assert.match(greetingText, /Diamond Animator/i); assert.equal((await page.locator("body").innerText()).includes("Searching "), false); assert.ok(greetingElapsedMs < 58000);
  ...
  const searching = page.locator('[role="status"]').filter({ hasText: "Searching YouTube Shorts for information…" });
  const finalizing = page.locator('[role="status"]').filter({ hasText: "Finalizing answer" }); let finalizingAt = 0;
  const finalizingSeen = finalizing.waitFor({ timeout: 58000 }).then(() => { finalizingAt = Date.now(); return true; }, () => false);
  const searchStartedAt = Date.now(); await send(youtubePrompt); await searching.waitFor({ timeout: 30000 });
  assert.equal(await searching.locator("span").innerText(), "Searching YouTube Shorts for information…"); await page.screenshot({ path: resolve(output, "searching.png") }); screenshots.push("searching.png");
  const searchReply = page.getByRole("article", { name: "Assistant reply", exact: true }); await searchReply.waitFor({ timeout: 58000 }); const replyVisibleAt = Date.now(); const searchElapsedMs = replyVisibleAt - searchStartedAt; assert.equal(await finalizingSeen, true); assert.ok(finalizingAt > 0 && replyVisibleAt - finalizingAt >= 2700 && replyVisibleAt - finalizingAt < 5000); assert.ok(searchElapsedMs < 58000); await searchReply.locator('[data-assistant-reveal="complete"]').waitFor({ timeout: 5000 });
  const answerText = await searchReply.innerText(); assert.match(answerText, /YouTube Shorts/i); assert.match(answerText, /Diamond Animator/i); assert.doesNotMatch(answerText, /\b(?:I|we)\s+(?:watched|viewed|listened to|downloaded|inspected)\b/i); assert.doesNotMatch(answerText, /https?:\/\/|\*\*|\]\(/);
  ...
  writeFileSync(resolve(output, "result.json"), JSON.stringify({ status: "PASS", origin, liveCalls: 2, retries: 0, requestMessages: requests.map(request => request.message), greeting: { answerSha256: sha(greetingText), model: localSession.turns[0].usage?.model, toolCalls: 0, elapsedMs: greetingElapsedMs }, search: { answerSha256: sha(answerText), model: searchTurn.usage?.model, toolCalls: searchTurn.usage?.toolCalls, processedSourceCount: searchTurn.search?.processedSourceCount, displayedSources, estimatedCostUsd: searchTurn.usage?.estimatedCostUsd, elapsedMs: searchElapsedMs, finalizingMs: replyVisibleAt - finalizingAt }, totalEstimatedCostUsd: totalCostUsd, hardDeadlineMs: 55000, searchingTextObserved: true, finalizingObserved: true, reasoningChanged: true, citationsPersistedAcrossReload: true, projectSentinelUnchanged: true, screenshots, errors, external }, null, 2));
