...
export function StickFigureCanvas({
  ...
  const backgroundGraphs = useMemo(
    () => backgroundContents.map((content) => ({
      content,
      jointsById: new Map(content.structureGraph.joints.map((joint) => [joint.id, joint])),
    })),
    [backgroundContents],
  );
    ...
    <div style={{ flex: 1, minWidth: 0, padding: 14 }}>
      ...
      >
        ...
        >
          ...
          >
            {backgroundGraphs.flatMap(({content, jointsById: backgroundJoints}, layerIndex) =>
              content.structureGraph.limbs.map((limb) => {
                const startJoint = backgroundJoints.get(limb.startJointId);
                const endJoint = backgroundJoints.get(limb.endJointId);
                if (!startJoint || !endJoint) return null;
                return (
                  <line
                    key={`background-${layerIndex}-${limb.id}`}
                    x1={startJoint.x}
                    y1={startJoint.y}
                    x2={endJoint.x}
                    y2={endJoint.y}
                    stroke="rgba(16,19,27,0.62)"
                    strokeWidth="8"
                    strokeLinecap="round"
                  />
                );
          ...
          {backgroundContents.flatMap((content, layerIndex) => content.figures.map((figure) => (
            <div
              key={`background-figure-${layerIndex}-${figure.id}`}
              aria-hidden="true"
              style={{
                position: "absolute",
