const STICK_FIGURE_TOOL_NAMES = ["Select", "Lasso", "Brush", "Eraser", "Fill", "Text", "Knife"] as const;
export type StickFigureToolName = (typeof STICK_FIGURE_TOOL_NAMES)[number];

type StickFigureToolBarProps = {
  activeTool: StickFigureToolName | null;
  onSelectTool: (tool: StickFigureToolName) => void;
};

function StickFigureToolIcon({ tool }: { tool: StickFigureToolName }) {
  if (tool === "Knife") {
    return (
      <svg
        viewBox="0 0 42 42"
  ...
  if (tool === "Brush") {
    return (
      ...
      >
        ...
        <path
          d="M29 2 C36 2 38 8 34 13 C32 16 30 18 29 19 C28 18 26 16 24 13 C20 8 22 2 29 2 Z"
          fill="#ffffff"
...
export function StickFigureToolBar({ activeTool, onSelectTool }: StickFigureToolBarProps) {
  return (
    ...
    >
      {STICK_FIGURE_TOOL_NAMES.map((tool) => {
